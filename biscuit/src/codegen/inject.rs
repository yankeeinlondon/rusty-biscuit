...
use super::{CodegenError, validate_syntax};
use quote::ToTokens;
use std::fs;
use std::path::Path;
use syn::{File, Item};
...
fn remove_enum_definition(content: &str, enum_name: &str) -> Result<String, CodegenError> {
    ...
    ast.items
        .retain(|item| !matches!(item, Item::Enum(e) if e.ident == enum_name));

    let after_count = ast.items.len();
    if before_count > after_count {
        debug!("Removed {} enum definition(s)", before_count - after_count);
    } else {
...
pub fn inject_enum_variants(
    ...
    let (before_enum_end, _) =
        remove_autogenerated_text_section(&original_content[..enum_closing_brace], enum_start_pos);
    ...
    generated_section.push_str(&format!(
        "    // Generated: {} via ProviderModel::update()\n",
        timestamp
    ));
...
"#;
    ...
    // === END AUTO-GENERATED ===
        ...
        let new_variants = vec!["New__Variant__1".to_string(), "New__Variant__2".to_string()];
