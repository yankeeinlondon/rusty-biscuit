...
- **`current`** is the lazy counterpart and exists **only in lifecycle hooks**. Today it is spelled `current.ctx.<key>`; a ratified change will make it `current.<key>`, with exactly the shape of `ctx`. In a body, `current` is an undefined variable and renders empty without an error.
...
- **`cargo nextest run -p claudine-cli --test shipped_prompt_contract`** checks every file under `prompts/`: schemas parse, expressions parse, and no lifecycle value nests a brace span inside a string literal. Run it after any change to a prompt.
