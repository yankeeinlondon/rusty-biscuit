...
Text blocks get some *title love* too.
...
- Markdown links support the *standard* syntax links: [your text here](https://somewhere.com).
...
The same logic is applied when we use the `md` CLI’s `--clean` flag but in this case it is adding the language to the markdown content, not the *rendered* markdown content.
