...
space survives `xargs -0` and Bash's `read -r -d ''`::
    ...
    --deleted<NUL>gone.rs<NUL>--<NUL>gone.rs<NUL>kept.rs<NUL>
...
`--name-only`, which prints the destination alone. A rename's source is
reported nowhere: it is absent from the tree like a deletion, but the planner
documents `deleted` as a subset of the changed paths, and the guard's exemption
maintenance already reads the full tree in either scan mode, so a renamed-away
exemption is caught without listing the old path here.
...
def parse_name_status(stream: bytes) -> tuple[list[bytes], list[bytes]]:
    """The changed and deleted path lists of one `--name-status -z` stream.

    ## Returns

    `(changed, deleted)`, each in the order Git reported it. `deleted` is a
    subset of `changed`.
...
def arguments(changed: list[bytes], deleted: list[bytes]) -> bytes:
    """The NUL-delimited planner argument tail for one pair of lists."""
    parts: list[bytes] = []
    for path in deleted:
...
def main() -> int:
    ...
    try:
        changed, deleted = parse_name_status(sys.stdin.buffer.read())
    except ValueError as error:
        print(f"diff_scope: {error}", file=sys.stderr)
        return 2
    sys.stdout.buffer.write(arguments(changed + extra, deleted))
