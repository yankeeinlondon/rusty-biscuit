...
def _body_normalize(line: str) -> str:
...
def diff_body(before: list[str], after: list[str]) -> tuple[dict[str, int], list[str]]:
    """(changed lines by kind, the `other` lines) between an old file and its moved module."""
    counts = dict.fromkeys(BODY_KINDS, 0)
    others: list[str] = []
    matcher = difflib.SequenceMatcher(a=[_body_normalize(l) for l in before], b=[_body_normalize(l) for l in after], autojunk=False)
...
def body_diff(manifest: dict[str, Any], base: SourceReader, after: SourceReader) -> dict[str, Any]:
    ...
    is a failure: wave 2 permits structural edits only.
    """
    failures: list[str] = []
    ...
    for module in manifest["modules"]:
        ...
        for old_path, new_path in pairs:
            ...
            if old_text == new_text:
                identical += 1
                continue
            counts, others = diff_body(old_text.splitlines(), new_text.splitlines())
...
def render_body_diff_markdown(report: dict[str, Any], base_rev: str, after_label: str) -> str:
    lines = [
        ...
        f"its new module in {after_label}. `structural` is an attribute, `mod`, `use`, blank, or",
