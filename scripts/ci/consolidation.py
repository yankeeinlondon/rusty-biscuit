...
def _body_normalize(line: str, identity_prefix: str | None = None) -> str:
...
def diff_body(before: list[str], after: list[str], identity_prefix: str | None = None) -> tuple[dict[str, int], list[str]]:
    """(changed lines by kind, the `other` lines) between an old file and its moved module."""
    counts = dict.fromkeys(BODY_KINDS, 0)
    others: list[str] = []
    matcher = difflib.SequenceMatcher(a=[_body_normalize(l, identity_prefix) for l in before],
                                      b=[_body_normalize(l, identity_prefix) for l in after], autojunk=False)
...
def body_diff(manifest: dict[str, Any], base: SourceReader, after: SourceReader) -> dict[str, Any]:
    ...
    own module prefix on a test path (R19's self-exec repair).
    """
    failures: list[str] = []
    ...
    for module in manifest["modules"]:
        ...
        for old_path, new_path in pairs:
            ...
            counts, others = diff_body(old_text.splitlines(), new_text.splitlines(), prefix)
