...
Lists all models across all runners in a table or JSON.
...
Shows detailed metadata for a model. If multiple models match, presents an interactive selection prompt.
...
Searches HuggingFace for GGUF models. Uses `biscuit-terminal::Table` for rich output with OSC8 hyperlinks.
...
### `model download <repo> [variant]`

Downloads GGUF variants from HuggingFace with `indicatif` progress bars.

```bash
model download bartowski/Qwen2.5-7B-Instruct-GGUF
model download bartowski/Qwen2.5-7B-Instruct-GGUF Q4_K_M --output ~/models
```

Without a variant argument, shows interactive multi-select with size/RAM estimates.

### `model remove <model>`

Removes models with confirmation. Shows symlink warnings for dependent links.
