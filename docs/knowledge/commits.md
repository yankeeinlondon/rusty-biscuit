...
- In concurrent subagent commit workflows, do not assume `HEAD` still points to your commit by the time you verify it. Another worker may advance the branch immediately afterward. Capture the commit hash from `git commit` output and verify that specific object with `git show -s <hash>` instead of relying on `git log -1`.
