...
---
I'll start by understanding the existing document and invoking the relevant skills.This is largely a writing task drawing on domain knowledge, but I want the recommendations grounded in the actual repo. Let me gather context on the CLIs and any existing release infrastructure.I have everything I need. The repo already runs `release-plz` (source releases/tags, `publish = false`, all crates use `path =` deps), Claudine's binary is `claudine`, Darkmatter's is `md`, both AGPL-3.0-only. That grounds the recommendations. Here is the replacement Markdown body.
