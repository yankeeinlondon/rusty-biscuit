...
is `complete typed Document -> one target fold`, plus the explicitly
excluded validation and page-frame work, satisfying acceptance
criterion 3.
...
- `terminal_unmatched_policy_does_not_flip_color_depth_for_unrelated_content`
  (review-4 finding 1) — at both a pinned no-color depth and a pinned TrueColor
