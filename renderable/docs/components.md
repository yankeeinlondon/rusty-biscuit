...
| Table           | Block  | ✅       | ✅      | ✅       | ✅   | both avail, tree renders     | tree          | `biscuit-terminal/lib/src/components/table/table.rs`         | A data table with typed columns, planned widths, alignment, and `StyledProse` cells that embed capability-aware inline Prose (links, emphasis, color) directly into the cell without pre-rendering. |
