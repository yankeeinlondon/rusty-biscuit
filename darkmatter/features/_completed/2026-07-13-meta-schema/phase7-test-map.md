...
| Definition hover separates the semantic artifact from every denoted union arm | `providers::frontmatter::tests::meta_schema_hover_renders_every_denoted_arm_and_constraints` covers `foo: string(required)` and a mixed `string`/native-object union. `lsp_session.rs::meta_schema_phase7_inline_hover_completion_and_diagnostics` parses the equivalent native YAML union plus a quoted scalar and asserts the rendered LSP Markdown. |
| Parser-state completion works for inline values, constraints, scaffolds, semantic arrays, and standalone pure/tagged documents | `providers::frontmatter::tests::meta_schema_completion_catalog_is_descriptor_driven` proves every shipped type descriptor is offered, valid constraints are filtered by the selected type, quoted and missing values preserve edit ranges, passive named types are offered, and `type-definition[]`/`schema[]` items delegate to scalar and path completion. `lsp_session.rs::meta_schema_phase7_inline_hover_completion_and_diagnostics` covers the normal inline request path and a malformed partial. `lsp_session.rs::meta_schema_phase7_standalone_pure_and_tagged_completion` covers pure mapping/root-union `$schema` and tagged `kind: schema` → `types` through real completion requests. |
...
| Typed activation is available to the deferred semantic-token family without fine-grained tokenization | `overlay::tests::frontmatter_meta_schema_activation_uses_effective_property_definitions` covers frontmatter `type-definition` and `schema` activation, while `overlay::schema::tests::semantic_type_regions_project_existing_activation_state` covers standalone scalar, native mapping, and union definitions with exact document byte spans and semantic kinds. Semantic-array gating is exercised by the completion test above. |
