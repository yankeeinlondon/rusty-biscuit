...
- [x] Define the visibility contract explicitly: all directives admitted to one stage see its opening immutable policy snapshot; approvals/persistence produced during that stage update the runtime but become policy input only for a subsequent stage. Add tests for both halves of that contract. (**Superseded at review-1 — see `results.md` → Finding 32.** This planned "frozen stage" contract was implemented, then found to change user-visible prompt frequency without owner approval; review-1 required either that approval or preservation of prior behavior. The behavior-preserving branch was taken. The contract is now: each directive sees a policy view taken at its own entry, so a rule persisted mid-stage **does** suppress a later same-stage prompt — Half 2's test was flipped to assert 1 approval and renamed `persistence_mid_stage_is_policy_input_for_the_same_stage`. Finding 32's actual goal — no per-directive rule-set clone — is still met, by sharing the collections behind `Arc` with copy-on-write writes rather than by freezing the stage. Half 1 (`persisted_whitelist_from_one_stage_is_policy_input_for_the_next_stage`) and the allow-once exemption (`allow_once_still_dedupes_within_a_single_stage`) are unchanged.)
