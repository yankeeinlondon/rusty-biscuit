...
- [ ] **Audit (parallelizable during Phase 1):** Compare the baseline property
  ...
  contract.
- [ ] **Resolve Open Question 2** (`style` shape): decide whether `style`
...
- [ ] **Resolve Open Question 3** (`change` validation shape): confirm `change`
  stays `any` in v1 (per Non-Goal 2) or narrow it if the delta surface
  documents a stable contract. Default: keep `any`.
- [ ] **Resolve Open Question 4** (generated property tables in docs): decide
...
- [ ] Freeze `darkmatter/docs/schemas/darkmatter.yaml`: remove the
...
- [ ] Confirm the deprecated root-level `hr` is **not** present in the schema
  (Non-Goal 6) and that `style.hr.*` is the only horizontal-rule surface.

### Validation Checkpoint

- [ ] `darkmatter/docs/schemas/darkmatter.yaml` parses cleanly via
  `parse_yaml_schema` (Phase 1 grammar) — verified by a test that reads the
  file with `include_str!` and parses it.
- [ ] The parsed schema converts to a baseline-compatible Draft 2020-12 JSON
  Schema via `to_json_schema` without errors.
- [ ] Audit findings are recorded (a short note in the feature directory or a
...
- [ ] Add a public `darkmatter_base_schema() -> SimplifiedSchema` function
...
- [ ] Add a companion accessor that returns the compiled Draft 2020-12 JSON
  ...
  cost.
- [ ] Add a `ComposeOptions` convenience method (e.g.
...
- [ ] If a checked-in generated Rust artifact is preferred over `include_str!`
  ...
  justifies it.
- [ ] Re-export the new accessors from `darkmatter/lib/src/lib.rs` so they are
  reachable at `darkmatter::darkmatter_base_schema` (or the chosen namespace).

### Validation Checkpoint

- [ ] Unit test: `darkmatter_base_schema()` returns a non-empty
  `SimplifiedSchema` whose `$schema`, `title`, `ctx` properties are present.
- [ ] Unit test: the compiled JSON Schema validates a known-good frontmatter
  sample and rejects a known-bad `title` (wrong type).
- [ ] Unit test: `ComposeOptions::new().with_darkmatter_baseline_schema()`
  injects the baseline and still allows unknown user keys (Non-Goal 1) and
  preserves document `$schema` precedence (Non-Goal 5).
- [ ] `just test` + `just doctest` pass; the new public symbols have rustdoc
...
- [ ] Remove `merge_deprecated_top_level_hr` and its caller in
...
- [ ] Remove the root-`hr` read in `render_tree/entrypoints.rs`:
  ...
  `style.hr`.
- [ ] Update or delete the tests in `style/parse.rs` that exercise
...
- [ ] Update render-tree tests in `entrypoints.rs` that feed root `hr` to
  `hr_defaults_from_frontmatter`.
- [ ] Update any docs/skill text that still advertises root `hr` as a
...
- [ ] If a migration diagnostic is desired (optional), add it as a one-time
...
- [ ] A document with root-level `hr: { color: red-500 }` no longer affects
  rendering; only `style: { hr: { color: red-500 } }` does.
- [ ] `just test` + `just test-l2` pass in `darkmatter/` with no remaining
  references to root `hr` in production code (only historical/feature docs).
- [ ] `rg "\"hr\"" darkmatter/lib/src/style/parse.rs darkmatter/lib/src/markdown/render_tree/entrypoints.rs`
...
- [ ] Create `darkmatter/docs/schemas/darkmatter-schema.md` with the intro
  ...
  `include_str!`s.
- [ ] Add prose documenting: (1) the base schema is added by default by the
...
- [ ] If Open Question 4 resolved in favor of generated property tables, add
  them here as generated content (otherwise omit — transclusion only).
- [ ] Verify the transclusion resolves: compose the doc file (or run the docs
  build) and confirm the schema YAML appears inline.

### Validation Checkpoint

- [ ] The transclusion path `./darkmatter.yaml` resolves to the exact file
  used by `darkmatter_base_schema()` (spec testing requirement 7).
- [ ] `just lint` passes; the new doc follows repo Markdown conventions.
...
- [ ] **Resolve Open Question 1:** Decide whether `md compose` injects the base
  ...
  section once made.
- [ ] If auto-injecting: wire `darkmatter_base_schema()` into the CLI compose
...
- [ ] Do **not** change the existing `md schema validate` baseline contract
...
- [ ] Add a `--baseline-schema`/`--no-baseline-schema` toggle to `md compose`
  if auto-injection is the default.

### Validation Checkpoint

- [ ] CLI integration test: `md compose` on a doc using `ctx.today` (a
  `generated` property) succeeds without the author declaring `ctx`.
- [ ] CLI integration test: a doc with `$schema: { title: number }` still
  fails validation (document `$schema` precedence preserved).
- [ ] CLI integration test: a doc with an unknown `custom_key: 42` composes
  without error (unknown keys allowed).
- [ ] `just test` + `just test-l2` pass.
...
- [ ] Spec test 1: `darkmatter/docs/schemas/darkmatter.yaml` parses as
  referenced `SimplifiedSchema`.
- [ ] Spec test 2: the parsed schema converts to a baseline-compatible JSON
  Schema.
- [ ] Spec test 3: known valid frontmatter examples pass validation.
- [ ] Spec test 4: invalid known-property values fail validation (e.g.
  `title: 42`, `draft: "maybe"`).
- [ ] Spec test 5: unknown user-defined frontmatter keys remain accepted.
- [ ] Spec test 6: document `$schema` definitions override baseline properties
  on conflict.
- [ ] Spec test 7: the documentation transclusion path points at the same
  schema file used by source integration.
- [ ] Spec test 8: nested mapping object syntax lowers to the same JSON Schema
  as the existing quoted object-literal syntax (equivalence test).
- [ ] Spec test 9: sequence union arms accept nested mapping object shapes.
- [ ] Spec test 10: `generated` properties are omitted from static-document
...
- [ ] Cross-platform build check: confirm `cargo build` (or `just build`)
...
- [ ] `just lint` clean across `darkmatter/` (lib + cli).
- [ ] `just doctest` clean — new public symbols have runnable doc examples.
- [ ] Update `.claude/skills/darkmatter/SKILL.md` "Schema Validation" section
  to mention the base schema is now a first-class library surface (drift
  maintenance per AGENTS.md).
- [ ] Final review: confirm Non-Goals 1–6 are honored (no unknown-key
...
- [ ] All 10 spec tests above pass as automated tests in the repo.
- [ ] `just test`, `just test-l2`, `just lint`, `just doctest` all green in
  `darkmatter/`.
- [ ] From repo root: `just test darkmatter` green.
