...
- Base schema parsing, conversion, exported accessors, known valid/invalid values, unknown top-level keys, document `$schema` precedence, and open `ctx` behavior have appropriate Level 1 tests.
...
- `md compose` default baseline injection, `--no-baseline-schema`, `DARKMATTER_NO_BASELINE_SCHEMA`, explicit baseline replacement, and custom `ctx.*` acceptance have appropriate Level 1 CLI tests.
...
The previous high-severity `ctx.*` regression appears fixed: the base schema now models `ctx` as an open `object`, and the default-compose path has Level 1 coverage for custom user context. The root-level `hr` compatibility removal also appears covered by behavior tests and corrected public rustdoc.
