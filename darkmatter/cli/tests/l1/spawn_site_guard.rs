...
//!
//! `level2_*`, `level3_*`, `browser_*`, `real_*`, and `slow_*` files, and every
//! file under the matching test-target directory (`level2/`), are out of both
//! scans — the same prefixes the area's `_tier_filter` drops from L1.
...
// This guard is the sanitizer's only user, so `common/mod.rs` does not declare it.
...
use crate::common::protected_env::{ProtectedClass, protected_class};
...
fn excluded(relative: &str) -> bool {
    ...
    // A consolidated target directory is named for its tier (`level2/`), which
    // covers a file whose `level2_` name became a neutral module alias
    // (`level2/harness_integrity.rs`) and the target's own `main.rs`.
...
fn tier_naming_decides_what_the_guard_governs() {
    ...
    // The consolidated Level 2 target: a prefixed file, the aliased file whose
    // module name lost the prefix, and the target's root.
    ...
    // Only a whole directory segment names a target; a prefix inside one does not.
