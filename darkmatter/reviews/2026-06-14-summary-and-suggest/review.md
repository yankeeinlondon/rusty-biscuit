...
| 5 | Fix compose phase docs (three → four) | Already implemented | resolution-context (touched `mod.rs:560-567` for the frontmatter ordering invariant); three→four fix from summary-and-suggest plan | `compose/mod.rs:4` says "four phases"; `:571` lists Inline Pre / Transclusion / Inline Post / Finalization; also confirmed resolution-context's stale-comment flips landed (`expression/mod.rs:192-196`, `:530-537`, `frontmatter_interpolation.rs:1-8`) |
