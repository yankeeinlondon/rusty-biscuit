...
- [ ] **1.1 — Record the exact pre-fix worktree state.** Capture `git rev-parse HEAD`,
...
- [ ] **1.2 — Refresh impact analysis on the current index.** Run GitNexus `impact` with
...
- [ ] **1.3 — Quiesce the host, then capture the named Criterion baseline.** Close other
...
- [ ] **1.4 — Transcribe baseline numbers.** For each of the three fixtures record sample
...
- [ ] **2.1 — Extract the shared engine.** Add a private
...
- [ ] **2.2 — Reduce `validate_with_graph` to the checked seam.** Its body becomes
...
- [ ] **2.4 — Rewrite `validate` to use it.** Keep the `build_reference_graph` call and its
...
- [ ] **2.5 — Preserve tracing behavior, including rejected prebuilt graphs.**
...
- [ ] **2.6 — Fix the drifted docs in the same change** (CLAUDE.md authoring discipline;
...
- [ ] **3.1 — Swap the call.** In `file_tree/mod.rs:243-250`, replace
...
- [ ] **3.2 — Keep the freshness invariant visible.** `val_opts.graph = self.graph_options.clone()`
...
- [ ] **3.3 — Replace the stale Finding-18 comment.** The comment at `file_tree/mod.rs:241-242`
...
- [ ] **3.4 — Review `ensure_built`'s rustdoc for drift.** Its public contract (lazy,
...
- [ ] **4.1 — Add the focused mechanism test** in `validate.rs`'s `#[cfg(test)] mod tests`.
...
- [ ] **4.2 — Run the focused reference suites.**
...
- [ ] **4.3 — Confirm the two load-bearing existing assertions still hold.**
...
- [ ] **4.4 — Prove the fresh seam is unreachable from the public API.** Verify by inspection
...
- [ ] **5.1 — Run the paired candidate benchmark** in the same session, on the same host, with
...
- [ ] **5.2 — Check the acceptance thresholds** against the Criterion comparison:
      - `multi_transclusion/build_and_validate` MUST improve by **both ≥10% and ≥500 µs** at
        the median. (Expected: the ~4.15 ms redundant floor disappears, so ~10.5 ms → ~6.2 ms.)
      - **No** `build_and_validate` fixture may regress by both >5% and >100 µs at the median.
        `small` and `large` have empty dependency manifests and should be roughly flat.
      If confidence intervals are wide or the fixture runs are noncomparable, rerun the
      **baseline and candidate as a pair** — never compare against Review 4's older numbers as
      pass/fail evidence.
- [ ] **5.3 — Run the final unfiltered benchmark** (`cargo bench -p darkmatter --bench reference_graph`)
      and confirm `validate_prebuilt` is still materially faster than `build_and_validate` for
      each fixture. That gap is the original Finding-18 reuse win and remains part of the feature
      contract even though `build_and_validate` just got much cheaper.
- [ ] **5.4 — Write `darkmatter/fixes/2026-07-16-redundant-walk/results.md`** containing, per
...
- [ ] **5.5 — Verify the benchmark's own comments are still accurate.** `reference_graph.rs:10-22`
...
- [ ] **6.1 — `just build`** from `darkmatter/`. Covers `darkmatter`, `darkmatter-cli`, and `dmls`.
- [ ] **6.2 — `just test`** from `darkmatter/`. Full area unit suite must be green.
- [ ] **6.3 — `just lint`** from `darkmatter/`. Clippy must be clean — watch for a
      `dead_code` warning if any helper ended up unreachable, which would signal a bad split.
- [ ] **6.4 — `git diff --check`** for whitespace damage.
- [ ] **6.5 — GitNexus change analysis.** Run worktree-scoped `detect_changes()` and compare its
      ...
      `markdown::reference`.
- [ ] **6.6 — Confirm the no-change surfaces.** Public Rust signatures, `ReferenceError` variants,
...
- [ ] **6.7 — Acceptance-criteria sweep.** Walk spec §"Acceptance Criteria" 1–9 and tick each
      against concrete evidence produced above (AC1→2.4, AC2→3.1, AC3→2.2, AC4→2.1+4.3,
      AC5→4.1, AC6→4.2, AC7→6.6, AC8→5.4, AC9→6.1-6.5).
- [ ] **6.8 — Report and stop.** Summarize the outcome for the user. **Do not commit** — commits
