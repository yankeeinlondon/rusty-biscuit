...
|---|-----------|-------|--------|
| 1 | A caller-supplied context without the group fails with a typed error naming the variable, group, and source | `missing_ctx_capture::body_reference_to_an_uncaptured_group_names_variable_group_and_source`; `missing_ctx_capture::the_regression_input_fails_instead_of_rendering_empty_values` (verbatim regression input, both `fail_fast` modes); `ambient_ctx_capture::caller_supplied_minimal_context_fails_instead_of_rendering_uncaptured_groups_empty`; `cli/tests/compose_interpolation.rs::test_compose_uncaptured_context_group_exits_nonzero_naming_variable_and_group` | Phase 3 |
...
| 4 | Unavailable supplied evidence keeps `PartialRuntimeCapture` and is not "uncaptured" | `missing_ctx_capture::a_captured_group_without_evidence_renders_its_typed_projection` (asserts the partial-capture diagnostic, the typed empty projection, and no missing-capture warning); `ambient_ctx_capture::a_capture_without_supplied_evidence_projects_every_key_of_each_captured_group` | Phases 2, 3 |
