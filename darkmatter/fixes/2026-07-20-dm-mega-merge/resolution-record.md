...
- Authority boundary: Darkmatter behavior documentation; generated hash is
  deferred until the resolved body is final.
...
- Evidence/follow-up: the frontmatter temporarily retains the Darkmatter-parent
  hash `87f17662fa397abe-7b030c5a6ebabb92`; it does not describe the merged body.
  Phase 7 must refresh and verify the Markdown-aware hash. The index entry
  remains intentionally unmerged because the operator forbids staging.
...
- Evidence/follow-up: the marker-free union preserves non-interactive
  ...
  `git commit -F - --only -- <paths>` ordering contract. The index entry remains
  intentionally unmerged because the operator forbids staging.
...
- Resolution status: Phase 4 working-tree resolution complete using the exact
  ...
  execution flows.
- Evidence/follow-up: these are explicitly temporary parent values, not merged
  tree counts. The dirty source-worktree count refresh was not imported. Phase
  7 must refresh GitNexus once and replace them. The index entry remains
  intentionally unmerged because the operator forbids staging.
...
- Evidence/follow-up: A3-01 through A3-05 and T3-01. The incoming file-local
  ...
  `#[serial(level2_terminal)]`; execution remains deferred to the owning
  `just test-l2` recipe in Phase 5. The index entry remains intentionally
  unmerged because the operator instruction for this phase forbids staging.
...
- Evidence/follow-up: A3-01 through A3-05 and T3-01. Exactly one
  ...
  and every invocation therefore uses the Cargo-built binary shim. The index
  entry remains intentionally unmerged because the operator instruction for
  this phase forbids staging.
...
- Evidence/follow-up: Review 8 is retained; Review 7 now points to Review 8;
  ...
  and retain the open quiet-host evidence requirement. Review 8's index entry
  remains intentionally unmerged because the operator forbids staging.
...
- **G3-03 — area tests:** the final warm-cache `just test` attempt passed
  ...
  package runs were therefore not executed. This broad gate remains incomplete
  and must be rerun in an environment that permits the full recipe duration.
...
|---|---|
| Skill union and deferred generated hash | Parent-delta audit plus marker scan; the Darkmatter-parent hash is recorded as temporary and Phase 7 remains responsible for `md hash --save` / `--diff`. |
...
- **G4-04 — marker and index audit:** 199 changed text files were scanned. The
  ...
  remains. `git ls-files -u` intentionally lists the four Phase 4 and two Phase
  3 entries because the operator forbids staging; their working content is
  marker-free and prepared for the separate staging step.
...
## Phase 7 metadata and final-audit evidence

Phase 7 introduces no runtime behavior change and therefore adds no regression
test. The Darkmatter-aware hash path updated the merged skill frontmatter to
`87f17662fa397abe-c0eb7c8a0924fdd4`; the immediate `--diff` verification
exited 0 with no semantic change. Locked metadata reports 72 workspace members,
the ignored lockfile remains byte-identical at
`52c2a58dc23331afe1cd82424ea5b295123ed4c1f40275fd35a400709d9ef286`, and
`git diff --check` passes.

The required final GitNexus refresh is blocked. The repository-local runner is
absent, the global command is incompatible with the default Node 16 runtime,
and the compatible Node 22 forced refresh exceeded the non-interactive
60-second ceiling after parsing/resolving the corpus. It was stopped with exit
130 before persistence/registration. `CLAUDE.md` consequently retains its
explicit temporary parent counts. The existing service still reports HIGH
`scope=all` impact (870 symbols, 206 indexed files, 13 processes) and CRITICAL
`compare main` impact (4,591 symbols, 855 indexed files, 78 processes), but
these results are labeled stale-index evidence.

The four working-tree comparisons cover 206 paths versus the Darkmatter
parent, 160 versus more-is-more, 348 versus the merge base, and 924 versus
`main`. Expected parent contributions reconcile with Phase 5 focused tests and
Phase 6 area gates. No unexpected runtime authority, snapshot, or marker was
found. The one snapshot pair is the reviewed Sniff host-value normalization;
the only marker-shaped text is the reviewed equals-only Windows route fixture.

Source refs and both backup refs remain intact. Source dirty-file identities
remain frozen except for the explicitly authorized source-plan documentation
delta required to check off completed phase tasks. The quiet-host and
invalid-frontmatter timing/native-platform gaps remain open and accurately
labeled.

The final staged-index audit cannot be completed because the operator
explicitly prohibited staging. Six marker-free resolutions remain unmerged in
the index, `git ls-files -u` is non-empty, and the staged/unstaged/status views
therefore cannot satisfy the Phase 7 handoff invariant. `merge-report.md`
records the complete evidence and exact follow-up. No commit, tag, push,
worktree deletion, or source-branch update was performed.

### Phase 8 implementation-request verification

The implementation-request numbering treats plan phases 0–7 as phases 1–8.
No implementation behavior changed in this final closure pass. Sniff discovery
was replayed from the integration root and locked Cargo metadata again reported
72 packages and 72 workspace members. GitNexus replayed the same HIGH
`scope=all` summary (870 symbols, 206 files, 13 processes) and CRITICAL
`compare main` summary (4,591 symbols, 855 files, 78 processes).

The Darkmatter area's `just lint` recipe passed again for `darkmatter`,
`darkmatter-cli`, and `dmls`. A duplicate `just test` replay passed 2,607 tests
with zero failures before it reached the non-interactive 60-second ceiling and
was interrupted; this bounded partial replay does not replace or weaken the
completed Phase 6 result of 6,084 Darkmatter, 643 CLI, and 640 DMLS tests
passing. No test, source, schema, template, prompt, configuration, or skill-body
file was changed in this closure pass.
