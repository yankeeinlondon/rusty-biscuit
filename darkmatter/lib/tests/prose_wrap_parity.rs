...
//! stripped) wrapped layout matches the legacy `for_terminal` renderer.

use darkmatter::markdown::Markdown;
use darkmatter::markdown::output::{ColorDepth, TerminalOptions, for_terminal};
...
/// The tree prose wrap must match the legacy `for_terminal` renderer on the
/// two load-bearing properties: every visible line stays within the content
/// width, and no word is lost or reordered (the concatenated words are equal).
///
/// Exact line-break columns are NOT asserted. The tree reuses the shared
/// `wrap_lines`/`WrapProse` primitive (also driving lists and two-column
/// layout), whose break point can differ from the legacy greedy `LineWrapper`
/// by one word in the exact-fit edge case (a word that ends exactly at the
/// width with no following whitespace inside the width): legacy keeps it on the
/// line, `WrapProse` breaks before it. Both produce within-width, lossless
/// output; the divergence is a benign break-column difference, not a fidelity
/// regression. Matching it byte-for-byte would require changing the shared
/// wrapping primitive, which lists/columns/blockquotes also depend on.
#[test]
fn tree_prose_wrap_matches_legacy_within_width_and_lossless() {
    ...
    let tree = render_tree_terminal(&md, &opts).expect("tree render").output;
    let legacy = for_terminal(&md, opts).expect("legacy render");

    let tree_words: Vec<String> = strip_ansi(&tree).split_whitespace().map(String::from).collect();
    let legacy_words: Vec<String> =
        strip_ansi(&legacy).split_whitespace().map(String::from).collect();
    assert_eq!(
        tree_words, legacy_words,
        "tree and legacy must preserve the same words in order",
    );
...
/// Block quotes rendered through the tree terminal path must carry the legacy
/// quarter-block bar (`▐`), not the shared `BlockQuote` component's `│` border.
/// This is the Stage 3 glyph-parity guard: the darkmatter terminal entry point
/// sets `blockquote_prefix = "▐   "`, matching the legacy `for_terminal`
/// renderer that the decorated-layout L2 guards pin.
#[test]
