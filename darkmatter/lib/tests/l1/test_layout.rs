...
fn every_test_source_is_compiled_by_a_declared_target() {
    ...
    // module on every host, the flattened `error_snapshots` crate, a helper
    // directory beside its module, and the helpers the roots include by path.
    assert!(files.len() > 100, "scanned only {} test source file(s)", files.len());
    for expected in [
        "tests/l1/main.rs",
        "tests/l1/declined_path_transclusion.rs",
        "tests/l1/error_snapshots/helpers.rs",
        "tests/level2/level2_render_tree_terminal/support/mod.rs",
        "tests/level3-browser/level3_popover.rs",
        "tests/image_test_support/mod.rs",
