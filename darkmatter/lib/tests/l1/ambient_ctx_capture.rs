...
fn render_every_variable(options: ComposeOptions) -> HashMap<String, String> {
    let document: String = context_variable_descriptors().iter()
...
fn every_catalog_variable_survives_ambient_options() {
    ...
    let mut categories: Vec<&str> = context_variable_descriptors().iter()
    ...
    let silent: Vec<&str> = categories
        .into_iter()
        .filter(|category| {
            !context_variable_descriptors().iter()
    ...
    let blanked: Vec<&str> = context_variable_descriptors().iter()
...
fn descriptors_by_requirements() -> Vec<(ContextRequirements, Vec<&'static str>)> {
    ...
    for descriptor in context_variable_descriptors().iter() {
        let requirements =
