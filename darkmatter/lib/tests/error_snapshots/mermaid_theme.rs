...
#[test]
fn invalid_json_surfaces_line_and_column() {
    let parse_error = serde_json::from_str::<serde_json::Value>("{ bogus }").unwrap_err();
    let err = MermaidThemeError::InvalidJson(parse_error);
    let out = render(&err);
    assert_contains_all(&out, &["MermaidThemeError", "invalid JSON", "line", "column"]);
}
