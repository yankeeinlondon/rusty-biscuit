...
use crate::markdown::transform::interpolation::{ComparisonOp, Expr, parse};
...
pub fn evaluate_condition(
    ...
    let parsed = parse(expr).map_err(|e| TransclusionError::ConditionParse {
        expr: expr.to_string(),
        line,
        message: e.to_string(),
    })?;

    let value = eval_expr(&parsed, state).map_err(|message| TransclusionError::ConditionEval {
        expr: expr.to_string(),
        line,
        message,
    })?;
