...
/// Source info derived from a transform source.
...
impl SourceContext {
    /// Builds source context from a transform source.
    pub fn from_source(source: &TransformSource) -> Self {
        match source {
            TransformSource::Unknown => Self::default(),
            TransformSource::File(path) => Self {
                file: Some(path.clone()),
                url: None,
            },
            TransformSource::Url(url) => Self {
                file: None,
                url: Some(url.clone()),
            },
        }
