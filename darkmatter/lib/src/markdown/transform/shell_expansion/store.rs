...
///
/// ```no_run
/// use darkmatter::markdown::transform::shell_expansion::store::resolve_policy_paths;
...
pub fn resolve_policy_paths(
    ...
    let policy_dir = if let Some(ref policy_root) = shell_opts.policy_root {
        ...
        let base_dir = match source {
            ...
            TransformSource::Url(_) | TransformSource::Unknown => {
                std::env::current_dir().map_err(|e| ShellExpansionError::PolicyIo {
                    path: PathBuf::from("."),
                    source: e,
                })?
...
///
/// ```no_run
/// use darkmatter::markdown::transform::shell_expansion::store::load_ruleset;
/// use std::path::Path;
///
...
///
/// ```no_run
/// use darkmatter::markdown::transform::shell_expansion::store::append_whitelist_exact;
...
///
/// ```no_run
/// use darkmatter::markdown::transform::shell_expansion::store::append_whitelist_prefix;
/// use darkmatter::markdown::transform::shell_expansion::types::ShellPolicyPaths;
/// use std::path::PathBuf;
///
/// let paths = ShellPolicyPaths {
...
///
/// ```no_run
/// use darkmatter::markdown::transform::shell_expansion::store::append_blacklist_exact;
/// use darkmatter::markdown::transform::shell_expansion::types::ShellPolicyPaths;
/// use std::path::PathBuf;
///
/// let paths = ShellPolicyPaths {
...
mod tests {
    ...
    fn resolve_policy_paths_from_file_source() {
        ...
        let options = ShellExpansionOptions::default();
        let source = TransformSource::File(file_path);
