...
pub const PURE_FUNCTIONS: &[PureFunction] = &[
    ...
    // Date arithmetic
    PureFunction { canonical: "date_delta", aliases: &["datedelta"], signatures: &["date_delta(date1, date2, diff)"], handler: date_delta },
    PureFunction { canonical: "older_than", aliases: &["olderthan"], signatures: &["older_than(date1, date2, diff)"], handler: older_than },
    PureFunction { canonical: "newer_than", aliases: &["newerthan"], signatures: &["newer_than(date1, date2, diff)"], handler: newer_than },
];
...
mod tests {
    ...
    mod fn_date_arithmetic {
        ...
        fn null_propagates_and_bad_input_errors() {
            assert_eq!(
                date_delta(&vec![json!(null), json!("2024-06-20"), json!("14d")]).unwrap(),
                json!(null)
            );
            // Non-string date arg → type-mismatch error.
            assert!(date_delta(&vec![json!(5), json!("2024-06-20"), json!("14d")]).is_err());
