...
fn execute_timeout_fallback_emits_warning() {
    ...
    let options = ComposeOptions::new().with_shell(ShellExpansionOptions {
        ...
        timeout_behavior: super::super::shell_expansion::types::ShellTimeoutBehavior::EmptyString,
        policy_root: Some(temp_dir.path().to_path_buf()),
        approval_handler: Some(Arc::new(MockApproval)),
        ..Default::default()
    });
...
"#,
    ...
    .unwrap();
    let barrier_cmd =
        |own: &str, other: &str| format!("$({} barrier.py {own} {other})", python.display());
    let mut fm = fm_from_json(json!({
    ...
    let pool = rayon::ThreadPoolBuilder::new()
        ...
        .unwrap();
    let report = pool
        .install(|| execute_frontmatter_shell_expansion(&mut fm, &options, &mut runtime, None))
        .unwrap();
