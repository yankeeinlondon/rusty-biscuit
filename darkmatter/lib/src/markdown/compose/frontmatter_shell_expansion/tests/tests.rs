...
fn detects_no_cache_suffix() {
    ...
    let directive = parse_shell_value(&value, "key", None).unwrap().unwrap();
...
fn leak_guard_ignores_plain_and_mixed_values() {
    ...
    fm.insert("mixed_prefix", json!("literal $(echo ok)"))
        .unwrap();
    fm.insert("mixed_suffix", json!("$(echo ok) trailing"))
        .unwrap();
...
fn split_top_level_ternary_parens_protect_punctuation() {
    ...
    let (cond, then_s, else_s) = super::split_top_level_ternary("(a ? b) ? then_cmd : else_cmd")
...
fn ternary_with_no_command_branch_errors_with_brace_suggestion() {
    ...
    let err = parse_shell_value("$( file_exists('x') ? 'a' : 'b' )", "spec", None).unwrap_err();
    match err {
        ShellExpansionError::ParseDirective { message, .. } => {
            assert!(
                message.contains("no shell command") && message.contains("{{"),
...
fn mixed_expression_condition_with_command_branches_parses() {
    ...
    let directive = parse_shell_value(
