...
use remote_signal_core::{
    ...
    PayloadKind, SignedEnvelopeWire, SyncAdvertiseEnd, SyncChunkAdvertise, SyncDelta, SyncEnd,
    SyncFrame, SyncHello, identity::PUBLIC_KEY_LENGTH, sync_frame,
};
...
impl SyncService {
    ...
    pub async fn sync_initiator(
        ...
        self.run_session(send, recv, Some(expected_peer_node_id.to_string()))
            .await
    }
        ...
        let work = async {
                ...
                .filter(|c| c.owner_node_id == local_node_id)
            {
                ...
                let envelope =
                    self.sealer
                        .lock()
                        .seal(document_id.clone(), payload_kind, exported.bytes);
...
mod tests {
    ...
    use remote_signal_core::{
        ChunkConfig, ChunkId, Entry, EnvelopeSealer, NodeIdentity, PayloadKind,
    };
    ...
    fn invalid_created_at_in_metadata_rejected_without_persistence() {
        ...
        let snapshot =
            make_snapshot_wrong_metadata("bad-ts", &peer_hex, "bad-ts-session", 0, -1, None);
        let envelope = harness
            .peer_sealer
            .seal(chunk.as_path(), PayloadKind::Snapshot, snapshot);
    ...
    fn metadata_owner_mutation_on_existing_chunk_rejected() {
        ...
        let valid_snapshot =
            make_valid_loro_snapshot("initial", &peer_hex, "mutate-owner-session", 0);
        ...
        let mutated =
            make_mutated_metadata_snapshot(&valid_snapshot, "owner_node_id", "impostor-node");
        let second_envelope =
            harness
                .peer_sealer
    ...
    fn metadata_chunk_index_mutation_on_existing_chunk_rejected() {
        ...
        let valid_snapshot =
            make_valid_loro_snapshot("initial", &peer_hex, "mutate-idx-session", 0);
    ...
    fn metadata_created_at_mutation_on_existing_chunk_rejected() {
        ...
        let mutated =
            make_mutated_metadata_i64_snapshot(&valid_snapshot, "created_at_unix_ms", 99_999);
        let second_envelope =
            harness
                .peer_sealer
                .seal(chunk.as_path(), PayloadKind::Snapshot, mutated);
    ...
    fn metadata_previous_chunk_mutation_on_existing_chunk_rejected() {
        ...
        let valid_snapshot =
            make_valid_loro_snapshot("initial", &peer_hex, "mutate-prev-session", 0);
