...
| `--repo` | Use only repo-scoped skills, commands, and agents via a provider overlay |
...
- **Idle output flush**: the lib `AssistantStream` streaming component records when the block buffer last grew. A dedicated ticker calls `flush_idle(silence_window)` on its own cadence, so a dangling final paragraph from a slow-to-close provider becomes visible within the silence window even if the provider never closes stdout. Cadence and silence window are a single value — **30 s** by default, overridable with `CLAUDINE_IDLE_FLUSH_INTERVAL` (duration string, read once when the ticker spawns; absent, malformed, or non-positive values keep the default). This is a render cadence, not a third timeout rule — it terminates nothing. See [`docs/topics/timeouts.md`](../docs/topics/timeouts.md#idle-flush-cadence).
...
- **`claudine inline-compose [flags] <file-ref> [key=value ...]`** — compose the frontmatter `prompt` property and launch the agent **on the document itself**: it is told the file's native absolute path (plus its `$schema` property table, when declared) and writes the body and any requested frontmatter directly. Its final response is a short summary for the caller and never enters the file. Claudine then reads the document back, restores `prompt`/`hash`/`last_updated` from the pre-run snapshot (warning once per property the agent touched), sets `last_updated` to today's date, stamps a Darkmatter `Simple` `hash:`, and writes once atomically. A run is accepted only when the body changed meaningfully and the document satisfies its `$schema` at completion; a provider failure, interrupt, or refused body atomically restores the captured document.
...
└── commands/
    ...
    │   └── provider_overlay.rs → Provider-overlay materialization (the user's home is never replaced)
    └── init/
