...
fn fake_goose(fixture: &CliProcessFixture, text: &str, code: i32) {
    write_executable(
...
fn run(fixture: &CliProcessFixture, args: &[&str]) -> (String, String) {
    let output = fixture
        .command()
        .args(args)
        .assert()
...
fn standalone_compose_body_sees_an_initialized_empty_accumulator() {
    ...
    fake_goose(&fixture, "provider said hello", 0);
...
fn success_sees_this_runs_output_appended() {
    let fixture = CliProcessFixture::named("composition-outputs");
    fake_goose(&fixture, "the summary line", 0);
    let md = fixture.cwd().join("doc.md");
    fs::write(
        &md,
...
fn finalize_after_success_sees_the_accumulated_entry() {
    let fixture = CliProcessFixture::named("composition-outputs");
    fake_goose(&fixture, "final text", 0);
    let md = fixture.cwd().join("doc.md");
    fs::write(
        &md,
...
fn start_sees_only_prior_entries() {
    let fixture = CliProcessFixture::named("composition-outputs");
    fake_goose(&fixture, "later output", 0);
    let md = fixture.cwd().join("doc.md");
    fs::write(
        &md,
...
fn a_failed_run_commits_no_entry() {
    let fixture = CliProcessFixture::named("composition-outputs");
    fake_goose(&fixture, "partial work", 7);
    let md = fixture.cwd().join("doc.md");
    fs::write(
        &md,
...
fn set_targeting_a_reserved_key_fails_the_run_before_launch() {
    let fixture = CliProcessFixture::named("composition-outputs");
    let witness = fixture.cwd().join("launched.txt");
    write_executable(
        &fixture.bin_dir().join("goose"),
        &format!(
            "#!/bin/sh\nprintf 'launched\\n' >> '{}'\nexit 0\n",
