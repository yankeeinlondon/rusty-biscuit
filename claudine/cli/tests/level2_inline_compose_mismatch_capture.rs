...
//! mismatch diagnostic.
//!
//! Complements `level2_inline_compose_mismatch_pty.rs` (raw PTY, authoritative
//! for the exact emitted SGR + OSC 8 bytes) by driving the real `claudine`
//! binary inside a real terminal emulator (`tmux`) and asserting on the surface
//! the emulator actually re-rendered. This is the "Level 2 real-terminal
//! capture" the second High finding in
...
//!
//! Skip-clean via `TmuxHarness::available()`; `BISCUIT_TEST_LEVEL_REQUIRED=2`
