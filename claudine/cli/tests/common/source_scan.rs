...
//! gates (`test_placement.rs`, `spawn_site_guard.rs`).
//!
//! Both gates answer "does this file *do* X?" by searching source text, and
//! both must not fire on an X that appears in a doc comment or a string
//! literal — the guards' own explanatory prose names the very APIs they
//! forbid. [`sanitize`] blanks comments and literals while keeping every byte
//! offset and newline in place, so a hit in the sanitized buffer indexes
//! straight back into the original source (line numbers, and the literal text a
//! call site passed).
