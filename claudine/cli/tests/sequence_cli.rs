...
#[cfg(unix)]
#[test]
fn sequence_reuses_shell_approvals_across_steps() {
    // Verifies that once a ::shell command is approved (here via the
    // whitelist) in the first step's preflight, subsequent steps don't
    // re-prompt. We use a whitelisted command so the test runs
    // non-interactively without a TTY; the shared approval cache is
    // populated on the first step and hit by step 2.
    let workspace = tempdir().unwrap();
    let path_dir = workspace.path().join("bin");
    fs::create_dir_all(&path_dir).unwrap();
...
"#,
    ...
    // Without approval reuse this would either prompt (no TTY) or fail.
    // A successful exit means preflight accepted the whitelisted command
    // on both steps.
    cargo_bin_cmd!("claudine")
        .env("NO_COLOR", "1")
        .env("HOME", workspace.path())
