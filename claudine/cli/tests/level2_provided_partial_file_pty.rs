//! Level 2 PTY tests for provided-partial `file(match)` resolution.
//!
...
/// Build a `claudine compose --goose <plan> spec=<partial>` command anchored
/// at `workspace_dir` (the launch area the glob walks) with HOME set to the
/// workspace so `prompt_for_missing` reads its default (`true`).
fn compose_command(
...
fn level2_pty_provided_partial_single_match_confirms_and_launches() {
    ...
    let cmd = compose_command(workspace.path(), &bin_dir, &md_file, "everywhere");
...
fn level2_pty_provided_partial_zero_match_preserves_error() {
    ...
    let cmd = compose_command(workspace.path(), &bin_dir, &md_file, "no-such-partial");
    let mut session: OsSession = Session::spawn(cmd).expect("spawn PTY session");
