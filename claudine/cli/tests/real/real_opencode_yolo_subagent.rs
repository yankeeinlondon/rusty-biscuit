...
//!   just test-real real_opencode_yolo_subagent:: --no-capture   # in claudine/
//! ```
//!
//! `just test-real` sets `CLAUDINE_CONTRACT_REAL=1` and runs the `real_`
//! filterset over the package's `real` test binary; the positional filter
//! narrows it to this module.
