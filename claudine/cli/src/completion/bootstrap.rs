...
_claudine() {
    ...
    if (( ${#candidates[@]} > 0 )) && [[ -n "${candidates[1]}" ]]; then
        # Supplement candidates are substring matches, so they may not share
        # any common prefix with what the user typed. Without the guards
        # below, zsh computes an empty unambiguous prefix and silently
...
mod tests {
    ...
    fn zsh_script_suppresses_compadd_prefix_filter() {
        let out = render(Shell::Zsh);
        // The supplement engine returns substring matches (e.g. `plan`
        // matches `prompts/plan.md`). Without `-U`, compadd drops any
        // candidate whose string doesn't start with what the user typed,
        // silently erasing the whole point of substring completion.
        assert!(
            out.contains("compadd -U"),
            "zsh script must pass `-U` to compadd so substring matches survive: {out}"
        );
    }
