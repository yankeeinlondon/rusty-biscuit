...
pub(crate) enum LaunchPlanError {
    ...
    Producer(String),
    /// A typed producer failure with replay-specific context.
    ProducerError {
        context: &'static str,
        source: Box<dyn std::error::Error + Send + Sync>,
    },
    /// A report-returning producer failure, plus its restorable diagnostic
    /// projection when the report contains a registered Claudine error.
    ProducerReport {
        context: &'static str,
        report: color_eyre::eyre::Report,
        diagnostic: Option<Box<RestoredDiagnostic>>,
    },
...
impl std::fmt::Display for LaunchPlanError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::ProducerError { context, source } => write!(f, "{context}: {source}"),
            Self::ProducerReport {
                context, report, ..
            } => write!(f, "{context}: {report}"),
...
impl LaunchPlanError {
    ...
    pub(crate) fn producer_report(
        context: &'static str,
        report: color_eyre::eyre::Report,
    ) -> Self {
        let diagnostic = DiagnosticSnapshot::select(report.as_ref())
            .map(RestoredDiagnostic::new)
            .map(Box::new);
        Self::ProducerReport {
            context,
            report,
...
impl std::error::Error for LaunchPlanError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            Self::ProducerError { source, .. } => Some(source.as_ref()),
            Self::ProducerReport { diagnostic, .. } => diagnostic
                .as_deref()
...
fn replay(
    ...
    if facets.yolo_requested {
        ...
        let outcome = profile
            ...
            .map_err(|e| LaunchPlanError::producer_report("permission-mode replay failed", e))?;
    ...
    .map_err(|e| LaunchPlanError::producer_report("model replay failed", e.into_report()))?;
    ...
    if !provider_moved {
        ...
        let application = profile
            ...
            .map_err(|e| {
                LaunchPlanError::producer_report("system-prompt replay failed", e)
            })?;
...
fn rebuild_mcp(
    ...
    let catalog = McpCatalogStore::load()
        .map_err(|e| LaunchPlanError::producer_error("failed to load MCP catalog", e))?;
    ...
    .map_err(|e| LaunchPlanError::producer_error("MCP session error", e))?;
...
fn fold_opencode_config(
    ...
    // exactly one place. The scratch plan is a carrier for the current value.
    {
        ...
        .map_err(|e| LaunchPlanError::producer_report("permission overlay replay failed", e))?;
    ...
    if let Some(raw) = system_prompt_config {
        let overlay = serde_json::from_str(raw).map_err(|e| {
            LaunchPlanError::producer_error("invalid system-prompt inline config", e)
        })?;
        merge(&mut current, overlay)?;
        touched = true;
    }
