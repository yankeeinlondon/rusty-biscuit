...
///
/// Returns `(exit_code, termination_kind)`. When `child_in_own_pgroup` is
/// `true`, signals are sent to `-child_pid` so the entire process group is
/// terminated; otherwise only the immediate child is targeted (interactive
/// TUI mode where the child shares the parent's process group).
#[cfg(unix)]
pub(crate) fn wait_with_timeout(
    ...
    let deadline = Instant::now()
        ...
        .ok_or_else(|| {
            color_eyre::eyre::eyre!("timeout value {seconds}s overflows the system clock")
        })?;
                        ...
                        match child.try_wait()? {
                            ...
                            None => {
                                if Instant::now() >= kill_deadline {
                                    ...
                                    let reap_deadline =
                                        Instant::now() + POST_SIGKILL_REAP_TIMEOUT;
                                    loop {
                                        match child.try_wait()? {
                                            Some(status) => {
                                                return Ok((
        ...
        match child.try_wait()? {
            ...
            None => {
                if Instant::now() >= deadline {
                    ...
                    child.kill()?;
                    let reap_deadline = Instant::now() + POST_SIGKILL_REAP_TIMEOUT;
                    loop {
                        match child.try_wait()? {
                            Some(status) => {
                                return Ok((
...
mod tests {
    ...
    /// Regression: an absurd `--timeout` value must return an error instead of
    /// panicking on `Instant` overflow.
    #[cfg(unix)]
    #[test]
    fn wait_with_timeout_rejects_absurd_timeout_without_panicking() {
        let mut child = std::process::Command::new("true")
            .spawn()
            .expect("true must be available on PATH");
        let result = wait_with_timeout(&mut child, true, u64::MAX);
        assert!(
            result.is_err(),
            "absurd timeout must error instead of panicking"
        );
        assert!(
    ...
    /// Non-Unix parity: absurd timeout must also error without panicking.
    #[cfg(not(unix))]
    #[test]
    fn wait_with_timeout_rejects_absurd_timeout_without_panicking() {
        ...
        let result = wait_with_timeout(&mut child, false, u64::MAX);
        assert!(
            result.is_err(),
            "absurd timeout must error instead of panicking"
        );
