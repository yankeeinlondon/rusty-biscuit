...
#[derive(Debug)]
struct TestProducerFailure;

impl std::fmt::Display for TestProducerFailure {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str("producer failed")
    }
}

impl std::error::Error for TestProducerFailure {}

#[test]
fn typed_producer_failure_remains_in_the_launch_plan_error_chain() {
    use std::error::Error as _;

    let error = LaunchPlanError::producer_error("replay context", TestProducerFailure);
    assert!(
        error
            .source()
            .is_some_and(|source| source.downcast_ref::<TestProducerFailure>().is_some()),
        "the launch-plan boundary must retain the concrete producer error",
    );
    assert_eq!(error.to_string(), "replay context: producer failed");
}

#[test]
fn report_producer_failure_restores_its_registered_diagnostic() {
    let cause = claudine::composition::CompositionError::FileNotFound("missing.md".into());
    let expected = claudine::diagnostics::DiagnosticSnapshot::from_diagnostic(&cause);
    let error = LaunchPlanError::producer_report("replay context", cause.into());
    let actual = claudine::diagnostics::DiagnosticSnapshot::select(&error)
        .expect("the registered cause remains selectable through launch-plan replay");

    assert_eq!(actual, expected);
    assert!(error.to_string().starts_with("replay context: "));
}
