...
fn malformed_memory_frontmatter_remains_an_error() {
    ...
    assert!(
        error
            .downcast_ref::<darkmatter::markdown::MarkdownError>()
            .is_some()
    );
}
...
fn wrapper_args_from(extra: &[&str]) -> WrapperArgs {
    ...
    Probe::try_parse_from(argv).expect("probe must parse").args
}
...
fn opencode_yolo_overlay_is_visible_in_added_env() {
    ...
    let entries: Vec<&(String, String)> = plan.added.iter().filter(|(k, _)| k == KEY).collect();
    assert_eq!(
        entries.len(),
        1,
        "no duplicate OPENCODE_CONFIG_CONTENT entry"
    );
