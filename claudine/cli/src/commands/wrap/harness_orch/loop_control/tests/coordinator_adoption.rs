...
fn adopt_rejects_a_missing_target_without_activating_it() {
    ...
    assert!(
        error.to_string().contains("target does not exist"),
    ...
    let snapshot = claudine::diagnostics::DiagnosticSnapshot::select(&error)
        .expect("the commit refusal retains its typed diagnostic");
    assert_eq!(snapshot.code, "composition.invalid_file_reference");
    assert_eq!(snapshot.detail["event"], "failure");
    assert_eq!(snapshot.detail["property"], "failure.stack[*].proxy");
    assert_eq!(snapshot.detail["reference"], missing.display().to_string());
    assert_eq!(
...
/// The same for the other variant: the authoring-context `CompositionError`
/// and its typed `HarnessError` cause must both remain reachable, or a missing
/// proxy target loses either its property/reference detail or its resolver
/// provenance at the render boundary.
#[test]
fn an_unresolvable_target_exposes_its_typed_cause_to_the_error_chain() {
    ...
    let source = std::error::Error::source(&error)
        .expect("a resolution failure must expose its typed composition error");
    assert!(
        source
            .downcast_ref::<claudine::composition::CompositionError>()
            .is_some_and(|composition| matches!(
        ...
        "the first source must retain the authored property/reference context"
    );
    assert!(
        source
            .source()
            .and_then(|cause| cause.downcast_ref::<claudine::harness::HarnessError>())
            .is_some(),
        "the next source must retain the resolver's typed failure"
    );
}
...
fn a_refused_handoff_projects_typed_err_facets_per_variant() {
    ...
    assert_eq!(
        resolution_info.kind, "CompositionError",
        "a resolution failure must reach the stack as its authoring-context error, \
         not as a generic action failure"
    );
    assert_eq!(
        resolution_info.snapshot.as_ref().map(|snapshot| snapshot.code.as_str()),
        Some("composition.invalid_file_reference"),
        "the resolution refusal must retain its file-reference identity"
    );
    ...
    assert_ne!(
        resolution_info.snapshot.as_ref().map(|snapshot| snapshot.code.as_str()),
        rejection_info.snapshot.as_ref().map(|snapshot| snapshot.code.as_str()),
        "the two composition-level refusals must stay distinguishable by diagnostic \
         code even though they share one concrete error family"
    );
