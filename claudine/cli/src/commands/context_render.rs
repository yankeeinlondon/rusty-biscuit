...
/// preserving all three columns at every width.
///
/// The shared `property_width`/`type_width` are pinned as minimum widths so
/// sections align — but only when the capped render width can afford them.
/// `with_min_width` is a hard floor: at widths too narrow to hold the pinned
/// columns it would make the table unrenderable (emitting the planner's
/// width-error string inline). Per the spec's narrow-terminal clause, when the
/// pinned layout does not fit we drop the pins and let the columns wrap instead
/// of dropping a column or overflowing. `Property`, `Type`, and the final
...
pub fn render_context_section(
    ...
    let build = |pin: bool| {
        let mut property = TableColumn::new("Property");
        let mut ty = TableColumn::new("Type").with_alignment(Alignment::Center);
        if pin {
        ...
        let mut table = Table::new().with_columns(vec![
            property,
            ty,
            TableColumn::new(final_header.to_string()),
        ]);
    ...
    // The render path caps the terminal to 140ch and the table reserves its 1ch
    // margins; `plan_widths` measures against that post-margin budget.
    let budget = term.width().min(MAX_REPORT_WIDTH).saturating_sub(2);

    let pinned = build(true);
    if pinned.plan_widths(budget).is_ok() {
        render_table_within_contract(&pinned, term)
    } else {
