...
fn every_test_source_is_compiled_by_a_declared_target() {
    ...
    // Non-vacuity: the walk read the real suite (both roots, their eleven
    // former targets, and this gate).
    assert!(files.len() >= 14, "scanned only {} test source file(s): {:?}", files.len(), files.keys());
    for expected in [
        "tests/l1/main.rs",
        "tests/l1/test_layout.rs",
        "tests/l1/drift.rs",
        "tests/level2/main.rs",
        "tests/level2/level2_report_terminal.rs",
    ] {
