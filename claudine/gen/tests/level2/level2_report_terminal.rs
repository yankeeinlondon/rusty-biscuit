...
//! the pane with `--exact level2_report_terminal::level2_report_probe
//! --nocapture`, which puts the report on a real TTY without a shipped binary
//! and without fabricating a synthetic package area to drive `claudine-gen`
//! against.
//!
...
fn capture_probe(mode: &str, cols: u32, rows: u32) -> CapturedFrame {
    ...
    let cmd = format!("'{}' --exact level2_report_terminal::level2_report_probe --nocapture", exe.display());
