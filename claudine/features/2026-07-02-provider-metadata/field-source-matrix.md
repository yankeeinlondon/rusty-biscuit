...
| `stream_protocol` | `Option<&'static str>` (`stream-json`/`jsonl`/`ndjson`/`wire-json-rpc`/None) | String (nullable) *(vocab gap: Optional)* | `optional_string_literal`; enum candidate | → `non-interactive-sessions` (`io_contract.framing` / `data_format`), but the vocabularies differ (`stream-json` vs `ndjson` — claude's constant says `stream-json` while its NIS `data_format` says `ndjson`); needs a vocabulary ruling before graduation. |
...
| `session_locations` | `&'static [PathTemplate]` | RecordArray | `PathTemplateList` (named) | → `agent-logging` `surfaces` — but the role mapping is ambiguous (claude's value is a *directory*, codex's mixes an app log and shell snapshots; no single `role` filter reproduces them). Needs a semantic ruling on what this field means before re-pointing (Open question 5). |
...
| `acp` | `AcpSupport` (server_mode/client_supported/events_via_acp) | Record *(vocab gap)* | `AcpRecord` (named) | → `acp` topic (`support`, `capabilities[]`, `reverse_requests[]` carried today) but the vocabularies are disjoint: constant `server_mode: available_via_wire_proxy` (kimi) has no sidecar counterpart (`native/adapter/partial/none`). Needs vocabulary reconciliation (the summary's `native/adapter/unassessed` model) before graduation. |
