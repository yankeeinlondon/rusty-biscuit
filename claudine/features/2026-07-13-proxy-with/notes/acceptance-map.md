...
1-5, and again after review-5 findings 1-6.** **Status: 30 of 30 criteria mapped
to passing tests.** The four criteria Phase 13 recorded as blocked on the R6
launch rebuild (AC 7, AC 9/10, AC 15) are now unblocked, and the
previously-`#[ignore]`d reproductions are re-enabled and pass. See
[Previously-blocked criteria](#previously-blocked-criteria-now-resolved).

**review-5 reconciliation (findings 1-6).** The launch rebuild now happens on the
**surfaced-coordinator** path, not only through `target_launch.rs`: a proxied
target (compose/inline-compose/sequence plus the initialize/terminal routes)
surfaces its handoff up to the command-owned coordinator, which re-prepares the
target as a fresh document through
`compose/prep.rs::prepare_and_run_active_document`. That rebuild recomputes — from
the target's own frontmatter, under explicit-CLI precedence — the stored
`ComposeContext`, provider and model selection, document-loop ownership,
`initialize` (behind the narrow safety gate) plus the stabilized reread, schema,
child CWD, and system-prompt delivery. The in-harness
`harness_orch/loop_control/target_launch.rs::rebuild_target_launch` remains a
fallback overlay and is still `AGENT`/`MODEL`/`YOLO`-only; the facets it does not
rebuild — profile/binary *sub-selection*, argv *entrypoint*, and MCP *runtime
injection* — stay borrowed from the originally-invoked document and diverge only
when a proxy changes the provider itself (the common explicit-CLI case pins it).
The resume session-compatibility key (`harness_orch/session_key.rs`) is computed
from the final typed launch bundle (`AttemptLaunch`) via `biscuit_hash::xx_hash`;
its facet *projection* is covered at L1 and the compatible/dropped-flag happy path
at L2, but the facet-*mutating* refusal (`CompositionError::LifecycleResumeIncompatible`)
is **latent** — no compat-key facet mutates per-resume on today's tree, so the
refusal branch has no reachable trigger yet. Findings 2-6 also added Level 2
coverage for looping terminal/sequence targets, the AC 29
missing-target/cycle/hop-limit routing, inline-closure rewriting only the final
target, approved==executed shell bytes, overlay survival across
retry/resume/loop, child CWD, CLI system-prompt survival, and target-authored
provider selection.
...
| 6 | `compose`/`inline-compose`/sequence-step remain command state | ✅ | `loop_control::tests::coordinator_adoption::inline_closure_ownership_follows_the_adopted_target`; L2 `wrap_compose_preflight::compose_dry_run_does_not_traverse_a_proxy_handoff`; L2 `level2_lifecycle_proxy_inside_sequence_step_is_contained` (a proxy inside a sequence step runs its target once per step and neither advances nor restarts the step); L2 `level2_lifecycle_inline_compose_proxy_closure_rewrites_only_final_target` (only the final target is rewritten inline) |
...
| 10 | **Provider/model/MCP/argv/env/CWD/system prompt recalculated per target** | ⚠️ | Rebuilt & tested: `level2_lifecycle_equivalence_target_pinned_model_matches_direct_run` (model→env); `level2_lifecycle_equivalence_target_authored_provider_matches_direct_run` (provider); `level2_lifecycle_equivalence_child_cwd_matches_direct_run` (child CWD); `level2_lifecycle_equivalence_cli_system_prompt_survives_the_proxy` (system-prompt delivery); `level2_lifecycle_sequence_step_proxy_rebuilds_target_launch_bundle` (full bundle in a step); `level2_lifecycle_equivalence_cross_repo_file_resolution_matches_direct_run` (workspace/CWD anchor); `level2_lifecycle_equivalence_stdout_stderr_routing_matches_direct_run` (output routing). **Still borrowed** on the in-harness fallback (`rebuild_target_launch`, `AGENT`/`MODEL`/`YOLO`-only): profile/binary *sub-selection*, argv *entrypoint*, and MCP *runtime injection* — each diverges only under a provider *switch*, untested because the equivalence rows pin `--goose` on both arms |
...
| 15 | **Resume retains only a session whose compatibility key matches; names facets** | ⚠️ | Projection (L1): `harness_orch::session_key::tests` — per-facet projection tests (`the_model_facet_reads_the_effective_child_env`, `changing_the_working_directory_projects_the_cwd_facet`, `toggling_yolo_projects_the_permission_facet`, `toggling_interactivity_projects_the_interactivity_facet`, `toggling_structured_output_projects_the_structured_facet`, `swapping_the_provider_changes_provider_binary_and_resume_protocol`, `a_changed_system_prompt_flag_projects_the_system_prompt_facet`, `a_changed_mcp_env_projects_the_mcp_facet`, …). Retain half: `coordinator::tests::resume_replaces_the_attempt_slice_but_retains_the_live_session`; happy path (L2): `level2_lifecycle_resume_with_dropped_launch_flag_stays_compatible` (a dropped-flag refresh stays compatible and reuses the session). **Latent:** the facet-*mutating* refusal (`LifecycleResumeIncompatible`) has no reachable trigger on today's tree — every compat-key facet is fixed by the invocation or rebuilt once at proxy adoption, never per-resume — so no L2 asserts the refusal itself |
...
review-5 findings 1-6 completed it on the **surfaced-coordinator** path; the four
are now mapped to passing tests (verified 2026-07-17, parallel self-spawn
`just test-l2`):
...
(with `harness_orch/loop_control/target_launch.rs` supplying the in-harness env
overlay), so an adopted target receives the same document loop as direct
invocation. The two reproductions are re-enabled and pass:
...
When a handoff surfaces to the coordinator, the target is re-prepared as a fresh
document and its provider/model/CWD/system-prompt are recomputed from the
target's own frontmatter (`compose/prep.rs::prepare_and_run_active_document`;
`harness_orch/loop_control/target_launch.rs::rebuild_target_launch` overlays the
`AGENT`/`MODEL`/`YOLO` env). So `level2_lifecycle_equivalence_target_pinned_model_matches_direct_run`
(re-enabled) resolves the same `env.MODEL` on both arms, and findings 2-6 add
launch-facet rows that move on the same rebuild: `..target_authored_provider..`
(provider), `..child_cwd..` (child CWD), `..cli_system_prompt_survives..`
(system-prompt delivery), and `..sequence_step_proxy_rebuilds_target_launch_bundle`
(full bundle inside a step); review-3 finding 5's
`..cross_repo_file_resolution..` (workspace/CWD anchor) and
`..stdout_stderr_routing..` (output routing) still hold. **Not rebuilt:**
profile/binary sub-selection, argv entrypoint, and MCP runtime injection stay
borrowed on the in-harness fallback and diverge only under a provider *switch*
(the equivalence rows pin `--goose` on both arms, so this is untested by
construction, not regressed).

### AC 15 — resume compatibility key (was Phase 11)

The `SessionCompatibilityKey` now exists (`harness_orch/session_key.rs`), is
computed from the final typed launch bundle (`AttemptLaunch`) via
`biscuit_hash::xx_hash`, and is compared after the resume refresh.
`harness_orch::session_key::tests` proves each launch facet projects into the key
— model, CWD, permission (`yolo`), interactivity, structured output, provider
binary + resume protocol, system prompt, and MCP — and
`level2_lifecycle_resume_with_dropped_launch_flag_stays_compatible` covers the
compatible happy path end-to-end. **Latent:** the mutating refusal
(`CompositionError::LifecycleResumeIncompatible`) has no reachable trigger on
today's tree — every compat-key facet is fixed by the invocation or rebuilt once
at proxy adoption, never per-resume — so the "changed facet detected and named"
branch is proven only at the projection layer (L1), not exercised as a live
refusal.
