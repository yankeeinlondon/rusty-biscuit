...
- [x] **[Parallelizable: Darkmatter track]** Extend side-effect errors/tests for invalid/non-top-level keys while leaving Claudine-specific reserved-key policy to Claudine; expose the new signature through the side-effect catalog. *(`EffectError::InvalidKey` rejects empty/dotted keys; 4 new `set` tests; Claudine's `side_effect_signature` auto-derives the capability form `set(key, value)` from `EFFECT_DESCRIPTORS`. The later mapping-only lifecycle change deliberately keeps that lower-level API while narrowing lifecycle YAML to `set: {property: value}`.)*
...
Fresh macOS L1, sequence seam/source guards, and package-area lint gates passed.
The macOS L2 run passed 227 of 228 tests; one unrelated-to-sequence lifecycle
retry row failed deterministically, while one sequence launch-bundle row passed
on retry. Required Linux, native Windows, and attended L3 evidence is not
attached. This feature remains active pending mega-merge closeout.
