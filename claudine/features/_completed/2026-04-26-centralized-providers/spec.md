...
Seven decisions made during clarification review (resolving Open Questions 1–6):
...
4. **Multiple focused trait objects for dynamic operations.** `ProviderInfo` carries multiple focused trait objects (`McpBehavior`, `AdapterBehavior`, `ConfiguratorBehavior`) for domain-specific dynamic operations, rather than a single monolithic `ProviderBehavior` trait or pure data structs. This keeps each behavior trait small and focused while preserving the structural serde round-trip on the data struct.
5. **`CaptureMethod` merged into `EventSupportLevel`.** `EventSupportLevel` absorbs `CaptureMethod` into a single enum that captures both status and mechanism. This eliminates the parallel structure between `EventSupportLevel` and `CaptureMethod` and makes event mapping tables the single source of truth.
6. **`Provider` enum moves to `claudine::provider` module.** The canonical home for the `Provider` enum becomes `provider/mod.rs`; `events/provider.rs` retains a `#[deprecated]` re-export for one release cycle. This establishes the new module as the authoritative home during Phase 1 rather than leaving ambiguity about which module owns the type.
7. **Required behavior traits with default no-op implementations.** Every provider implements all three behavior traits (`McpBehavior`, `AdapterBehavior`, `ConfiguratorBehavior`). Unsupported operations have default implementations that return typed "not supported" errors. This keeps call sites simple (no `Option` unwrapping) while minimizing per-provider boilerplate.

> **Note on `LazyLock`:** `LazyLock` was considered for static catalog initialization and rejected because it breaks `#[derive(Serialize)]` and introduces unnecessary atomic overhead for declarative config data that is fully known at compile time. All static catalog data uses `&'static [T]` and `&'static str` instead of `Vec<T>`, `PathBuf`, or `LazyLock`.
...
Introduce a new top-level `claudine::provider` module containing a partitioned design: a public **data struct** carrying every static fact about a provider, and a small **behavior trait** for the genuinely dynamic operations. The two are linked by a `&'static dyn ProviderBehavior` field on the struct, so a single registry lookup returns both halves. The `Provider` enum itself moves to `provider/mod.rs` as the canonical home; `events/provider.rs` becomes a thin deprecated re-export.
...
pub struct ProviderInfo {
    ...
    // ----- MCP support (all providers implement; unsupported ops have defaults) ----
    #[serde(skip)]
...
### Per-domain behavior traits

In addition to the cross-cutting `ProviderBehavior` trait, three domain-specific behavior traits live on `ProviderInfo`. Each trait defines a complete contract with default no-op implementations for unsupported operations, so every provider implements all three traits but only overrides the methods it actually supports.

```rust
/// MCP import, state, injection, and export operations.
pub trait McpBehavior: Send + Sync + std::fmt::Debug + 'static {
    /// Import MCP configuration from provider-specific JSON/TOML.
    fn import(&self, raw: &serde_json::Value) -> Result<McpConfig, McpError> {
        Err(McpError::NotSupported)
    }

    /// Track MCP state across hook invocations.
    fn state(&self) -> Option<&'static dyn McpState> { None }

    /// Inject MCP runtime configuration (shadow HOME, env vars, etc.).
    fn inject(&self, ctx: &mut InjectionContext) -> Result<(), McpError> {
        Err(McpError::NotSupported)
    }

    /// Export current MCP configuration to provider-native format.
    fn export(&self, config: &McpConfig) -> Result<String, McpError> {
        Err(McpError::NotSupported)
    }
}

/// Inbound payload → AgenticEvent parsing.
pub trait AdapterBehavior: Send + Sync + std::fmt::Debug + 'static {
    /// Attempt to parse a raw JSON payload into an AgenticEvent.
    /// Returns None if the payload is not recognized by this provider.
    fn parse_payload(&self, raw: &serde_json::Value) -> Option<AgenticEvent> { None }

    /// Detect whether this payload originated from this provider.
    /// Default delegates to ProviderBehavior::detect_from_payload.
    fn detect(&self, raw: &serde_json::Value) -> bool { false }
}

/// Hook registration and config-file mutation.
pub trait ConfiguratorBehavior: Send + Sync + std::fmt::Debug + 'static {
    /// Register hooks for this provider in the given configuration file.
    fn register_hooks(
        &self,
    ...
    /// Deregister hooks for this provider.
    fn deregister_hooks(&self, config: &mut ConfigFile) -> Result<(), ConfigError> {
        Err(ConfigError::NotSupported)
    }

    /// Supported configuration file formats (JSON, JSONC, TOML, YAML).
    fn supported_formats(&self) -> &'static [ConfigFormat] { &[] }
}
```

Each trait's default implementations return typed errors (`McpError::NotSupported`, `ConfigError::NotSupported`) or `None`, so callers always invoke the method directly without `Option` checking. Providers override only the methods they support (e.g., only Codex/Gemini/OpenCode override `McpBehavior::inject`; only providers with hooks override `ConfiguratorBehavior::register_hooks`).
...
The same type backs `agents::*::session_locations`, `agents::*::log_locations`, `mcp::*::config_paths`, and the new `ProviderInfo::dashboards` field. `ConfigCapabilities::user_files`, `project_files`, and `local_files` migrate from `Vec<PathBuf>` (already typed) to `&'static [PathTemplate]` so home expansion is consistent.
...
pub struct NonInteractiveCapabilities {
    ...
    pub stdin_supported: bool,
    pub output_formats: &'static [OutputFormatSupport],  // typed
    pub structured_output_supported: bool,
    pub resume_supported: bool,
    pub limitations: &'static [KnownLimitation],
}
...
pub enum SystemPromptDelivery {
    ...
    ShadowHomeFile { relative_path: &'static str },
...
`event_support_level` and `native_event_name` become trivial lookups through the unified `EventSupportLevel` enum, which is now the single source of truth for both support status and capture mechanism.
...
Every provider implements all three behavior traits with default no-op implementations for unsupported operations. Some logic is genuinely irreducible and stays in per-provider files:

- Stream protocol modules (`stream/protocol/<provider>.rs`) — typed event enums per stream format.
- Stream semantic parsers (`stream/<provider>_semantic.rs`) — `serde_json::Value` → `SemanticEvent` projection; reached via `ProviderBehavior::create_semantic_parser`.
- Event adapters (`adapters/<provider>.rs`) — inbound-payload parser; implements `AdapterBehavior`.
- Hook configurators (`config/<provider>.rs`) — config-file write logic with format-specific (JSON/JSONC/TOML/YAML) mutation; implements `ConfiguratorBehavior`.
- MCP import/export logic (`mcp/import.rs`, `mcp/export.rs`) — provider-specific JSON/TOML round trips; implements `McpBehavior`.
- MCP runtime injection (`mcp/inject.rs`) — shadow `HOME` synthesis (Codex, Gemini) vs env-var content (OpenCode); implements `McpBehavior`.
- Wrapper overrides for genuine quirks (small subset of `WrapperProfile`).

Each of these is reachable through `provider_info(Provider).<field>` (or `provider_info(Provider).<behavior_trait>.<method>(...)` for dynamic operations) so callers never `match` over `Provider`; only the central registry does. The CLI-side `wrapper_for(Provider)` registry is the one permitted parallel match.
...
2. **Cross-module exhaustiveness** — `for p in PROVIDERS_DISPLAY_ORDER { let info = provider_info(p); let _ = info.event_mapping; let _ = info.capabilities; let _ = info.resource_support; … }`. Touches every field (and exercises `info.behavior` via `detect_from_payload`/`create_semantic_parser` smoke calls, plus exercises `info.mcp`, `info.adapter`, `info.configurator` via smoke calls) to force a panic-or-stub if any provider hasn't been wired through a facet.
...
├── event_mapping.rs    # EventMappingTable, EventMapping, EventSupportLevel
...
1. Move the `Provider` enum definition to `provider/mod.rs` and re-export it from `events/provider.rs` with `#[deprecated]`.
2. Create `claudine/lib/src/provider/` module with `ProviderInfo`, `ProviderBehavior`, identity types, and one stub `ProviderInfo` constant per provider whose data fields delegate to (or duplicate) values from the existing modules (`agent_for`, `capabilities_for`, etc.). The lib stub does **not** delegate to CLI types — there is no `wrapper_profile` field.
3. Add `provider_info(Provider) -> &'static ProviderInfo`.
4. Wire the four exhaustiveness tests above (the CLI-side wrapper exhaustiveness lives in the CLI crate; the other three live in lib).
5. Add `--describe` JSON output backed by `serde_json::to_string(&provider_info(p))` so a snapshot test can pin behavior across migrations.
...
4. Unify `EventSupportLevel` variants to include capture mechanism and wire `EventSupportLevel::Acp` for Goose `request_permission` and Kimi `ApprovalRequest`.
...
3. Same for `mcp::import::*`, `mcp::state::*`, `mcp::inject::*` — each `match` becomes a call into `provider_info(p).mcp.<method>(…)` on the `McpBehavior` trait object.
4. `adapters/<provider>.rs` parsers become `AdapterBehavior::parse_payload` implementations.
5. `config/<provider>.rs` hook configurators become `ConfiguratorBehavior::register_hooks` implementations.
...
1. Replace `Vec<&'static str>` log-paths with `&'static [PathTemplate]`. Add resolver. Update `claudine logs` and any reporting that previously didn't consume them.
2. Replace `output_formats: Vec<&'static str>` with `&'static [OutputFormatSupport]`. Update wrappers to derive `apply_output_format`.
3. Replace `entrypoints: Vec<&'static str>` with `&'static [EntrypointSpec]`. Update `apply_entrypoint`.
...
6. Replace `gaps: Vec<&'static str>` with `&'static [KnownGap]`.
...
- If `support` is `Hook { .. }`, the configurator is registered.
- If `support` is `StreamParse { .. }`, the stream protocol is `Some(_)`.
...
4. **External `sniff` crate dependency.** **Resolved:** the sniff binding becomes a typed `Result<AiCli>` so missing variants surface as compile-time/test-time errors rather than runtime panics. The round-trip exhaustiveness test in Phase 1 catches drift immediately.

5. **Backwards compatibility for external consumers.** **Resolved:** legacy re-exports carry `#[deprecated]` markers from phase 5 onwards and are removed in a release after phase 8. This gives external consumers two full cycles to migrate before the re-exports are deleted.
