...
- **S1 — Register history growth / compaction (spike, shared with the data-model doc).** Measure a Loro map document under realistic write rates (especially the D1 `repos` register on a commit-heavy day): document size over time, and whether shallow-snapshot re-basing (Loro's mechanism for dropping old history) interacts safely with peers holding older sync state. This spike gates any register with more than ~1 write/minute.
- **S2 — Single-writer enforcement on import.** Today "only the owner writes" is convention. A buggy or misbehaving peer could write to another host's document and the merge would happily accept it. The import path should reject deltas whose operations originate from a peer other than the document's `owner_node_id`. Decide where the check lives (sync engine vs storage layer) — flagged as an open question in the data-model doc.
...
- **S4 — Canonical repo identity.** "Same repo" must mean the same thing on every host despite different remote URL forms (SSH vs HTTPS, trailing `.git`, forks). Small but load-bearing: the scheduler's has-the-repo test and the logging pipeline's repo attribution (logging-refactor) must share one canonicalization — define it once, in one crate, used by both.
