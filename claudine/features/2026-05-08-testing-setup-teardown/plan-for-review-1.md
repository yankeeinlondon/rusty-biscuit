...
- [x] Evaluate adding a safe convenience wrapper around `EnvGuard`
  ...
  - **Decision:** Implemented Option B. Added `ENV_GUARD_LOCK` (std::sync::Mutex) that safe constructors (`set_safe`, `remove_safe`) acquire during creation and drop. Existing unsafe constructors (`set`, `remove`) remain for callers who already use `#[serial_test::serial]` and want to avoid lock overhead.
- [x] Update `trace_phase!` rustdoc to warn about `Level::INFO` default and recommend `init_test_tracing()` or `RUST_LOG=test_toolkit=trace`
- [x] Update `.claude/skills/rust-testing/SKILL.md` to document:
  - `rstest` fixture pattern with `Drop`-based teardown
  - `trace_phase!` usage with `init_test_tracing()`
  - `EnvGuard` with `#[serial_test::serial]` (and the new safe constructors)
  - Nextest configuration and how to verify it
- [x] Run the full `claudine` package-area test suite (`just test` or `cargo nextest run -p claudine -p claudine-cli -p test-toolkit`)
- [x] Run `cargo clippy -p test-toolkit -p claudine-cli` to ensure no new warnings
