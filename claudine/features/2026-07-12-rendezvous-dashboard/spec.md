...
- **D5 — Intervention definition (see above).** Which triggers ship in v1, and is the signal binary or tiered (needs-input vs possibly-stuck)? *Recommendation:* v1 ships trigger 1 (exact) and trigger 2 (good), tiered; trigger 3 arrives with the process monitor's idle spike. **Impl (2026-07-13):** the dashboard renders the signal from each session entry's `status` field (carrying its basis). Because the register only carries what a producer writes, v1 also wires **trigger 1 producer-side**: the Claudine wrapper reports `ReportSessionEvent(UPDATED, status: "waiting_on_user")` when a `PermissionRequest` surfaces in the stream and clears it back to `active` on the next progress event — mirroring the existing `STARTED`/`ENDED` bracket in `session_report.rs`. Trigger 2 (interactive-idle) and trigger 3 (unwrapped) remain follow-ons.
