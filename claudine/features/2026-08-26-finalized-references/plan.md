...
- [ ] Record the baseline commit (`git rev-parse HEAD`) in this feature directory and verify green: `just test` and `just lint` in `biscuit-file/`, `darkmatter/`, and `claudine/`; the Claudine area recipe already includes its library, CLI, contract, catalog-types, and generator L1 suites.
