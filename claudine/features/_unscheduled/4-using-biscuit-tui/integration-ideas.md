...
`biscuit-tui` exposes a reusable widget set whose relevant model is:
...
- Keep domain-specific conversion in Claudine. `biscuit-tui` should provide selection mechanics; Claudine should still decide how selected provider names, events, sounds, and config route names mutate `App`.
...
- Introduce `ConfigTextInputState` that wraps `biscuit_tui::TextInputState` and carries Claudine metadata: label, field name, secret/display mode, validation function, and submit target.
...
- If the secret behavior is generally useful, add it to `biscuit-tui` as `TextInputState::with_mask(Some(char))` or a sibling `SecretTextInput`. The masking must only affect rendering; `value()` must return the real buffer.
- Preserve messenger webhook validation in Claudine. `biscuit-tui` can display validation errors, but Claudine should own URL/env-only rules and `redact_webhook_urls`.
...
- `biscuit-tui`: `ratatui 0.29`, `crossterm 0.28`

Recommended fix: upgrade `biscuit-tui` to `ratatui 0.30` and `crossterm 0.29`, then run `just test` in `biscuit-tui` and focused `claudine-cli` tests. `queue/cli` already uses the same newer versions as `claudine-cli`, so the workspace has precedent.

### Modal State Ownership

`biscuit-tui` states are long-lived mutable widget states. Claudine's current `ModalState` variants store domain primitives like `highlighted`, `buffer`, `fields`, and `error`. Integration should not push `biscuit-tui` state into global app fields ad hoc. Keep it inside modal variants or a small modal component enum.
...
This likely justifies adding first-class masked input support to `biscuit-tui`.

### Key Binding Compatibility

`biscuit-tui` defaults include vim navigation and component-local submit/cancel behavior. Claudine currently uses tab-specific hotkeys heavily. Adoption should preserve the visible Claudine hotkey contract:

- global overview/detail behavior stays in `App`,
- tab hotkeys remain in tab handlers,
- modal-local arrows/Enter/Esc can move to `biscuit-tui`,
...
Upgrade `biscuit-tui` to the same `ratatui` and `crossterm` versions used by `claudine-cli`. Run the `biscuit-tui` test suite and fix any API drift. Do this before touching Claudine integration so failures are localized.
...
- `component_modal.rs` for rendering `biscuit-tui` widgets inside existing `render_modal`,
...
Replace `TextInput` and `ActionFieldInput` modal handling with `biscuit-tui::TextInput`. This improves editing behavior without touching messenger secrets yet.
...
Add masked input support to `biscuit-tui`, then migrate `MessengerInput`. Preserve Claudine's provider-field sequencing and webhook test behavior in the wrapper.
