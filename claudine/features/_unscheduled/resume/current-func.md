...
- **`redirect { resume: true }` now fails explicitly** with `ResumeUnsupported` when the provider doesn't support resume (this was fixed -- it no longer silently downgrades)
- **Shell approval context** isn't source-aware during resume/retry loops -- `policy_root` is derived from the current `source_path` only, so during a redirect the policy root shifts to the target file rather than preserving the original source context
- **Resume metadata capture** is incomplete -- the compose-refactor spec called for preserving enough session context (provider, session ID, timestamp, file ref) to later support a standalone `claudine resume` command, but `StreamExecutionSummary` lacks a source file path reference and explicit timestamp
