...
- [ ] Rewrite the **Action Forms** section of `claudine/docs/topics/lifecycle.md`
...
- [ ] Update the **Flow Control Actions** table and surrounding prose in
      `lifecycle.md` so the verb signatures read as positional examples
      (`error: "reason"`, `proxy: "@other.md"`, `retry: 3`) rather than
      `verb(args)`.
- [ ] Add a lifecycle topic subsection explaining object-valued side-effect
...
- [ ] Add a migration callout in `lifecycle.md` covering both breaking-change
...
- [ ] Port any in-tree prompt file that uses short form to positional form.
...
- [ ] Validation checkpoint: `rg` over docs/skills/prompts returns no
...
- [ ] Run `just test` in the `claudine` package area and address failures
      without broad formatting churn (no `cargo fmt` write-mode — match
      surrounding style by hand per repo convention).
- [ ] Run `just test-l2` in the `claudine` package area.
- [ ] Run `just lint` in the `claudine` package area and address warnings
      relevant to the changed code.
- [ ] Run `cargo fmt --check` (read-only) for diagnosis only; do NOT
      reformulate if it complains about pre-existing style.
- [ ] Manually invoke `claudine compose --dry-run` against (a) the spec's
      ...
      key/value actions, and the second is unchanged.
- [ ] Confirm short-form rejection renders correctly in both TTY (excerpt
      ...
      color.
- [ ] Confirm lifecycle chatter stays on stderr (no lifecycle side effect
      writes to stdout) by piping the L2 fixture's compose output.
- [ ] Review `git diff` for accidental unrelated refactors, formatting-only
      ...
      `//` comments; fix or delete drifted ones in the same change).
- [ ] Validation checkpoint: every acceptance criterion in the spec is mapped
