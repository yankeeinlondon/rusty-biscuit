...
5. Limit the first implementation to OpenCode and to `ERROR`-level logs.
...
| `Unclassified` | `SemanticEvent::Info` |
...
3. `rate_limit` is filled from stderr only when stdout did not already provide stronger rate-limit data.
...
4. if a bridge exists, let it ingest the line
5. if stderr is not suppressed on success and the bridge does not consume the line, keep the current text passthrough behavior
...
1. Enable `--print-logs --log-level ERROR` by default for OpenCode structured non-interactive runs.
