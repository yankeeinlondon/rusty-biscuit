...
That contract holds today for everything preparation owns **and** for the launch inputs the coordinator rebuilds per target: provider, model, document-loop ownership, child CWD, and system-prompt delivery all come from the target's own frontmatter (see [Target-specific behavior](#target-specific-behavior) below). The narrow inputs still borrowed from the originally-invoked document — profile/binary *sub-selection*, the argv *entrypoint*, and MCP *runtime injection* — are the remaining ledger; each diverges only when a proxy changes the provider itself, which the common explicit-CLI case pins.
...
- **Launch identity** — when the handoff surfaces to the command-owned coordinator, `compose/prep.rs::prepare_and_run_active_document` re-prepares the target as a fresh document and rebuilds, from the target's own frontmatter under explicit-CLI precedence: provider and model selection, document-loop ownership/recognition, child CWD, and system-prompt delivery. A proxied target therefore selects its authored `model:` and acquires its own `loop:`, matching a direct invocation. Verified by L2 equivalence rows for provider, model, loop-iteration count, child CWD, and CLI system-prompt survival.

> **Remaining launch-rebuild gaps.** Three launch facets are still borrowed from the originally-invoked document on the in-harness fallback path (`harness_orch/loop_control/target_launch.rs::rebuild_target_launch`, which rebuilds the `AGENT`/`MODEL`/`YOLO` env only): profile/binary *sub-selection*, the argv *entrypoint*, and MCP *runtime injection*. Each diverges only when a proxy changes the provider itself, so the common explicit-CLI case — which pins the provider — is unaffected; a target-frontmatter-driven provider *switch* is the remaining structural work. Interactivity and structured-mode are command-level and non-differentiating. Tracked in [`notes/acceptance-map.md`](../../features/2026-07-13-proxy-with/notes/acceptance-map.md) → AC 10.
...
- **`--dry-run`** never traverses a dynamic proxy route. Dry-run *does* fire lifecycle events — `initialize` → `blocked` → `finalize` is contract — so an `initialize` can request a handoff on a dry run; the dry-run seam consumes that transition explicitly and renders the document it was given rather than following it.
