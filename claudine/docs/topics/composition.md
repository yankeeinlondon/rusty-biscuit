---
hash: ef46db3751d8e999-473b8f3d56c5f461
...
A narrower env-only rebuild applies on the **in-harness fallback** path — the direct provider wrappers, which have no run ledger to surface a handoff to. See [The in-harness fallback path](#the-in-harness-fallback-path) below.
...
**Launch identity is rebuilt at the fresh-read boundary, not snapshotted at adoption.** Every retry/resume re-materializes the active document from disk and recomputes the `AGENT`/`MODEL`/`YOLO` launch env against *that* read (`harness_orch/loop_control/target_launch.rs::rebuild_launch_env`, called from `materialize_attempt_prompt_phase`). An attempt therefore launches with the identity the document it is about to run resolves to now — not with an adoption-time snapshot. This is what lets a refreshed `model:` actually reach the child environment, and what makes the resume compatibility check below able to *see* a changed model.

> **The resume session-compatibility key.** A `resume` retains the live provider session only when a compatibility key of the target's launch properties still matches across the canonical refresh; when a facet changed, the resume refuses with `CompositionError::LifecycleResumeIncompatible { facets }`, names the incompatible facets, and recommends `retry` to start a fresh session. The key is computed from the *final* typed launch bundle (`AttemptLaunch` — the resolved argv plus the effective child environment `build_harness_launch` produces) and digested with `biscuit_hash::xx_hash`; it distinguishes inline (`--append-system-prompt`) from file-backed (`--append-system-prompt-file`) system-prompt delivery, hashing the file's *content* rather than its unstable temp path. Its facets — provider, model, binary, resume protocol, workspace CWD, permission mode, interactivity, structured output, system prompt, and the MCP signal set — are each projected and pinned at L1 (`harness_orch::session_key::tests`).
>
> **Reachability, honestly.** Both directions of the comparison are proven end-to-end for **`model`** only: `level2_lifecycle_resume_refuses_when_refresh_changes_model` drives the live refusal, and `level2_lifecycle_resume_with_dropped_launch_flag_stays_compatible` proves an intentionally dropped resume-only flag is *not* a false refusal. `model` is reachable because it is rebuilt at every fresh-read boundary (above). The other eight facets are **projection-only (L1)**: they are argv-derived or CLI-resolved, so they cannot move across a same-document refresh, and a proxy that would change them re-prepares through the command coordinator — which opens a new session rather than resuming, leaving nothing to refuse. `SessionCompatibilityKey::extra` is deliberately empty: no provider adapter has a precise resume identity to contribute.
>
> **Observed refusal behavior (open question).** On refusal, `start` fires *before* the key comparison, and `success`/`finalize` never fire — the refusal propagates as a hard error rather than routing through the lifecycle recovery stacks. The spec does not rule on whether `finalize` is owed here. Tracked in [`features/2026-07-13-proxy-with/notes/acceptance-map.md`](../../features/2026-07-13-proxy-with/notes/acceptance-map.md) → AC 15, which remains **partial** for this reason.
...
#### The in-harness fallback path

A handoff with no run ledger to surface to — the **direct provider wrappers** (`claudine claude`, `claudine goose`, …), not the composition commands — is adopted inside the harness instead, by `harness_orch/loop_control/target_launch.rs::rebuild_target_launch`. That rebuild is env-only: it recomputes provider/model into the `AGENT`/`MODEL`/`YOLO` overlay and the target's early-binding context, but does not re-select profile/binary, the argv entrypoint, or MCP runtime injection, which stay as the invocation resolved them.

These are limits of **this fallback alone**, not of the surfaced command coordinator above. They diverge only when a proxy changes the provider itself, and explicit CLI provider selection — the common case on the wrapper path — pins it.
