...
- **Context** — the prepared document stores the exact `ComposeContext` it composed against. Plain `ctx.*` is captured from the immutable invocation launch CWD and launch repository/topology evidence, then the selected target's agent/model environment overrides are applied. Body interpolation, effective frontmatter, lifecycle DM2 lookup, and shell preflight all read that one stored snapshot. The target's `SourceContext` remains separate and owns `$schema`, transclusion, provenance, and authored file resolution. `current.ctx.*` remains live at event time and is explicitly *not* a fallback for prepared plain `ctx.*`.
...
Darkmatter scans each document for its required `ctx.*` groups. For every document preparation epoch, the invocation owner constructs one target-adjusted launch snapshot and every preflight, compose, loop, and lifecycle consumer reuses it. A post-`initialize` stabilized reread extends only newly required groups from the same retained launch evidence; it does not replace the snapshot's anchor, environment capture, or target overrides. Proxy, retry, and resume entry begin fresh epochs while preserving those immutable launch facts.

The `SourceContext` never supplies plain `ctx.*`. It remains authoritative for `$schema`, transclusion, provenance, and authored file references, including a document stored in another repository. `current.ctx.*` is intentionally different: it is captured live at lifecycle event time. Canonical paths do not recapture ambient CWD, HOME, environment, Git, or topology for prepared plain `ctx.*`.
