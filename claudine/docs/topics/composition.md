---
hash: ef46db3751d8e999-b49dd427cfb31b3c
...
That is the contract, and it holds today for everything preparation owns. It does **not** yet hold for the launch inputs the wrapper computes before the loop — provider, model, MCP, argv, environment, and loop recognition — which are called out under [Target-specific behavior](#target-specific-behavior) below. Read the contract as the invariant the code is converging on, and that section as the ledger of what is left.
...
> **Not yet implemented: the resume session-compatibility key.** The spec requires a resume to retain a live session only when a compatibility key of the target's launch properties still matches, and to name the changed facet when it does not. Today's resume retains the live session without that check. The key's facets are pre-loop launch values, so it depends on the target-specific launch rebuild described below. Tracked in [`features/2026-07-13-proxy-with/notes/acceptance-map.md`](../../features/2026-07-13-proxy-with/notes/acceptance-map.md) → AC 15.
...
> **Not yet implemented: the target-specific launch rebuild.** Provider, model, MCP, argv, child environment, child CWD, system prompt, interactivity, and document-loop recognition are still computed **once, before the loop, from the originally-invoked document**. A proxied target therefore does not yet select its authored `model:`, and does not yet acquire its own `loop:`. This is the largest known gap in the equivalence contract; it is reproduced by two `#[ignore]`d L2 rows and tracked in [`notes/acceptance-map.md`](../../features/2026-07-13-proxy-with/notes/acceptance-map.md) → AC 7, 9, 10.
...
Two of the four fixes named in that note are live today — a proxied target requests approval for its own shell actions, and surfaces the same typed error direct invocation produces. **The other two are not yet landed**: additional loop iterations and authored-provider/model selection both wait on the target-specific launch rebuild above. They are fixes in intent and blocked in fact; see [`notes/acceptance-map.md`](../../features/2026-07-13-proxy-with/notes/acceptance-map.md) for the named criteria and their reproductions.
