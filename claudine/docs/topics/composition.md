...
>
> **Observed refusal behavior (open question).** On refusal, `start` fires *before* the key comparison, and `success`/`finalize` never fire — the refusal propagates as a hard error rather than routing through the lifecycle recovery stacks. The spec does not rule on whether `finalize` is owed here. Carried as an open question in [`features/2026-07-13-proxy-with/notes/acceptance-map.md`](../../features/2026-07-13-proxy-with/notes/acceptance-map.md) → AC 15; it does not qualify the refusal contract above.
