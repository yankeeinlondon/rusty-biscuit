...
The root is removed when the launch ends, after the guarded write-back described below, unless that write-back failed. Claudine holds an OS file lock on a sibling `<root>.lock` for the launch's lifetime; a root whose lock is free when the next launch starts (the process crashed or was force-exited before cleaning up) is removed then. Removal never follows the Unix mirror's links into the user's source root.
...
- **Write-back failed.** If a changed file cannot be written to its source — a permission error, a Windows sharing violation, a full disk — the overlay copy is the only copy of that state, so the launch root is kept rather than removed. Claudine prints a notice on stderr naming the provider, the file, the error, the kept copy's path, and the source path (never the contents), and writes the same notice to a sibling `<root>.retained` marker. A later launch's sweep never removes a root with that marker: copy the kept file over its source, then delete the root and its marker. If even the marker cannot be written, the notice says so, because a later sweep may then remove the root.
