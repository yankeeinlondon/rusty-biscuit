...
prompt: |-
    ...
    The **remote-signal** daemon will include a process monitor that looks across a host and detects running instances of Agentic CLI's:
    ...
    It is highly valueable for the **remote-signal** daemon to not only observe the start/stop lifecycle of Agentic processes but also to extract as much metadata and understanding as possible from these processes.
