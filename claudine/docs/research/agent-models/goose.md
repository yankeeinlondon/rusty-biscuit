...
custom_models:
  - kind: local
    config_site: "built-in Ollama / LM Studio / Atomic Chat / Docker Model Runner / Ramalama providers (OLLAMA_HOST, OPENAI_HOST+OPENAI_BASE_PATH, ATOMIC_CHAT_HOST)"
    notes: "Run any local tool-calling model. Ollama is the primary path (auto-lists installed models in the picker; default host localhost:11434; context via OLLAMA_CONTEXT_LENGTH). LM Studio (localhost:1234), Atomic Chat (localhost:1337), and Docker Model Runner (via the OpenAI provider with OPENAI_HOST=http://localhost:12434 + OPENAI_BASE_PATH=/engines/llama.cpp/v1/chat/completions) speak OpenAI-compatible APIs. Ramalama reuses the Ollama provider (OLLAMA_HOST=http://0.0.0.0:8080, needs --runtime-args=--jinja). Models without tool calling can only do chat and require all extensions disabled."
  - kind: openai_compatible
    config_site: "built-in OpenAI provider (OPENAI_HOST / OPENAI_BASE_PATH / OPENAI_CUSTOM_HEADERS / OPENAI_ORGANIZATION / OPENAI_PROJECT) AND custom_providers JSON (engine: \"openai\")"
    notes: "Two routes. (1) Point the built-in OpenAI provider at any OpenAI-compatible endpoint (vLLM, KServe, Docker Model Runner, enterprise proxies/gateways) via OPENAI_HOST (+ OPENAI_BASE_PATH for non-standard paths, + OPENAI_CUSTOM_HEADERS for extra auth). (2) Drop a JSON file in ~/.config/goose/custom_providers/ with engine \"openai\", a display_name, api_key_env, base_url, a models[] list (each {name, context_limit}), optional headers{}, supports_streaming, requires_auth. Custom providers appear in the picker and can be selected like any built-in; keys go to the keyring or secrets.yaml."
  - kind: anthropic_compatible
    config_site: "built-in Anthropic provider (ANTHROPIC_HOST) AND custom_providers JSON (engine: \"anthropic\")"
    notes: "Redirect the built-in Anthropic provider with ANTHROPIC_HOST, or register an Anthropic-Messages-compatible endpoint as a custom provider (engine \"anthropic\") in ~/.config/goose/custom_providers/. Custom headers for Anthropic/Ollama-engine providers must be added by editing the JSON file (the CLI wizard only supports headers for OpenAI-engine providers)."
  - kind: provider_plugin
    config_site: "~40 built-in first-party providers, each authenticated via its own env-key family"
    notes: "goose ships first-class providers for Amazon Bedrock (AWS_* / AWS_BEARER_TOKEN_BEDROCK), Amazon SageMaker TGI, Azure OpenAI (AZURE_OPENAI_*; API-key / Entra-bearer / credential-chain auth), GCP Vertex AI (GCP_PROJECT_ID+GCP_LOCATION), Databricks, Snowflake, Groq, Mistral, GitHub Copilot (device-flow OAuth), ChatGPT Codex (browser OAuth), Tetrate Agent Router (PKCE OAuth), OpenRouter, LiteLLM, Perplexity, OVHcloud, Scaleway, SaladCloud, Novita, EmpirioLabs, FuturMix, Routstr, Venice, Cerebras, xAI, iFlytek Spark/Astron, Avian, NEAR AI Cloud, and more. Each ships a declarative model catalog (crates/goose/src/providers/declarative/<provider>.json)."
  - kind: other
    config_site: "custom_providers JSON (engine: \"ollama\"); ACP providers (claude-acp, codex-acp); deprecated CLI providers (claude-code, codex, cursor-agent, gemini-cli)"
    notes: "Ollama-compatible custom providers use engine \"ollama\" in the custom_providers JSON. ACP providers (claude-acp, codex-acp) wrap Claude Code / Codex over the Agent Client Protocol and pass goose extensions through as MCP servers (npm install -g @agentclientprotocol/claude-agent-acp / @zed-industries/codex-acp). The bare CLI pass-through providers (claude-code, codex, cursor-agent, gemini-cli) are deprecated in favor of the ACP providers and do NOT carry goose extensions."
