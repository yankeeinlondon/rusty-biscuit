...
- name: finalize
prompt: |-
  Several agentic CLIs support installable plugins or extensions that can bundle slash commands, agents, MCP servers, hooks, and other assets into a distributable unit. Plugin ecosystems overlap with Claudine's own resource linking and MCP catalog, so understanding what each provider's plugins can carry — and how they are installed and discovered — matters.

  ## Task

  Your task is to report on plugin and extension ecosystems across the Agentic CLI providers Claudine supports.

  - your report should start by outlining why plugin ecosystems matter to agentic processes and how they relate to Claudine's cross-provider resource linking
  - and then shift its focus to how providers differ: whether plugins exist at all, their format and manifest shape, what asset types a plugin can carry, install/discovery mechanisms, and scoping
  - close with a point of view on how plugins intersect Claudine's linking strategy and MCP catalog, and whether plugins are a distribution channel Claudine should exploit

  As background material we have plugins research documents for each provider that Claudine supports. They can be found at `@claudine/docs/research/plugins/*.md`.
  ...
  - Note: the initial draft has already been created — it is the body of `@claudine/docs/research/summary/agent-plugins.md` (everything below the frontmatter); read it from there
  ...
  The document has now gone through several rounds of improvement and your task is just to make sure the document is consistent in tone and detail and that nothing looks incorrect or incomplete. The current draft is the body of `@claudine/docs/research/summary/agent-plugins.md` (everything below the frontmatter); read it from there, make any adjustments, and your final response will be considered the finalized summary document.
  ::end-block
hash: ef4a7a23363d40d0-5368f8393cb2e68a
