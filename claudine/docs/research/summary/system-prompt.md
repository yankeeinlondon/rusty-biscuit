...
- name: finalize
prompt: |-
  The system prompt is the most powerful lever for shaping an agentic session's behavior, but Agentic CLI providers differ widely in whether and how a user can influence it: some allow appending, some allow full replacement, some only offer context files (e.g. `AGENTS.md`-style memory), and the delivery mechanism varies between CLI flags, config settings, and environment. Claudine's wrappers expose a uniform file-backed interface — `--append-system-prompt`/`--asp`, `--replace-system-prompt`/`--rsp`, and `system-prompt.md` discovery from the launch-CWD hierarchy — and must map that onto each provider's native mechanism.

  ## Task

  Your task is to report on system prompt support across the Agentic CLI providers Claudine supports.

  - your report should start by outlining why system prompt customization matters to agentic processes and the difference between appending to versus replacing a provider's built-in system prompt
  - and then shift its focus to how each provider supports (or doesn't support) system prompt customization: native mechanisms, append vs replace capability, delivery method, and notable risks or limitations
  - close with a point of view on how well Claudine's uniform append/replace interface maps onto each provider

  As background material we have system-prompt research documents for each provider that Claudine supports. They can be found at `@claudine/docs/research/system-prompt/*.md`.
  ...
  - Note: the initial draft has already been created — it is the body of `@claudine/docs/research/summary/system-prompt.md` (everything below the frontmatter); read it from there
  ...
  The document has now gone through several rounds of improvement and your task is just to make sure the document is consistent in tone and detail and that nothing looks incorrect or incomplete. The current draft is the body of `@claudine/docs/research/summary/system-prompt.md` (everything below the frontmatter); read it from there, make any adjustments, and your final response will be considered the finalized summary document.
  ::end-block
hash: 988628e4c7a8858b-0e1d646ee0bee70a
