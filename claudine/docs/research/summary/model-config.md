...
- name: finalize
prompt: |-
  Beyond the out-of-box model set, every agentic CLI offers some path for users to add models — brand-new cloud models before the CLI's catalog absorbs them, or local models served by runners like Ollama and LM Studio. The extension mechanism is near-universally one of two informal standards (OpenAI-compatible or Anthropic-compatible APIs), with provider-specific config shape layered on top.

  ## Task

  Your task is to report on user-side model configuration and extension across the Agentic CLI providers Claudine supports.

  - your report should start by outlining why user-side model extension matters to agentic processes (local models, early access to new cloud models, aggregator routing)
  - and then shift its focus to how providers differ: config file and format, API standards spoken, base-URL delivery, adapter mechanisms, merge-versus-shadow semantics against the built-in catalog, and env-var overrides
  - close with a point of view on the cross-provider patterns and how cleanly each provider bridges to local runners

  As background material we have model-config research documents for each provider that Claudine supports. They can be found at `@claudine/docs/research/model-config/*.md`.
  ...
  - Note: the initial draft has already been created — it is the body of `@claudine/docs/research/summary/model-config.md` (everything below the frontmatter); read it from there
  ...
  The document has now gone through several rounds of improvement and your task is just to make sure the document is consistent in tone and detail and that nothing looks incorrect or incomplete. The current draft is the body of `@claudine/docs/research/summary/model-config.md` (everything below the frontmatter); read it from there, make any adjustments, and your final response will be considered the finalized summary document.
  ::end-block
hash: 5ac74435375ac0a0-a1959b2295d1617b
