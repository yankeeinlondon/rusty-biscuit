...
- name: finalize
prompt: |-
  Every agentic provider ships as a CLI binary, and each one chooses its own binary names per OS, install methods, auto-update behavior, release cadence, documentation surfaces, and command-line switch inventory. These identity facts are what Claudine needs for binary discovery, preflight checks, install guidance, and provider onboarding.

  ## Task

  Your task is to report on CLI identity and installation facts across the Agentic CLI providers Claudine supports.

  - your report should start by outlining why CLI identity facts matter to a wrapper like Claudine (binary discovery, install guidance, docs links, switch synthesis)
  - and then shift its focus to how providers differ: binary names and aliases per OS, install methods and whether they auto-update, version discovery, documentation surfaces, and notable differences in switch inventories
  - close with a point of view on which of these facts are stable enough to compile into Claudine's provider catalog versus which drift too fast

  As background material we have agent-cli research documents for each provider that Claudine supports. They can be found at `@claudine/docs/research/agent-cli/*.md`.
  ...
  - Note: the initial draft has already been created — it is the body of `@claudine/docs/research/summary/agent-cli.md` (everything below the frontmatter); read it from there
  ...
  The document has now gone through several rounds of improvement and your task is just to make sure the document is consistent in tone and detail and that nothing looks incorrect or incomplete. The current draft is the body of `@claudine/docs/research/summary/agent-cli.md` (everything below the frontmatter); read it from there, make any adjustments, and your final response will be considered the finalized summary document.
  ::end-block
hash: 664cfe0b460d6754-ed88075a43918a4d
