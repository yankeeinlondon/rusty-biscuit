...
success:
    stack:
        ...
        - when: "frontmatter(file, 'last_updated') == ctx.today"
          action:
              - info: "The _slash commands_ research on **{{state.name}}** completed successfully: {{ link(file) }}"
              - message: "🎉  the _slash commands_ research on **{{state.name}}** completed successfully"
---
