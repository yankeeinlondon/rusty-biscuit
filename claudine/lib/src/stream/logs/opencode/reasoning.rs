...
impl<S: SemanticEventSink> OpenCodeLogBridge<S> {
    ...
    fn on_step_loop(
        ...
        // succeed do not accumulate toward a false abort. The same advance is
        // progress for the stalled-generation backstop — reset its churn count
        // and silence clock. Deduped repeats returned early above, so they do
        // not reach this reset.
