//! Typed event models for Kimi Code.
//!
//! Two protocol surfaces are modeled here:
//!
//! 1. **Legacy stream-json** — the `KimiEvent` enum and its payload structs
//!    (`KimiInit`, `KimiContent`, `KimiStatusUpdate`, `KimiTool`,
//!    `KimiErrorEvent`). Retained for the in-flight semantic parser; will be
//!    removed when the parser switches to wire mode end-to-end.
//! 2. **Wire mode (JSON-RPC 2.0)** — `KimiEnvelope` plus the `KimiWireEvent` /
//!    `KimiWireRequest` enums and their typed payload structs. This is the
//!    protocol Kimi exposes via `kimi --wire`, and the protocol Claudine's
//!    Kimi wrapper switches to in this feature. All structs derive
//!    `Deserialize` with `#[serde(default)]` on every field, and unknown
//!    `event.type` / `request.type` discriminants fall through to a
//!    `RawNotification` / `RawRequest` arm so parsers retain the legacy
//!    `_ => Ok(None)` fallback semantics.
