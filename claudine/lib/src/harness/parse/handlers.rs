...
use crate::harness::model::{
    ApprovedRuntimeCommand, FailureEvent, HandlerAction, HandlerRule, HandlerTable,
};
...
pub(super) fn parse_failure_event(
    ...
    match name {
        ...
        "shell_audit_denied" => Ok(FailureEvent::ShellAuditDenied),
        _ => Err(HarnessError::InvalidFrontmatter {
            source_path: source_path.to_path_buf(),
            property: format!("handle_{name}"),
            detail: "must be one of: agent_failure, timeout, shell_audit_denied".to_string(),
        }),
    }
}
...
pub(super) fn parse_handler_entry(
    ...
    if is_direct {
        ...
        for (subject_key, subject_value) in obj {
            ...
            let canonical_key = normalize_handler_subject_key(subject_key, &event);
