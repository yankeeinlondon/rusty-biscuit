...
pub(crate) fn errors(
    ...
    let sql = builder.finish(
        ...
        SELECT timestamp, provider, event, session_key, repo_name, tool_name, error,
               prompt_text, tool_input_json, notification_message, extra_json
        FROM events
        "#,
    ...
    for row in rows {
        ...
        // Use extra_json only when it has meaningful content (not empty `{}`).
        let extra = extra_json.filter(|j| j != "{}");
