...
//! - **Bespoke records are documentation** — skipped here; bespoke emitters
//!   construct [`SignalEvent`]s directly and feed the same sink.
...
impl SignalEngine {
    ...
    ///
    /// Bespoke records never enter the groups. Regex compilation cannot
    /// fail for a table the generator validated — the impossible-error
    /// branch skips the record with a `warn!` instead of panicking.
