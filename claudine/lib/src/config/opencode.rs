  ...
  onEvent(event: string, data: unknown) {
    ...
    if (mapped) {
      try {
        ...
        execSync(`${CLAUDINE_BIN} handle ${mapped} '${payload}'`, {
          timeout: 30000,
          stdio: "ignore",
        });
      } catch {
...
///
/// Returns `None` for events that OpenCode doesn't support natively.
/// Unsupported events: ToolError, SubagentStart, SubagentStop, AfterModel.
fn to_opencode_native(event: &AgenticEvent) -> Option<String> {
    match event {
        AgenticEvent::BeforePrompt => Some("before_prompt".to_string()),
        AgenticEvent::TurnComplete => Some("turn_complete".to_string()),
        AgenticEvent::BeforeTool => Some("before_tool".to_string()),
        AgenticEvent::AfterTool => Some("after_tool".to_string()),
        AgenticEvent::SessionStart => Some("session_start".to_string()),
        AgenticEvent::SessionEnd => Some("session_end".to_string()),
        AgenticEvent::PermissionRequest => Some("permission_request".to_string()),
        AgenticEvent::BeforeModel => Some("before_model".to_string()),
        AgenticEvent::BeforeCompact => Some("before_compact".to_string()),
        AgenticEvent::Notification => Some("notification".to_string()),
        AgenticEvent::TurnError => Some("turn_error".to_string()),
        // Unsupported events - OpenCode doesn't have these event types
