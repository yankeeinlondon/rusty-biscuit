...
source_code:
  ...
  - claudine/cli/tests/fixtures/ctx_launch_anchor/in_repo.md
  ...
  - claudine/cli/tests/sequence_ctx_launch_anchor.rs
...
documentation:
  ...
  - claudine/fixes/2026-08-12-ctx-launch-anchor/phase-1-baseline.md
  - claudine/fixes/2026-08-12-ctx-launch-anchor/phase-6-acceptance.md
...
- [x] Expose the epoch-extension operation: project only the missing
...
- [x] Run `just test` and `just lint` from `claudine/`; confirm the new API is
...
- [x] Move context construction in
...
- [x] Change lifecycle pipeline setup to consume the prepared epoch snapshot
...
- [x] Update overlay preparation, passthrough harness seed materialization,
...
- [x] Run focused system-prompt, overlay, harness, passthrough, and
...
- [x] Run the path-sensitive regression matrix on macOS and ensure fixtures are
  structurally portable to Windows and Linux; where CI is available, require
  all three OS jobs before closure (AC13).
- [x] Review all behavior-adjacent `///`, `//!`, and inline comments touched by
  the change for contract accuracy, and verify relevant README/docs/skill
  snapshots changed alongside public behavior.

### Final validation checkpoint

- [x] Run `just test` from `claudine/` and record a clean nextest result.
- [x] Run `just test-l2` from `claudine/` with terminal/browser fixtures kept in
  the background so no window gains focus (AC14).
- [x] Run `just lint` from `claudine/` and record a clean result (AC14).
- [x] Confirm the original real-world repro now reports the launch package area
  from both repository-root and package-local prompt copies, including
  lifecycle interpolation and `when:` evaluation.
- [x] Review `git diff --check` and the final diff for unrelated edits; do not
