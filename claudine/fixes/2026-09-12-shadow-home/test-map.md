...
| Native Windows | `L2::windows::level2_wezterm_windows_codex_repo_overlay_copies_nested_directories_and_keeps_the_user_home` (WezTerm `cmd.exe`, rustc-built recorder, write-through copy proof) | compile only: `cargo check -p claudine-cli --tests --features terminal-tests --target x86_64-pc-windows-gnu`, clean. Not run: `build-win-native` `W:` had 53 KB free on 2026-09-16 (review-2 finding 2). The test no longer depends on the known-folder home: its launcher names the Codex source through `CODEX_HOME` and overlay storage through `CLAUDINE_OVERLAY_DIR`, both inside the fixture workspace, and waits for the `cmd.exe` prompt instead of sleeping; the same explicit-root launch is L1-proven on Unix by `level1_provider_overlay_home::explicit_codex_home_and_overlay_dir_keep_the_overlay_out_of_the_home`. CI's `windows-latest/L2` cell is an `accepted-gap`; the Phase 5 copy-mode unit tests are the only native Windows materialization evidence (Phase 5 log, 26 passed on `build-win-native`) | **unmet** |
