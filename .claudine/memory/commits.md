...
- **`git commit --only -- <new_path>` for a staged rename breaks rename detection.** The previous lesson covers the "rename commit only recorded deletions" failure mode; the corollary is that even when both halves of the rename are in the staged set, using `--only` with the new path produces a commit that records the *old* path as a deletion and the *new* path as an unconnected addition — `git show --stat` shows `D …/old.md` and `A …/new.md` rather than `R100 …/old.md → …/new.md`. The rename is still detectable in history (the prior commit's `add` of the old path can be matched by `-M` similarity), but the current commit loses the explicit rename. To keep the rename atomic, either include both `old` and `new` paths in the `--only` pathspec, or commit without path-limiting. For 1:1 file moves the cleanest pattern is a single commit with both paths listed. For moves of many files at once, run a single sequential commit covering all old + new pairs in one `--only` invocation.
