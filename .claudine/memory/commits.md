...
- **New staged additions can be demoted to untracked in a concurrent batch (2026-07-12 claudine Phase-6 fleet batch).** `A ` (staged addition) → `??` (untracked) is a distinct failure mode from the existing "staged set can shrink" lesson. Where `--only -- <paths>` silently excludes paths that a sibling already committed (because they're no longer in the index), a sibling `git restore --staged <paths>` (or an aborted sibling operation that rolls back the index) demotes the new files back to untracked without removing them from disk. Observed in the Phase-6 fleet batch: between the subagent's initial `git status --short` and its first `git commit --only -- <paths>` attempt, the 10 assigned new files showed as `??` instead of `A `; `--only --` then committed zero files because the pathspecs no longer matched the index. Recovery is `git add <verified-paths>` followed by the same `git commit --only -F - -- <paths> <<'COMMIT_MSG' … COMMIT_MSG` command; the subagent confirmed the retry committed the full set in one shot. Diagnostic: if `git commit` returns 0 but `git show --name-status <hash>` shows fewer files than expected (or empty), check `git status --short` — paths showing as `??` need to be re-staged, not re-committed. Distinct from the sibling-committed case (where the path appears in *another* commit's tree) because here the path is not in any commit's tree and the working-tree file is the only copy.
