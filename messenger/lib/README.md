...
The `research` feature adds `messenger::research`, the typed, offline loader and validator for the provider research contract (`messenger/docs/platforms.yaml` and `messenger/docs/research/`). It checks each file against its shipped schema through Darkmatter's library, applies the semantic rules listed in `docs/research/platforms/_rules.md`, derives which constraints are executable, and decides whether reviewed implementation assessments can be reused. It also projects validated accepted research into a deterministic catalog (`research::project`), compares a candidate with accepted research fact by fact (`research::delta`), builds filterable reports with the per-adapter truncation and diagnostic handoffs (`research::report`), and publishes the accepted snapshot atomically through a committed manifest and a local journal (`research::publish`, driven by `research::generate`). `research::refresh` owns the refresh and review lifecycle: refresh selection, prepared per-pass run inputs in the gitignored state area, stage checks that judge written outputs rather than agent exit codes, the approval policy (human approval for substantive changes; automatic acceptance only for verified unchanged renewals), promotion with durable review records and a generated research CHANGELOG, and dry-run-first cleanup of local records. It prepares Claudine sequence documents but never spawns Claudine or an agent. It never changes `CapabilitySet` or delivery behavior, and send builds without the feature carry none of its dependencies (`darkmatter`, `biscuit-file`, `biscuit-hash`, `serde_path_to_error`).
