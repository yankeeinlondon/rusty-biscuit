---
# Several controls in one message, each submitting independently: grouped
# controls are not a single-submission form. Criterion 22. Expect: valid.
...
- id: fb.fx.controls
  interface: discord_bot_api
  container: message_controls
  question_kinds:
  - single_choice
  - confirmation
  submission: per_field_events
  ...
  lifecycle:
    proactive: 'yes'
    repeat_submissions: replaces_previous
  knowledge:
    state: known
    evidence:
    - src.fx.docs
