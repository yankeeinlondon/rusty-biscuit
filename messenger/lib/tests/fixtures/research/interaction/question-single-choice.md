---
# Single choice with stable option IDs distinct from labels, responder and
# correlation locators, and a stale option ID. Criterion 21. Expect: valid.
...
- id: qb.fx.single
  ...
  visibility: public
  fidelity: exact
  answer_event: callback_query
  answer_locator: /callback_query/data
  option_id_locator: /callback_query/data
  options: static
  min_selections: 1
  max_selections: 1
  responder_locator: /callback_query/from/id
  correlation_locator: /callback_query/message/message_id
  lifecycle:
    repeat_submissions: allowed
    closable: 'yes'
  knowledge:
    state: known
    evidence:
...
- id: ix.fx.pick
  binding: qb.fx.single
  provenance: constructed_from_docs
  payload: '{"callback_query": {"data": "opt.b", "from": {"id": "USER_ID"}, "message": {"message_id": "MESSAGE_ID"}}}'
  expect: answer
  expected_answer: '"opt.b"'
- id: ix.fx.stale
  binding: qb.fx.single
  provenance: constructed_from_docs
  payload: '{"callback_query": {"data": "opt.removed", "from": {"id": "USER_ID"}, "message": {"message_id": "MESSAGE_ID"}}}'
  expect: stale_option
  note: Option ID no longer offered by the current question version.
changes: []
gaps:
- id: gap.fx.scope
  kind: research
