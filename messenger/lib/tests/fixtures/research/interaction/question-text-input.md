---
# Text input bound to a question: an empty string answer versus a dismissed
# modal. Criterion 21. Expect: valid.
...
- id: qb.fx.text
  ...
  kind: text_input
  native: native
  mechanism: text_input
  operation: interaction_callback
  native_locator: components[] (text input in modal)
  answer_type: string
  ...
  visibility: public
  fidelity: exact
  answer_locator: /data/components/0/components/0/value
  lifecycle:
...
- id: ix.fx.empty
  ...
  payload: '{"data": {"components": [{"components": [{"value": ""}]}]}}'
  expect: answer
  expected_answer: '""'
  note: An empty submitted string is an answer.
- id: ix.fx.dismissed
  binding: qb.fx.text
  provenance: constructed_from_docs
  payload: '{}'
  expect: dismissed
  note: Closing the modal is a separate outcome, never an empty string.
changes: []
gaps:
- id: gap.fx.scope
  kind: research
