---
# An exact intent mapping without evidence for the effect's meaning.
# Schema-valid by design; the Rust rule rejects it.
# expect-rule: SR-EXPRESSION
...
- id: ex.fx
  ...
  discovery: unknown
  intent: anger
  intent_fidelity: exact
  knowledge:
    state: known
    evidence:
    - src.fx.docs
    explanation: Assumed from the effect name.
delivery_controls: []
eligibility: []
rate_limits: []
envelopes: []
