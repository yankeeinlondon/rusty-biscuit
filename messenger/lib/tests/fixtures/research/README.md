...
| `lifecycle/fleet/` | One complete accepted document per roster platform (every roster interface, a coverage matrix resting on one investigated gap, a few Discord and Slack constraints). `tests/research_publication.rs` and the CLI tests copy them to the accepted paths of a temporary repository (rebinding `$schema`) as the publication baseline | Schema-valid and semantically valid in `Accepted` scope |
