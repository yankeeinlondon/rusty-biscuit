...
proves validation is passive, and scans the corpus for secrets and control
sequences.
...
| `negative/semantic/` | One semantic defect per file that SimplifiedSchema cannot express | Schema-valid by design; the Rust rule in the `# expect-rule:` header (and the file-stem prefix, `sr-unique--…`) rejects it. `sr-enforceable--*` fixtures validate but must never reach the executable projection |
