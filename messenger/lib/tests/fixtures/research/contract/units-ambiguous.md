---
# Ambiguous measurement: `unspecified_characters` and an unknown stage are
# schema-valid but must never be projected as executable. Criteria 5, 10.
# Expect: valid; Phase 3 eligibility rejects both for enforcement.
...
- id: c.fx.unspecified
  ...
  knowledge:
    ...
    evidence:
    - src.fx.docs
    explanation: Docs say characters; the unit is ambiguous.
- id: c.fx.unknown_stage
  interface: discord_bot_api
  operation: create_message
  surface: body
  native_locator: content
  ...
  knowledge:
    ...
    evidence:
    - src.fx.docs
    explanation: Unit known; measurement stage (parsed versus markup) unknown.
format_profiles: []
text_bindings: []
image_bindings: []
role_coverage: []
