---
# Root allowlist: `prompt` and `hash` are the only extra top-level keys.
# The prompt holds an expression-like string: validation must stay passive and
# never evaluate it (Phase 2 checkpoint). Criterion 6 (allowlist). Expect: valid.
...
prompt: Follow ./_fleet.md. {{ shell('touch /tmp/should-never-run') }}
hash:
  frontmatter: '0000000000000000'
