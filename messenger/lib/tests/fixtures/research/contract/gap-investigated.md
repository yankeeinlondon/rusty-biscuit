---
# Every category cell is an investigated gap (question, searches, inspected
# sources, unresolved reason, blocked decision, next step), plus a structured
# `requires_messenger_update` gap with its reason. Criteria 2, 28; the Phase 2
# freeze gate ("every required category can express an investigated gap").
# Expect: valid.
...
- id: gap.fx.update
  ...
  status: investigated
  interface: discord_bot_api
  category: delivery_controls
  question: Should the adapter expose silent delivery?
  facts: []
  searches:
  - adapter source review
  inspected_sources:
  - src.fx.code
  unresolved_reason: Needs a Messenger API decision.
  blocked_decision: Silent-delivery support for this adapter.
  next_investigation: Propose a follow-up feature.
requires_messenger_update: true
reason: The adapter drops a researched field; see gap.fx.update.
---
