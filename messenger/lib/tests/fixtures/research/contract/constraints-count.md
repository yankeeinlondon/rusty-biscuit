---
# Collection counts, a minimum item count, and a lower text bound that
# affects whether a shortened message stays valid. Criterion 4. Expect: valid.
...
chronology: []
constraints:
- id: c.fx.attachments.max
  interface: discord_bot_api
  operation: create_message
  surface: attachments
  native_locator: attachments
  kind: item_count_max
  value: 10
  unit: items
...
- id: c.fx.content.min
  ...
  knowledge:
    ...
    evidence:
    - src.fx.docs
    explanation: Empty text is rejected; a shortened message must keep at least one character.
format_profiles: []
text_bindings: []
image_bindings: []
role_coverage: []
