---
# A recommendation stored apart from the service threshold (Slack-style 4000
# recommended versus 40000 truncation). Both bounds stay visible. Criterion 4.
# Expect: valid.
...
- id: c.fx.text.recommended
  ...
  applies_when: []
  recommended_by: src.fx.docs
  knowledge:
    state: known
    evidence:
    - src.fx.docs
    explanation: A recommendation; it names its source and is not enforced.
- id: c.fx.text.truncation
  interface: slack_web_api
  operation: chat_post_message
  surface: body
  native_locator: text
  ...
  applies_when: []
  overflow_details: Service truncates silently; the send still succeeds.
  knowledge:
    state: known
    evidence:
    - src.fx.docs
