...
use crate::discovery::SonyDiscovery;
...
pub struct SonyIntegrationHandler {
    ...
    driver: SonyDeviceDriver,
    discovery: SonyDiscovery,
...
impl SonyIntegrationHandler {
    ...
    async fn handle_setup_driver(
        ...
        if let Err(error) = context.send(setup_driver_response(request.id, 200)).await {
            warn!(error = %error, req_id = request.id, "failed to acknowledge Sony setup start");
            return None;
        }

        let remote_id = remote_id_from_context(context);
        info!(remote_id, req_id = request.id, "Sony setup started");
        ...
        info!(
            ...
            "Sony setup discovery finished"
        );
        for device in &candidates {
            registry.add_known_device(device.clone()).await;
        }
        if let Err(error) = registry.save().await {
            warn!(error = %error, "failed to persist Sony setup candidates");
        }
        ...
        if let Err(error) = context.send(selection_event).await {
            warn!(error = %error, "failed to send Sony setup selection");
        }
    ...
    async fn handle_set_driver_user_data(
        ...
        if let Err(error) = context.send(result_response(request.id, 200)).await {
            warn!(
                error = %error,
                req_id = request.id,
                "failed to acknowledge Sony setup user data"
            );
            return None;
        }
        ...
        let mut config = if let Some(device_id) = selected_device_id.as_deref().filter(|value| !value.is_empty())
        {
                ...
                .cloned()
            {
                ...
                return self
                    .setup_error(
                        request.id,
                        context,
                        format!("unknown Sony device selection {device_id}"),
                        400,
                    )
                    .await
                    .and(None);
            ...
            let Some(metadata) = self
                .discovery
                .validate_host(&host, SONY_DEFAULT_PORT, SETUP_VALIDATE_TIMEOUT)
                .await
            else {
                return self
                    .setup_error(
                        request.id,
                        context,
                        format!("unable to validate Sony receiver at {host}"),
                        400,
                    )
                    .await
                    .and(None);
            ...
            ConfiguredDevice {
                device_id: metadata
                    .mac_address
                    .clone()
                    .unwrap_or_else(|| format!("{host}:{SONY_DEFAULT_PORT}")),
                device_name: requested_name
                    .clone()
                    .filter(|value| !value.is_empty())
    ...
    async fn discover_candidates(&self, known_devices: Vec<KnownDevice>) -> Vec<KnownDevice> {
        let candidates = build_candidate_list(
            &known_devices,
            &[],
            &local_ipv4_candidates(SONY_DEFAULT_PORT),
        );
    ...
    async fn setup_error(
        ...
        if let Err(error) = context
            ...
            .await
        {
            warn!(error = %error, "failed to send Sony setup error");
        }
...
impl WsHandler for SonyIntegrationHandler {
    async fn handle_message(&self, message: Value, context: WsConnectionContext) -> Option<Value> {
        if let Some(error) = remote_setup_abort_error(&message) {
            let remote_id = remote_id_from_context(&context);
            self.sessions.clear(&remote_id).await;
            warn!(remote_id, error, "UC Remote aborted Sony setup");
            return None;
        }
...
mod tests {
    ...
    async fn test_handle_get_available_entities() {
        ...
        handler
            .manager
            .registry()
            .assign_device("connection-1", "192.168.99.99:10000")
            .await
            .unwrap();
        let resp = handler
