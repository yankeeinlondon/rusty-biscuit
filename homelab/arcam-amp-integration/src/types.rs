...
pub fn driver_metadata() -> DriverMetadata {
    DriverMetadata {
        ...
        name: HashMap::from([("en".to_string(), "Arcam Amplifier".to_string())]),
        version: DRIVER_VERSION.to_string(),
        min_core_api: MIN_CORE_API.to_string(),
        description: Some(HashMap::from([(
            "en".to_string(),
            "Control Arcam amplifiers over the local network.".to_string(),
        )])),
...
mod tests {
    ...
    fn test_driver_metadata() {
        ...
        assert_eq!(metadata.name["en"], "Arcam Amplifier");
