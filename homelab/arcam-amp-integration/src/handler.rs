...
impl ArcamIntegrationHandler {
    ...
    async fn handle_setup_driver(
        ...
        if let Err(error) = context.send(setup_driver_response(request.id, 200)).await {
            warn!(error = %error, req_id = request.id, "failed to acknowledge Arcam setup start");
            return None;
        }

        let remote_id = remote_id_from_context(context);
        info!(remote_id, req_id = request.id, "Arcam setup started");
        ...
        info!(
            ...
            "Arcam setup discovery finished"
        );
    ...
    async fn handle_set_driver_user_data(
        ...
        if let Err(error) = context.send(result_response(request.id, 200)).await {
            warn!(
                error = %error,
                req_id = request.id,
                "failed to acknowledge Arcam setup user data"
            );
            return None;
        }
        ...
        info!(
            ...
            "Arcam setup user data received"
        );
...
impl WsHandler for ArcamIntegrationHandler {
    async fn handle_message(&self, message: Value, context: WsConnectionContext) -> Option<Value> {
        if let Some(error) = remote_setup_abort_error(&message) {
            let remote_id = remote_id_from_context(&context);
            self.sessions.clear(&remote_id).await;
            warn!(remote_id, error, "UC Remote aborted Arcam setup");
            return None;
        }
...
mod tests {
    ...
    async fn make_handler(add_device: bool) -> ArcamIntegrationHandler {
        ...
        if add_device {
            let config = ConfiguredDevice {
                device_id: "192.168.99.99:50000".to_string(),
    ...
    fn setup_string_reads_nested_values() {
        assert_eq!(
            super::setup_string(
                &json!({"user_data": {"device_name": {"value": "office"}}}),
                "device_name"
            )
            .as_deref(),
            Some("office")
        ...
        assert_eq!(
            super::setup_string(
                &json!({"setup_data": {"device_name": "office"}}),
                "device_name"
            )
            .as_deref(),
            Some("office")
