...
## `--pathspec-from-file` Rejects Empty Lines

`git commit --pathspec-from-file=<list>` reads the list as a set of pathspecs and treats *empty lines* (and whitespace-only lines) as the literal empty-string pathspec, producing `fatal: empty string is not a valid pathspec` and aborting without committing. The `-F` / `-m` flags and the file format note ("each path on its own line (LF or CRLF) and `#`-comments are allowed") do not call this out. The gotcha is silent in code generation: a script that builds the pathspec list with `echo $path` after a conditional branch, or that pipes `git status --short` paths through `awk` and ends with a trailing newline, leaves a single empty line that defeats the commit. Defense: always pipe the generated list through `grep -v "^$"` (or build it explicitly with `printf "%s\n" "$@" > list`) before passing to git, and validate with `wc -l < list` against `git diff --cached --name-only <scope> | wc -l` so a missing-or-extra line is visible before the commit. Hit in the claudine docs restructure (`4d24947c8`) where an `awk`-built path list ended with a blank line and aborted the first commit attempt.
