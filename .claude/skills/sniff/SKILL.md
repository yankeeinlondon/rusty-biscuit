...
hash: 3cd50ffff2b8b5db-b635023f1c5ff58b
...
- **Every subprocess goes through `process::run_with_timeout`** (`process.rs`), which owns the deadline, the pipe draining, and the reaping. **Never poll `try_wait()` over a piped stdout you are not draining** — the child blocks in `write()` past one pipe buffer (64 KiB Linux / ~16 KiB macOS), never exits, and gets killed at its deadline with its output lost. That bug was live in `os/time.rs`, `programs/schema.rs`, and `host_capability.rs`; `Command::output()` drains correctly but blocks forever without a deadline. Deadlines are policy and live in `process::timeouts` (services/Windows-locale 3s, `diskutil` 5s, host-capability 2s, program-schema/NTP 3s). Tests inject short deadlines; never sleep for a production one.
...
- **`repo_full` ≈ `repo_structure` + inventory.** Since Phase 4 their probe and `read_dirs` counts land within 1 of each other on the 375-package fixture. A full-mode counter materially above structure's means a second enrichment pass has been re-introduced.
...
- **Final baselines are the Phase 8 table** (`phases/08-cross-platform-validation/spec.md`), which compares every case against Phase 1 and flags the three counters that cannot be compared naively. Headline vs Phase 1: `metadata_probes` −48%/−49%, `read_dirs` −50%, `manifest_parses` −42%/−40%, `package_enrichments` −50%, deep-status `blob_loads` 400 → 200. Results-shaped counters (`entries_visited`, `files_accepted`, `documents_parsed`) are **unchanged** — the feature changed work, not results. The two structure-mode cases are byte-identical to Phase 1 and serve as the drift bracket.
