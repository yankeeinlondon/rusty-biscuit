...
The silence rule was hardened on 2026-05-10 (fix: `2026-05-10-opencode-timeout-regression`) after a regression where commit-style flows on OpenCode + Minimax 2.7 were misclassified as hangs. Two layers compensate for OpenCode's sparse stream (no `tool_start`, no `task_started`, so `in_flight` / `in_flight_subagents` stay empty during legitimate work): **(1)** a provider-agnostic raw-byte heartbeat — every non-empty stdout/stderr chunk from the wrapped child refreshes `LiveMetricsState.last_byte_at` **before** the bytes reach the semantic parser, so partially-buffered output still counts as activity; whitespace-only writes are ignored so blank-line flushes cannot look infinitely active. The `step_timeout` formula now uses `LiveMetricsState.last_activity_at()` which returns `max(last_event_at, last_byte_at)`. **(2)** an OpenCode-specific `provider_status` grace — until at least one `step_finish` boundary has been observed (`LiveMetricsState.provider_status.is_some()`), the silence rule is suppressed entirely for OpenCode; the wall-clock `timeout` rule remains the unconditional backstop. The guard fires only for `Provider::OpenCode`; richer-stream providers do not need it. `TimeoutConfig` carries the active `Provider` discriminant from spawn time so `evaluate_timeout_tick` can apply the guard without re-resolving. A genuinely stuck child producing zero bytes still allows `step_timeout` to fire normally — the byte heartbeat does not mask real hangs. Test fixtures in `claudine/cli/tests/wrap_commands.rs` (`watchdog_*` family) cover both the byte-heartbeat and the OpenCode `provider_status` paths.
