...
hash: bbb32528c11dc53d-39cd1fd6008836aa
---
...
- **Lifecycle stack-only globals** — three globals supplement frontmatter/`ctx.*`/`env.*`/`doc.*`: `err` (`kind`, `variant`, `msg`) for `blocked`/`failure`/optional-error `finalize`, `timing` (`document_ms`, `total_ms`, `step_ms`), and `current` (`current.ctx.*`, `current.env.*` lazy snapshots at event time). They reach two kinds of surface differently. **Expression surfaces** — `when:` clauses and multi-argument expression-verb args — evaluate them as bare references, and a bare `err` in a no-error event (`initialize`/`start`/`success`/`loop`) is rejected at parse time. **Single-parameter action message bodies are literal text** (the `literal-short-form-args` change), so a global reaches them only through `{{ … }}` interpolation resolved at render time in `render_message`: `say({{err.msg}})` prints the error, while a bare `say(err.msg)` is the literal characters `err.msg` (no parse-time rejection, because it is not a reference). `doc.err` reaches a literal frontmatter `err` property everywhere.
