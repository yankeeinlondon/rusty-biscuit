...
A provider-agnostic subagent and stream-idle watchdog was added on 2026-05-03 (feature: `2026-05-02-opencode-stability`). `LiveSemanticSink` maintains a shared `WatchdogState` (populated by `SubagentStart` / `SubagentStop` and updated by any event carrying a recognized subagent id) that is inspected by a dedicated ticker running every 5 s (configurable via `CLAUDINE_SUBAGENT_WATCHDOG_INTERVAL_SECONDS`). Two rules evaluate in priority order: (1) if any active subagent exceeds `CLAUDINE_SUBAGENT_IDLE_KILL_SECONDS` (default 180 s, `0` disables), the child process group receives SIGTERM with a 10 s grace (`CLAUDINE_SUBAGENT_KILL_GRACE_SECONDS`) before SIGKILL escalation, and the run reports exit reason `subagents_unresponsive`; (2) if no subagents are outstanding and the stream has been silent for `CLAUDINE_STREAM_IDLE_KILL_SECONDS` (default 300 s, `0` disables), the same escalation fires with exit reason `stream_idle_timeout`. The existing `flush_if_idle` ticker also emits at most one diagnostic line per active subagent per 30 s silence window (`⏳ Awaiting subagent: <name> (<elapsed>)`), gated on the subagent watchdog being enabled. Both exit reasons surface in the JSONL summary as `error_kind: "subagents_unresponsive"` and `error_kind: "stream_idle_timeout"`, respectively, and render as `Agent Error` BlockQuotes on stderr. The watchdog cooperates with the existing `wait_with_signal_and_early_termination` path so SIGTERM escalation and parser finalization reuse the same plumbing already used for rate-limit recovery and step timeouts.
