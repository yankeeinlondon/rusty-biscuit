...
The live stderr surface was hardened on 2026-04-14 (feature: `2026-04-14-more-meta-response`, Plan 3). `LiveSemanticSink` in [`claudine/cli/src/commands/wrap/live_semantic_sink.rs`](../../../claudine/cli/src/commands/wrap/live_semantic_sink.rs) no longer emits truncated raw-JSON payload tails on status lines. Both `summarize_provider_payload` (for `ProviderExtension` events) and `summarize_input` (for tool calls) walk a curated set of text locations — known top-level fields (`message`, `status`, `name`, `path`, `text`, `content`, `error.message`, `title`, `description`, …), nested content arrays (`message.content[*].text`, `item.content.parts[*].text`, …), and the first non-empty top-level string value as a last resort. When none of those resolve, the sink renders `provider/kind` with no ` · <payload>` tail rather than a JSON blob. A hard-coded `SILENT_PROVIDER_EXTENSION_KINDS` allowlist in the same file fully suppresses the stderr line for high-volume or redundant extension events (currently Claude `stream_event` and `hook_started` / `hook_response` / `hook_progress`); dispatch and the JSONL semantic log still fire for those events. The Gemini parser in [`claudine/lib/src/stream/gemini_semantic.rs`](../../../claudine/lib/src/stream/gemini_semantic.rs) silently drops non-assistant `message` events (the operator's own prompt replay) rather than emitting a `message.non_assistant` `ProviderExtension`. The OpenCode wrapper adds an `stderr_noise_prefixes` override with `✱ `, `$ `, `> build `, and `████ ` to suppress OpenCode's default-mode TUI formatter output when running `--format json`.
