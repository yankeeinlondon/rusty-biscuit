...
Test commands run from the `claudine/rendezvous/` area justfile: `just check`, `just build`, `just test`, `just lint` each iterate all three crates (`test-l2` is a no-op — real-terminal tests do not apply). Ordinary package-keyed L1 cells for `rendezvous-core`, `rendezvous-client`, and `rendezvous-daemon` gate runtime coverage on macOS, Linux, Windows, and WSL2 with no `continue-on-error`; cross-compilation is explicitly **not** accepted as Windows evidence. The per-package Windows check compiles all targets, but the non-test `register_compaction_spike` example no longer receives the retired macOS/Linux compile-only check.
