...
|--------|-------|-------------|
| `sony_receiver` | ~1286 | Sony ES receiver JSON-RPC client (HTTP) |
| `arcam` | ~240 | Arcam PA240/PA410/PA720 binary TCP protocol |
| `network` | ~26 | `Host` enum (V4, V6, DNS) with Display |
...
- **Data types** — `PowerStatusResult`, `VolumeInfo`, `InputSource`, `PlayingContentInfo`, `SystemInformation`, `MethodSignature`, `GenericSettingResult`
...
- Content: `get_scheme_list()`, `get_source_list()`, `get_content_count()`, `get_content_list()`
- Playback: `get_playing_content_info()`, `stop_playing_content()`, `pause_playing_content()`, `set_play_next_content()`, `set_play_previous_content()`
- Settings: `get_speaker_settings()`, `get_bluetooth_settings()`, `set_bluetooth_settings()`, `get_playback_mode_settings()`
- System: `get_system_information()`, `get_sw_update_info()`, `act_sw_update()`
...
- Device host via `--host` flag or environment variables (`SONY_RECEIVER`, `ARCAM_AMP`)
...
│   │   └── AlexaStatus / EciaInfo / WuTangInfo
│   ├── Audio(SonyAudioAction)
│   │   ├── Volume / SetVolume / MuteStatus / Mute / Unmute
│   │   └── SpeakerSettings { target: SpeakerTarget }
│   ├── Input(SonyInputAction)
│   │   ├── List / Current / Set / Schemes / Sources
│   │   ├── ContentCount / ContentList / Browse / SetTerminal
│   │   └── Bluetooth / SetBluetooth / PlaybackMode
│   ├── Playback(SonyPlaybackAction)
│   │   ├── NowPlaying / Stop / Pause / Next / Previous
│   │   ├── Functions / SupportedFunctions / Preset
│   │   └── Seek / Scan (with Direction enum)
...
- **State**: `AppState` with `Arc<SonyReceiver>` (shared) and Arcam host string
...
`AppState::from_env()` parses Sony host with support for:
- IPv4: `192.168.1.100`
...
|---------|-------|------|-----------|
| `homelab` (lib) | Unit tests | Action serialization | `#[cfg(test)]` |
| `homey` (cli) | 21 integration | CLI structure, help, completions | `assert_cmd` + `predicates` |
| `homelab-server` | 22 integration | HTTP status codes, error responses | `axum-test` |
