...
+  `current` (of `ctx`) and `current_env` (of `env`).
+- Transitional during `features/2026-09-09-more-context`: declared-but-unbuilt
+  functions live in `expression/functions/pending.rs` and fail loudly, and
+  `context::catalog::PENDING_CAPTURE_KEYS` lists cataloged keys without
+  capture. Both lists are ratchets, so implementing a name fails a test until
+  it is removed from its list.
