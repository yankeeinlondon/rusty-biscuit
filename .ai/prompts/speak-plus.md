...
The original intent was to have a binary which would -- out of the box -- interrogate and use the host's TTS capabilities without any need for other external libraries. However, it's clear that `tts` _does_ need additional work when used on at least Linux to make it work correctly.
