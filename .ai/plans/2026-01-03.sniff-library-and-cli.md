...
lint:
    cargo clippy -p sniff-lib -p sniff-cli
...
**Blast Radius:** `cargo build -p sniff-cli && cargo test -p sniff-cli`
...
- [ ] `cargo run -p sniff-cli -- --help` shows help text
- [ ] `cargo run -p sniff-cli -- --format json` outputs valid JSON
- [ ] `cargo run -p sniff-cli -- --base .` works
...
**Blast Radius:** `cargo test -p sniff-lib && cargo test -p sniff-cli`
...
- [ ] `cargo test -p sniff-cli --test cli` runs 3+ tests
- [ ] All tests pass
- [ ] `cargo build --release -p sniff-cli` succeeds
