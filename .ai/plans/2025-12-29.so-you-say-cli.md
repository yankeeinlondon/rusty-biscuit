...
**Resolved Concerns:**
- **Package naming** → Package name is `so-you-say`, binary name is `speak` via `[[bin]]`
- **Stdin handling** → Read all lines until EOF, join with spaces, 10,000 char limit
- **Empty stdin** → Print helpful usage message, exit with code 1
- **Test organization** → Unit tests in `#[cfg(test)]`, integration tests in `tests/cli_test.rs`
- **Buffer safety** → Explicit character limit to prevent memory exhaustion
...
```
┌─────────────────────────────────────┐
│  so-you-say CLI (binary: speak)     │
│  ┌───────────────────────────────┐  │
│  │  main.rs                      │  │
│  │  - Clap argument parsing      │  │
│  │  - Stdin handling             │  │
│  │  - Call shared::tts::speak()  │  │
│  └───────────────────────────────┘  │
└─────────────────────────────────────┘
               │
               │ depends on
...
┌─────────────────────────────────────┐
│  Host Device Audio                  │
│  (macOS/Linux/Windows native TTS)   │
└─────────────────────────────────────┘
```
...
2. Clap parses arguments → Vec<String>
3. Join arguments with spaces → "Hello world"
...
2. Detect no arguments → read from stdin
3. Read line from stdin → "Hello world"
4. Call shared::tts::speak("Hello world")
...
1. Multi-word: `["Hello", "world"]` → `"Hello world"`
2. Single word: `["Hello"]` → `"Hello"`
3. Empty vector: `[]` → `""`
4. Unicode: `["Hello", "世界", "🚀"]` → `"Hello 世界 🚀"`
5. Empty strings: `["", "Hello", "", "world"]` → `" Hello  world"`
6. Special chars: `["Hello,", "world!"]` → `"Hello, world!"`

**Integration Tests:**
7. Binary with args: `speak test` → exit 0
8. Help flag: `speak --help` → shows help, exit 0
9. Version flag: `speak --version` → shows version, exit 0
10. Stdin input: `echo "test" | speak` → exit 0
11. Empty stdin: `speak < /dev/null` → shows error, exit 1
...
|----------------|--------|--------|
| Sequential | Phase 1 → Phase 2 → Phase 3 → Phase 4 | Each phase depends on previous completion |
...
- [x] **Should the CLI support any flags?** → No, keep it simple (positional args + stdin only)
- [x] **Should we add rate limiting or caching?** → No, out of scope for v1
- [x] **Should we support file input (`speak -f file.txt`)?** → No, use stdin redirection instead
- [x] **Binary name: `speak` or `so-you-say`?** → `speak` (simpler UX, matches README description)
- [x] **Package name vs binary name?** → Package: `so-you-say`, Binary: `speak` via `[[bin]]`
- [x] **Stdin: single line or multi-line?** → Read all lines until EOF, join with spaces, 10,000 char limit
- [x] **Empty stdin behavior?** → Print helpful usage message, exit with code 1
- [x] **Should we add to a justfile for installation?** → Check if root justfile exists; if so, add target
