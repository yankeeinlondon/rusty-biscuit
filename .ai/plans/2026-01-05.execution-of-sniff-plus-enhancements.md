...
- [x] Snapshot Complete [5 January 2026]
    - [x] 1 lint error (fixed: `.filter_map(|name| name)` → `.flatten()` in git.rs:279)
...
- [ADD]: metal = "0.30" (macOS only, via target cfg) - GPU detection via Metal API
- [REMOVE]: hardware-query optional dependency (had upstream bugs)

Note: GPU detection uses platform-specific cfg guards. Code compiles on all platforms:
- macOS: Full GPU detection via Metal API
- Linux/Windows: Returns empty Vec (future: add Vulkan/D3D12 support)
