...
**Acceptance Criteria:**
- [x] `WorktreeInfo` struct defined with `branch`, `filepath`, `sha`, `dirty` fields
- [x] `GitInfo` has `worktrees: HashMap<String, WorktreeInfo>` field
...
**Acceptance Criteria:**
- [x] `in_worktree` field added to `GitInfo`
- [x] Returns `false` for normal repositories, `true` when in worktree
...
**Acceptance Criteria:**
- [x] `CommitInfo` has `remotes: Option<Vec<String>>` field
- [x] `GitInfo.head_commit` removed, replaced with `recent: Vec<CommitInfo>`
- [x] Returns up to 10 commits in reverse chronological order
...
**Acceptance Criteria:**
- [x] `--deep` flag added to CLI with help text
- [x] Config threads through to git detection
- [x] Default behavior unchanged when not specified
...
**Acceptance Criteria:**
- [x] `RemoteInfo.branches` is `None` without `--deep`
- [x] `RemoteInfo.branches` populated with `--deep`
...
**Acceptance Criteria:**
- [x] `is_behind` is `None` without `--deep`
- [x] Lists remotes where local is behind when `--deep`
- [x] `CommitInfo.remotes` populated for recent commits
...
**Acceptance Criteria:**
- [x] Recent commits displayed (first as HEAD, rest at -v)
- [x] Worktrees listed at verbose level 1+
- [x] `in_worktree` indicator shown when true
- [x] `is_behind` and remote branches shown with `--deep`
...
**Acceptance Criteria:**
- [x] Worktree detection tests
- [x] Recent commits tests
- [x] Deep mode tests (is_behind, remote branches)
...
**Acceptance Criteria:**
- [x] `--deep` flag parses correctly
- [x] Help text includes `--deep` description
...
1. `GitInfo.head_commit` removed → use `recent.first()`
