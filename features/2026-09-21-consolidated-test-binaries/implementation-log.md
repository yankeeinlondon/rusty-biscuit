...
Worked 2026-09-22 on macOS (`aarch64-apple-darwin`) with a clean tree at start.
This phase changes documentation only. The source edits are comments, failure
messages, one persisted provenance field, and the sweep script. No test body,
input, assertion, or selector-bearing test logic changed. The Phase 6
human-review items (free `W:`; confirm the manifest as sole authority) are
still open. Phase 7 does not depend on either, so it went ahead.

### What was done

| Plan task | Result | Artifact |
|---|---|---|
| Re-sweep tool | `consumer-sweep.py --after`: the old per-file target names are no longer Cargo targets, so it reads them from the four `*-migration.json` manifests. Other packages' current targets still mark a bare name ambiguous, and a `-p <other>` line is still dropped. Historical rows carry a record date (dated directory, else last commit). The default (Phase 1) mode's header is unchanged | `baseline/consumer-sweep.py` |
| Starting point | 3 active and 7 in-flight hits remained. Phases 3–6 had already rewritten 27 of the 30 active entries | — |
| Active-doc sweep | rewrote the last 3 active hits: `.claude/skills/claudine/completions/shell-completions.md` (now matches `claudine/docs`), `rust-testing/SKILL.md`'s narrowing paragraph, `prompts/_prompt.md` | — |
| In-flight specs | rewrote the 2 live procedures (steering "Repeat the test", perf-followup "Regenerating fixtures"). Kept 4 execution records (a checked task's measured note, a "this exact command" run record, 2 checked validation checkpoints) | inventory annotation |
| Result | **0 active**, 4 in-flight (all records), 38 self, 547 historical; regenerated byte-identical | `baseline/test-selector-consumers-after.md` |
| Inventory annotation | Phase 7 section appended to the Phase 1 inventory: dispositions of all 30 active and 8 in-flight entries, record dates, and the rule used (guidance is rewritten, records are kept) | `baseline/test-selector-consumers.md` |
| `rust-testing` skill | "Narrowing a recipe to one subject" (positional filter, ANDed with the tier `-E`) and a new section "Consolidated Integration-Test Binaries": layout, explicit discovery and the layout gates, adding a test file, module name = old target name (tier markers apply), the feature boundary, snapshots follow `module_path!()`, `parity_helpers` as a preserved legacy shape, and a two-sided process-isolation subsection. The spawn-guard row now names the real paths. Topic pages `integration-tests.md`, `cli-output-testing.md`, `nextest.md` fixed. `hash`/`last_updated` refreshed with `md hash` | `.claude/skills/rust-testing/` |
| Old-path drift | a second scan for old per-file *paths* in active files. Fixed: claudine spawn-guard (×2) and dispatch-guard failure messages, the `dispatch-inventory.json` `tool` field, the `transport-allow.toml` header, comments in `level2_lifecycle_control.rs` and `biscuit-terminal-cli`'s `level2_style_everywhere_matrix.rs`, `cliclick.md`'s `level3_*.rs` glob | — |
| READMEs and docs | `biscuit-terminal/README.md` Level 2 row (flagged in Phase 6). `docs/testing-strategy.md`, the human-facing deep dive, gains the process-isolation and positional-filter note | — |
| `rust-devops`, `os` skills | checked: neither describes per-file test binaries or archive composition, so no change | — |
| Spec notes | no ruling changed a behavior the spec documents, so no spec note. `parity_helpers` falls outside §2's `common` statement and is recorded in Phase 6 and in the skill | — |

### Decisions

- **Guidance vs. record.** In an in-flight spec, a line that says how to do
  something next time is rewritten. A line that records what was run, with its
  result, is kept. The same rule decided which old paths to fix.
- **`dispatch-inventory.json`: one line, not a re-bless.** Regenerating with
  `CLAUDINE_UPDATE_INVENTORY=1` also rewrote 49 `line` numbers. That drift comes
  from earlier source edits, and the match test masks line numbers. To keep the
  diff surgical, the JSON was restored and only line 2 (`tool`) was edited. The
  match test compares that field: with the literal updated and the JSON not, it
  fails; with both, 12/12 pass.
- **Not changed, on purpose.** `test_placement.rs:832-834` and
  `test_affected_scope.py:4508` pass synthetic paths into assertions (a test
  input edit). `sniff/cli/tests/spawn_site_guard.rs` is correct, because sniff
  is not migrated. `.claude/skills/claudine/timeline.md` has a dated entry.
  `.claude/agents/feature-tester-rust.md` has a generic `cargo test --test '*'`
  table row that names no migrated target. The 276 path mentions in in-flight
  spec plans are records.
- **Claudine's own layout-gate copy** (`test_placement.rs`, not yet switched to
  `test_toolkit::test_layout`) is a test-code change that changes its manifest
  `additions`. It is not a docs-sweep item and stays a follow-up for the
  author, as Phase 4 recorded.
- **Positional filter semantics verified, not assumed.** On nextest 0.9.136,
  `-E 'test(/(^|::)level2_/)' completion_perf::` lists 0 tests and the L1
  expression with the same filter lists 5, so the filter and `-E` are ANDed.
  The skill states this and names the version.
- The spec's illustrative `wrap_env` module does not exist in claudine. The
  skill and `testing-strategy.md` use `context_command`, which does.

### Requirement-to-test mapping

| Changed behavior | Test / check (targeted) | Red-then-green shown |
|---|---|---|
| Acceptance 11: no active `--test <old-binary>` | `consumer-sweep.py --after` → 0 active; regenerated byte-identical | yes: a planted `cargo nextest run -p claudine-cli --test context_command` line was reported as active; a planted `-p sniff-cli --test spawn_site_guard` line was correctly dropped; restored, byte-identical |
| Rewritten selectors select real tests | `cargo nextest list` for `--test l1 shipped_prompt_contract::` (4), `--features real-tests --test real real_pi_steering:: --run-ignored all` (3), darkmatter `--test l1 benchmark_fixtures::` (≥3), `--test l1 completion_perf:: --run-ignored only` (≥3) | — |
| Documented recipe pattern works | `just test-cli context_command::` in `claudine/`: 26 run, 26 passed | — |
| Positional filter ANDs with the tier `-E` | `cargo nextest list` with L2 vs L1 expression plus `completion_perf::`: 0 vs 5 | the L2 case is the negative |
| `dispatch-inventory.json` `tool` field | `dispatch_inventory::dispatch_inventory_matches_committed_file` | yes: JSON line reverted → FAIL; restored → 12/12 PASS |
| Skill documents layout, positional filter, feature boundary, two-sided isolation | independent read-only reviewer agent (sonnet), checked against spec §6–7 and acceptance 11, with tree spot-checks | PASS on both items, no issues |
| Skill renders | `md render` exit 0, new headings present; `md validate refs` 0 issues on the 5 edited Markdown files | — |

"Passive corpus": the `--after` sweep reads every git-tracked text file.
"Shipped artifact end-to-end": the rewritten selectors were run and listed
through nextest and the real `just` recipe. No value is persisted except the
`dispatch-inventory.json` field, whose read-back is the match test above.

### Gates run

- claudine `INSTA_UPDATE=no just test --no-fail-fast`: 7,334/7,334 passed,
  9 skipped.
- claudine `just lint`: exit 0.
- biscuit-terminal `INSTA_UPDATE=no just test --no-fail-fast`: 3,263/3,263
  passed, 55 skipped.
- biscuit-terminal `just lint` (lib + CLI): exit 0.
- No `.snap.new` files.
- GitNexus `detect-changes --scope all`: risk low, 0 affected processes.
  `impact` on the two edited spawn-guard test functions returned `UNKNOWN`
  (the index predates the Phase 3 move). `git grep` confirmed they have no
  callers, being `#[test]` functions, and only message text changed.
- Not run: `just test-l2` (the only L2 edit is a comment in
  `level2_lifecycle_control.rs`, and `just lint` compiled it), darkmatter area
  suites (no darkmatter source changed), and cross-OS runs (no code path
  changed).

### Pre-existing failures

None met. The darkmatter failures recorded in Phases 4 and 5 are in an area
this phase did not touch.

### Host problems

Not re-checked. Windows and WSL2 remain pending for all four packages (see
Phase 6). Phase 8 needs them.

### Changed files

Skills: `.claude/skills/rust-testing/{SKILL,integration-tests,cli-output-testing,nextest}.md`,
`.claude/skills/claudine/completions/shell-completions.md`,
`.claude/skills/cli/cliclick.md`. Docs: `biscuit-terminal/README.md`,
`docs/testing-strategy.md`, `prompts/_prompt.md`, and the two in-flight
procedure READMEs. Source (message, comment, or provenance only):
`claudine/cli/tests/l1/{spawn_site_guard,dispatch_inventory}.rs`,
`claudine/cli/tests/l1/error_guards/transport-allow.toml`,
`claudine/cli/tests/level2/level2_lifecycle_control.rs`,
`claudine/docs/providers/dispatch-inventory.json`,
`biscuit-terminal/cli/tests/level2_style_everywhere_matrix.rs`. Feature
directory: `baseline/consumer-sweep.py`, `baseline/test-selector-consumers.md`,
the new `baseline/test-selector-consumers-after.md`, `plan.md`, and `spec.md`.

Suggested commits: (1) the sweep tool and inventory files; (2) the skill and
doc rewrites; (3) the claudine and biscuit-terminal-cli message and comment
fixes, with the one-line `dispatch-inventory.json` change. Each is comment-,
doc-, or message-only, per the repo's scope rule.
...
Worked 2026-09-22 on macOS (`aarch64-apple-darwin`) at `6b9eb3dc7`, with a
clean tree at start. This phase adds evidence files and two read-only check
scripts in the feature directory. No package source, test, recipe, or skill
changed.
...
| Plan task | Result | Artifact |
|---|---|---|
| Producer observations | **Pending, left unchecked.** The branch is 58 commits ahead of `origin/main` (`8d2b0c6ce`) and has never been pushed. `gh run list --branch fix/ci-build-feature-divergence` returns nothing. The spec forbids triggering a run for this. The file holds the pull request 92 context (as context only) and a harvest procedure | `ci-observations.md` |
| WSL2 closure | Re-checked, **still pending**. `ssh build-win-native "Get-PSDrive C, W"`: `W:` 36.77 GB free (36 at Phase 6), `C:` 25.23 GB, both below the 50 GiB preflight. `ssh build-win 'df -h ~'`: `kex_exchange_identification: read: Connection reset by peer`, the full-`W:` symptom in the `os` skill's `build-hosts.md`. No ordinary nightly run exists. Nothing was deleted and no preflight was overridden | `acceptance.md` §4 |
| Acceptance document | all 12 criteria, each linked to its artifact. 9 met, 5 met pending reviewer reading (spec: "fail review"), 4 and 10 partly pending | `acceptance.md` |
| `cargo metadata` check | new `acceptance/metadata-check.py`: Cargo test targets (name, path, required features) equal each manifest's `targets`, `autotests = false`, and each old target maps to exactly one module. PASS: 4/5/2/2 targets, 139/74/53/38 old targets | `acceptance/metadata-check.{py,txt}` |
| Body-diff review aid (criterion 5) | new `acceptance/body-diff.py`: every old file at its base against the new module at `HEAD`. Changed lines are classified structural, comment, or other, and every "other" line is listed | `acceptance/body-diff.{py,md}` |
| Final sweep | see Gates run | — |
| Status transition | spec `status: implemented`, `implemented: true`, `implemented_by: claude/opus`. Not moved to `_completed` (the author's action) | `spec.md` |

### Decisions

- **Producer observations stay unchecked.** The task asks for numbers from a
  run that does not exist yet, and the spec forbids triggering one. Checking it
  would claim evidence nobody has.
- **Windows is a gap, not a pass.** Criterion 4 allows only *WSL2* to be
  pending. Native-Windows listings are required, and the evidence is still
  compile-only (the `x86_64-pc-windows-gnu` cross-compile). `acceptance.md`
  names that gap for native Windows too, and gives the steps to close it.
- **Body-diff reads `HEAD`, not the migration commit.** A reviewer needs the
  tree as it stands, including the Phase 7 message rewrites. The report
  labels "other" lines as review items. All 16 modules with "other" lines were
  read: path repairs for the extra directory level (`include_str!`, fixtures,
  allow-list files), guard path sets that now carry the tier directory
  (darkmatter-cli spawn guard, Phase 5), message and selector text (Phase 7),
  and claudine's layout gate (acceptance 12, manifest `additions`). None
  changes an input, expected value, timeout, or skip.
- `test-l2` and `test-browser` were not re-run. No test source changed after
  their last green runs (Phases 3–6), apart from Phase 7's comment and message
  edits, which `just lint` compiled.

### Requirement-to-test mapping

| Changed behavior / claim | Test / check | Red-then-green shown |
|---|---|---|
| Criterion 1/3: Cargo targets = manifest targets, exact features | `acceptance/metadata-check.py` on the live tree → PASS, exit 0 | yes: tampered metadata with darkmatter `level3-browser` on `terminal-tests` → 2 FAIL lines; an extra `biscuit-terminal` `stray` target → FAIL, exit 1; real metadata → exit 0 |
| Criterion 5: only allowed edits | `acceptance/body-diff.py` over all 304 modules | the classifier's "other" bucket is its negative. It caught 16 modules, all read and dispositioned above |
| Criterion 12 guards live | the four `every_test_source_is_compiled_by_a_declared_target` tests passed in the sweep | red shown in Phases 3–6 |
| Criterion 6: no `.snap.new` | `find claudine darkmatter biscuit-terminal -name '*.snap.new'` → 0; sweep under `INSTA_UPDATE=no` | — |
| Criteria 2, 4, 6, 7 | re-read the committed JSON: 4× `comparison-darwin-linux.json` verdict `identical`, 0 failures; 4× `attribute-check.json` 0 failures; 4× `snapshot-check.json` 0 failures | — |

"Passive corpus" and "shipped artifact end-to-end": the metadata check reads
the real `cargo metadata` and every shipped manifest. The body diff reads every
moved file on both sides. Nothing is persisted.

### Gates run

- `INSTA_UPDATE=no just test claudine darkmatter biscuit-terminal` (root):
  18,819 run, **18,818 passed, 1 failed**, 73 skipped. The failure is the
  pre-existing
  `darkmatter::l1 schema_phase_validation::public_docs_and_skill_describe_required_and_eager_as_independent_axes`.
- `just lint` in `claudine`, `darkmatter` (lib + CLI), `biscuit-terminal`
  (lib + CLI): all exit 0.
- `just check-canonical`: 27 passed, 0 failed.
- `just check-tier-coverage claudine darkmatter biscuit-terminal`: claudine and
  biscuit-terminal pass. darkmatter has 1 stranded test, the pre-existing
  `schema_phase_validation::real_shipped_inline_schema_uses_normal_resolution_and_phase_path`.
- `acceptance/metadata-check.py`: PASS.
- Not run: `just test-l2`, `just test-browser`, `just test-l3`, cross-OS (see
  Decisions and Host problems).

### Pre-existing failures

Both come from Phase 4, proven identical on the unmigrated base `048e44f7a`,
and are outside this feature's scope (spec §3 forbids test edits here):

1. the `schema_phase_validation` missing-doc test, which reads a file deleted
   by `aa1f03c70`,
2. the stranded `real_`-named test in `check-tier-coverage darkmatter`.

### Host problems

- `build-win-native`: `W:` 36.77 GB free, under the 50 GiB preflight.
- `build-win` (WSL2): SSH reset at key exchange, the same full-`W:` symptom.
- `build-linux` was not needed. The Linux evidence from Phases 3–6 stands.

### Changed files

Created: `acceptance.md`, `ci-observations.md`,
`acceptance/metadata-check.py`, `acceptance/metadata-check.txt`,
`acceptance/body-diff.py`, `acceptance/body-diff.md`. Updated: `plan.md`,
`implementation-log.md`, `spec.md`. All are in this feature directory.
