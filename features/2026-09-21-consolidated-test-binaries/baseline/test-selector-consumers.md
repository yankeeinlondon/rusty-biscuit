...
## Phase 7 annotation (2026-09-22)

The tables above are the Phase 1 snapshot and are kept as generated. The
re-sweep after all four migrations is
[`test-selector-consumers-after.md`](test-selector-consumers-after.md),
generated by `consumer-sweep.py --after`. That mode reads the old per-file
target names from the four `*-migration.json` manifests, because they are no
longer Cargo targets. Result: **0 active**, 4 in-flight-spec, 38 self, 547
historical. The historical table there lists each file's record date. The one
historical file added since Phase 1 is this feature's own
`implementation-log.md`.

### The 30 active entries

All 30 now use the positional-filter form (`just test-cli <module>::`, or
`cargo nextest run -p <pkg> --test <consolidated> <module>::`). 27 were
rewritten in the package phases (3–6). Phase 7 rewrote the last three:
`.claude/skills/claudine/completions/shell-completions.md:779`,
`.claude/skills/rust-testing/SKILL.md:565` (now the "Narrowing a recipe to
one subject" paragraph), and `prompts/_prompt.md:200`. The test-code string
literals (R9) were done in Phases 3–4.

### The 8 in-flight-spec entries

| Entry | Phase 7 disposition | Record date |
|---|---|---|
| `claudine/features/2026-07-13-error-propogation/plan.md:571` | **historical record**: the note on a checked task, with the timing it measured (`(1.4s)`); not rewritten | 2026-07-22 |
| `claudine/features/2026-07-13-proxy-with/notes/acceptance-map.md:652` | **historical record**: "selected with this exact command", followed by its Nextest run IDs; not rewritten | 2026-07-22 |
| `claudine/features/2026-09-08-steering/verification/README.md:41` | **rewritten**: a "Repeat the test" procedure, so live guidance; now `--test real real_pi_steering::` (lists 3 tests) | — |
| `darkmatter/features/2026-07-15-performance-followup/benchmarks/README.md:51` | **rewritten**: the "Regenerating fixtures" procedure; now `--test l1 benchmark_fixtures::`, matching `manifest.yaml`'s header | — |
| `darkmatter/features/2026-07-15-performance-followup/benchmarks/README.md:53` | **rewritten**: same procedure | — |
| `darkmatter/features/2026-07-15-performance-followup/benchmarks/manifest.yaml:4` | already rewritten in Phase 4 (the header `benchmark_fixtures.rs` emits, R9) | — |
| `renderable/features/2026-06-30-style-everywhere/plan.md:275` | **historical record**: a checked validation checkpoint with its recorded result (`✅ 2563 lib + 404 cli tests pass`); not rewritten | 2026-06-30 |
| `renderable/features/2026-06-30-style-everywhere/plan.md:643` | **historical record**: a checked validation checkpoint; not rewritten | 2026-06-30 |

Record dates are the `git blame` commit date of the line. The rule used: a
line that says how to do something next time is guidance and is rewritten. A
line that says what was run, with its result, is a record and is kept.

### Old source paths (not `--test` selectors)

A second scan, using the same classifier, looked for the old per-file
*paths* (`cli/tests/<old>.rs`) in active files. Phase 7 fixed the stale
ones: the claudine spawn-guard and dispatch-guard failure messages, the
`dispatch-inventory.json` `tool` field, the `transport-allow.toml` header,
and comments in `level2_lifecycle_control.rs` and
`biscuit-terminal-cli`'s `level2_style_everywhere_matrix.rs`. Left as they
are: `claudine/cli/tests/l1/test_placement.rs:832-834` and
`scripts/ci/test_affected_scope.py:4508`, which are synthetic path inputs to
assertions, not references to files; `sniff/cli/tests/spawn_site_guard.rs`
(sniff is not migrated); and the dated entry in
`.claude/skills/claudine/timeline.md`. The 276 path mentions in in-flight
spec plans and matrices record work done against the old layout and are
treated as records.
