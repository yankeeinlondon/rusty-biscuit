...
human_review_items:
    - |-
        **Windows and WSL2 proof is still blocked by a full disk on the Windows build machine, and now two packages are waiting on it.**

        This project moves each package's many small test programs into a handful of larger ones. Two packages are done: `claudine-cli` (139 programs down to 4) and now `darkmatter` (74 down to 5). For both, on macOS and Linux, a tool proved test by test that every test still exists, runs in the same tier, and is skipped or ignored exactly as before. On Windows I could only show that the code *compiles* (built on macOS for Windows, with no warnings). The plan also wants the test-by-test proof on a real Windows machine and a WSL2 check.

        The Windows machine (`build-win-native`) still has 43 GB free on `W:`, and every build recipe refuses to start below 50 GB. The WSL2 machine (`build-win`), whose disk lives on the same drive, still drops every SSH connection. I did not build there and did not delete anything.

        The next phase moves `darkmatter-cli`, the third of four packages. The plan's reason for proving Windows early is that a Windows-only mistake found now costs one or two packages, while one found at the end costs four.

        - **Option A: free space on `W:` (at least ~60 GB free), then have the next agent run the Windows and WSL2 proof for both finished packages before Phase 5 (recommended).** Pros: any Windows-only flaw is caught before it is copied a third time, and the proof covers both packages in one sitting of about 1–2 hours of machine time. Cons: you need to clear disk space first.
        - **Option B: continue with Phase 5 now and run the Windows/WSL2 proof for all packages once space is free.** Pros: no waiting. Cons: a Windows-only layout problem would be repeated in a third package before anyone sees it. The risk looks low (the cross-compile is clean and the Windows-only test modules compile), but it is not zero.
        - **Option C: accept compile-only evidence for Windows, and let the next ordinary CI run on `main` provide the real Windows run.** Pros: no machine work. Cons: CI runs Windows only after merge, so a problem would be found after it lands. It also weakens the plan's acceptance criterion 4.

        I recommend **Option A**. Everything else is clean on macOS and Linux, so this is likely a formality. But it is the only check that can catch a Windows-only test silently disappearing, and it gets more expensive to redo with each package.
    - |-
        **Still open from Phase 2: confirm that the migration manifest is the one authoritative record of where each test moves.**

        Two packages now rely on it (`claudine-cli-migration.json`, `darkmatter-migration.json` in the feature folder). Each records, for every old test program, the new program and module it landed in. Every before/after check reads only that file, and all came back identical. The darkmatter file carries two hand-recorded additions, each with a written reason: one reviewed warning (a test that lists its own program's tests as sample shell output) and the one new layout-guard test.

        - **Option A: approve the manifest as the only authority (recommended).** Pros: one record, and every check is mechanical. Cons: a mistake in it reaches every check, but the before/after comparison names any test that lands in the wrong place.
        - **Option B: also require a hand-reviewed table per package.** Pros: a person reads every row. Cons: 1,000+ rows across the two packages so far, and a second record that can drift.
        - **Option C: change the format first.** Pros: still cheap now. Cons: delays Phase 5 and changes the toolkit.

        I recommend **Option A**.
message_to_agent: |-
    Phase 4 (darkmatter) is implemented. Read the log's "## Phase 4" section first. Status: macOS and Linux fully proven (identity identical across 6 feature sets and both slow-policy states; suites green apart from pre-existing failures proven on the base). Native-Windows listings and WSL2 are still PENDING (`W:` 43 GB free, `build-win` resets SSH), for claudine-cli and darkmatter alike. If the owner has freed `W:`, do both packages before Phase 5: on-host `consolidation.py capture --package <pkg>` of the base and migrated trees (use `py` on Windows; see the `os` skill), `compare` with the package's manifest (feed darwin+linux+windows captures together), then `just cross-check <pkg> --os windows` and `--os wsl`.

    For Phase 5 (darkmatter-cli):
    - `plan` refuses `selfproof/capture-a` for every package now: `.config/nextest.toml` overrides were rewritten in Phases 3 and 4. Re-capture the unmigrated package first (`capture --package darkmatter-cli --out <feature>/darkmatter-cli/capture-before`), and `compare` it with `capture-a` as a sanity check (expect only override-rewrite notes). Phase 4 also made comment-only edits to `darkmatter/cli/tests/level2_{code_block_styling,disclosure_blocks}.rs` (moved-path swaps), which change no identity.
    - The layout gate is now shared: `test_toolkit::test_layout::{collect_test_sources, layout_violations}` (`tools/test-toolkit/src/test_layout.rs`). darkmatter-cli already dev-depends on test-toolkit; copy `darkmatter/lib/tests/l1/test_layout.rs` (non-vacuity asserts + the call) and record it as a manifest `addition`. claudine-cli still has its own Phase 3 copy in `tests/l1/test_placement.rs`. Switching it to the shared module (deleting ~250 lines and its three `layout_gate_*`/`module_declarations_*` unit tests, whose coverage now lives in test-toolkit) is a deliberate follow-up that also means updating `claudine-cli-migration.json`'s `additions`. Do it as its own commit, or leave it for Phase 7. biscuit-terminal (Phase 6) has test-toolkit only as an optional `terminal-tests` dependency, so its L1 gate needs a dev-dependency.
    - `pilot/apply-move.py <manifest> <guard-module.rs>` is now package-neutral. It moves nested crate roots whole and declares `common` only when `tests/common/mod.rs` exists. darkmatter-cli has `common/` + `mod common;`, like claudine. Helper directories and `#[path]` helpers are still hand edits (see Phase 4 "Decisions"). Move `*.proptest-regressions` files beside their modules if any exist.
    - Expect the known alias `level2_harness_integrity` → `harness_integrity` (R2); the planner produces it.
    - Linux: the `build-linux` standing clone is still locked by `feat-nightly-perf` and lacks HEAD. Use a private `--shared` clone plus `git bundle create … 3efdbe450..HEAD`, and a working-tree patch built with a temporary `GIT_INDEX_FILE` (`git read-tree HEAD; git add -A -- . ':!features/…'; git diff --cached --binary HEAD`). Export `RUSTC_WRAPPER=""` everywhere. Delete `~/scratch/*` afterward.
    - Windows cross-compile: `RUSTC_WRAPPER="" CARGO_TARGET_DIR=target/windows-check CFLAGS_x86_64_pc_windows_gnu=-Wa,-mbig-obj CXXFLAGS_x86_64_pc_windows_gnu=-Wa,-mbig-obj cargo check -p <pkg> --tests --target x86_64-pc-windows-gnu --features <all>`.

    For Phase 6 (biscuit-terminal): `renderable/justfile` `drift-report` now switches its selector per crate (darkmatter uses `--test l1 render_comparison::`). Change biscuit-terminal's branch in the same package migration, and drop the `case` once both use `l1`. `biscuit-terminal/lib/examples/layout_matrix.rs` `#[path]`-includes `../tests/layout_matrix_support/mod.rs`, so keep that helper at `tests/` (as darkmatter did).

    Pre-existing failures seen in Phase 4 (not caused by it, proven on base `048e44f7a`, separate-defect candidates): `schema_phase_validation::public_docs_and_skill_describe_required_and_eager_as_independent_axes` (reads a doc deleted by `aa1f03c70`; fails everywhere, so darkmatter `just test` is 1 red); `check-tier-coverage darkmatter` (stranded `real_shipped_inline_schema_…`); Linux-only `horizontal_rule_integration::tests::test_custom_weight_thick_differs_from_thin`.

    Left for Phase 7 (in addition to Phase 3's list): the `rust-testing` skill should mention `test_toolkit::test_layout` as the acceptance-12 gate.
---
