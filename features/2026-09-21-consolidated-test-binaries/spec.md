...
human_review_items:
    - |-
        **Windows and WSL2 proof is still blocked by a full disk on the Windows build machine. Three packages are now waiting on it, and the disk has less free space than last time.**

        This project merges each package's many small test programs into a few larger ones. Three of four packages are done: `claudine-cli` (139 programs down to 4), `darkmatter` (74 down to 5), and now `darkmatter-cli` (53 down to 2). For each one, a tool checked on macOS and Linux, test by test, that every test still exists, runs in the same group, and is skipped or ignored exactly as before. Every check came back identical. On Windows, I could only show that the code *compiles*: it was built on macOS for Windows, with no warnings. The plan also calls for the test-by-test check on a real Windows machine, plus a WSL2 check.

        The Windows machine (`build-win-native`) now has **37.9 GB** free on `W:` (43 GB last phase), even after the build recipe's own automatic cleanup. Every build recipe refuses to start below 50 GB. The WSL2 machine keeps its disk on the same drive. I did not override the limit and did not delete anything by hand.

        The next phase moves the last package, `biscuit-terminal`.

        - **Option A: free space on `W:` (at least ~60 GB free), then have the next agent run the Windows and WSL2 proof for all three finished packages before Phase 6 (recommended).** Pros: a Windows-only problem is caught before it is repeated a fourth time, and one session of about 2 hours of machine time covers all three packages. Cons: you have to clear disk space first, and the machine is losing space between phases, so something is filling it.
        - **Option B: continue with Phase 6 now, and run the Windows and WSL2 proof for all four packages at the end (Phase 8), once space is free.** Pros: no waiting. Cons: any Windows-only layout problem would be in all four packages before anyone sees it. The risk looks low (every Windows cross-compile is clean, including the Windows-only test modules), but it is not zero.
        - **Option C: accept compile-only evidence for Windows, and let the next ordinary CI run on `main` supply the real Windows run.** Pros: no machine work. Cons: CI runs Windows only after merge, so a problem would be found after it lands. It also weakens acceptance criterion 4.

        I recommend **Option A**. Everything else is clean on macOS and Linux, so this is probably a formality. But it is the only check that can catch a Windows-only test silently disappearing, and it gets more expensive with every package.
    - |-
        **Still open from Phase 2: confirm that the migration manifest is the one authoritative record of where each test moves.**

        Three packages now rely on it: `claudine-cli-migration.json`, `darkmatter-migration.json`, and `darkmatter-cli-migration.json` in the feature folder. Each records, for every old test program, the new program and module it moved into. Every before/after check reads only that file, and every check came back identical. The files carry a few hand-recorded additions, each with a written reason: the new layout-guard test in each package, and one reviewed warning in `darkmatter`.
        ...
        - **Option B: also require a hand-reviewed table per package.** Pros: a person reads every row. Cons: more than 1,600 rows across three packages, and a second record that can drift from the first.
        - **Option C: change the format first.** Pros: still possible. Cons: it delays Phase 6 and changes a toolkit three packages already depend on.

        I recommend **Option A**.
message_to_agent: |-
    Phase 5 (darkmatter-cli) is implemented. Read the log's "## Phase 5" section first. macOS and Linux are fully proven: identity is identical for both feature sets and both slow-policy states, the guards' file sets match the baseline by path, and the suites are green apart from Phase 4's two pre-existing darkmatter-library failures. Native Windows and WSL2 are still PENDING for claudine-cli, darkmatter, and darkmatter-cli (`W:` has 37.9 GiB free, below the 50 GiB preflight). If the owner has freed `W:`, prove all three before Phase 6: on-host `consolidation.py capture --package <pkg>` of the base and the migrated trees (`py` on Windows), then `compare` with the package's manifest, feeding darwin + linux + windows captures together. Then `just cross-check <pkg> --os windows` and `--os wsl`.

    For Phase 6 (biscuit-terminal):
    - Re-capture the unmigrated package first (`capture --package biscuit-terminal --out <feature>/biscuit-terminal/capture-before`). `plan` refuses `selfproof/capture-a`. Sanity-`compare` it against `capture-a` (expect only override-rewrite notes). `.config/nextest.toml` override `test(=level2_render_tree_style_in_wezterm)` may belong to biscuit-terminal: `plan` reports it under `override_rewrites` if the move changes its identity, and the override string must then be rewritten (R5), as in Phases 3 and 4.
    - Guards that key on paths are the main hazard. In Phase 5, `spawn_site_guard` excluded Level 2 files by file-name prefix, and the R2 alias (`harness_integrity`) plus the new `level2/main.rs` would have silently joined its population. Before moving, grep the package's tests for `starts_with`, `file_name`, `read_dir`, and relative-path string literals, and prove each guard's population by path (temporary probe, restore, diff against `baseline/guard-scans.md`).
    - Also grep justfiles for paths into `tests/` (e.g. darkmatter's `lint-files` over-cap list), not just `--test` selectors.
    - Layout gate: copy `darkmatter/cli/tests/l1/test_layout.rs`, adjust its non-vacuity paths, and record it as a manifest `addition`. biscuit-terminal has test-toolkit only as an optional `terminal-tests` dependency, so the L1 gate needs a dev-dependency (update `docs/dependencies.md` if the dependency graph changes).
    - `pilot/apply-move.py <manifest> test_layout.rs` does the mechanical part. It does not add `mod test_layout;` to `l1/main.rs`, and it emits module declarations in manifest order, so sort them. `renderable/justfile` `drift-report` still has a biscuit-terminal branch (`--test render_comparison`). Switch it to `--test l1 render_comparison::` and drop the `case`. `examples/layout_matrix.rs` `#[path]`-includes `../tests/layout_matrix_support/mod.rs`, so keep that helper at `tests/`. biscuit-terminal's 1,086 `layout_matrix__*` snapshots belong to an `#[ignore]`d test (check-snapshots must say so).
    - Size row: build the before side into its OWN `--target-dir`. Phase 4's shared-target base worktree left a stale `zed-dmls-cli` build-script path that broke `just test` until `cargo clean -p zed-dmls-cli`.
    - Linux: the `build-linux` lock is still held by `feat-nightly-perf`. Use the private `--shared` clone, `git bundle create … 3efdbe450..HEAD`, and a `GIT_INDEX_FILE` working-tree patch (exclude this feature dir), with `RUSTC_WRAPPER="" KACHE_AUTO=0`. Delete `~/scratch/*` and `/tmp` uploads afterward.

    Left for Phase 7 (in addition to the earlier lists): the rust-testing SKILL.md:673 spawn-guard path, claudine's `l1/spawn_site_guard.rs` failure messages that still name `cli/tests/spawn_site_guard.rs`, and switching claudine's own layout-gate copy to `test_toolkit::test_layout`.
---
