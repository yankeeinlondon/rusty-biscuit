...
human_review_items:
    - |-
        **Windows and WSL2 proof for the `claudine-cli` pilot is blocked by a full disk on the Windows build machine.**

        The pilot moved `claudine-cli`'s 139 test programs into 4. On macOS and Linux I proved, test by test, that every test still exists, runs in the same test tier, and is skipped or ignored exactly as before. On Windows I could only prove that the code *compiles*, from macOS. The plan asks for the same test-by-test proof on a real Windows machine, and for a WSL2 check, before the other three packages are moved. Its reasoning: a Windows-only mistake found now costs one package, but found later it costs four.

        The Windows build machine (`build-win-native`) has 43 GB free on its `W:` drive. Every build recipe refuses to start below 50 GB. This repository's notes record that filling that drive once froze the machine. The WSL2 machine (`build-win`), whose disk lives on the same drive, currently drops every SSH connection, which the notes say is what a full `W:` looks like. I did not build there and did not delete anything. Freeing space is your call.

        - **Option A: free space on `W:` (at least ~60 GB free), then have the next agent finish the Windows and WSL2 proof before Phase 4 (recommended).** Pros: the pilot closes as the plan intended, and Phase 4 starts from a fully proven pattern. Cons: needs you to clear disk space, and adds roughly an hour of machine time.
        - **Option B: start Phase 4 now and finish Windows/WSL2 for both packages once space is free.** Pros: no waiting. Cons: if the pilot's layout has a Windows-only flaw, it would be copied into `darkmatter` before anyone sees it.
        - **Option C: accept compile-only evidence for Windows, and let the next ordinary CI run on `main` provide the real Windows run.** Pros: no machine work. Cons: CI runs Windows only after merge, so a problem is found after it lands. It also weakens the plan's own acceptance criterion 4 for this package.

        I recommend **Option A**. The macOS and Linux results are clean, so the Windows proof is probably a formality. But it is the one check that can catch a platform-gated test going missing on Windows, and it is cheap once disk space exists.
    - |-
        **Still open from Phase 2: confirm that the migration manifest is the one authoritative record of where each test moves.**

        The pilot now shows the manifest in use. `features/2026-09-21-consolidated-test-binaries/claudine-cli-migration.json` records, for all 139 old test programs, the new program and module each one landed in. Every before/after check read only that file, and all came back identical. The file carries two hand-recorded additions, each with a written reason: one false-positive warning that was cleared, and the 4 new layout-guard tests.
        ...
        - **Option B: also require a hand-reviewed table per package.** Pros: a person reads every row. Cons: 300+ rows, and a second record that can drift from the one the tools check.
        - **Option C: change the format first.** Pros: cheap to change now. Cons: delays Phase 4 and changes the toolkit.

        I recommend **Option A**. `pilot.md` shows the comparison catches mis-mappings on its own.
message_to_agent: |-
    Phase 3 (claudine-cli pilot) is implemented. Read `pilot.md` first; the log's "## Phase 3" section has the details. Status: macOS and Linux are fully proven (identity identical, suites green apart from 3 pre-existing Linux-only `sequence_groups` failures). Native-Windows listings and WSL2 are PENDING because `build-win-native`'s `W:` has 43 GB free (under the 50 GiB preflight) and `build-win` resets SSH. Plan task "Linux and native Windows" and the first Phase 3 checkpoint are deliberately unticked. If the owner has freed `W:`, finish them before Phase 4: on-host `consolidation.py capture --package claudine-cli` of the base and migrated trees (use `py`, not `python3`, on Windows; see the `os` skill), `compare` with `claudine-cli-migration.json` (feed the darwin+linux+windows captures together so platform-absent includes Windows), then `just cross-check claudine-cli --os windows` and `--os wsl`.

    Traps that cost time in Phase 3:
    - kache. `unset RUSTC_WRAPPER` is NOT enough. The macOS host's `~/.cargo/config.toml` sets `rustc-wrapper = "kache"`, and `build-linux`'s `/usr/local/bin/cargo` shim enables kache on any cold target dir, which breaks the next rebuild with "output file ... is not writeable". Export `RUSTC_WRAPPER=""` (explicitly empty). This is now in the `os` skill (`build-hosts.md` §Compiler cache).
    - The `build-linux` standing clone's cross-check lock has been held since 2026-09-14 by another branch. Use a private `--shared` clone plus a `git bundle` of `3efdbe450..HEAD` (the clone's HEAD is an ancestor), per `build-hosts.md`. Delete `~/scratch/*` afterwards.
    - Moving `mod common;` to `use crate::common;` produces `unused_imports` on the OS where every use is cfg-gated. Run `cargo check --tests --target x86_64-pc-windows-gnu` (with `RUSTC_WRAPPER="" CARGO_TARGET_DIR=target/windows-check CFLAGS/CXXFLAGS_x86_64_pc_windows_gnu=-Wa,-mbig-obj`) to catch these before any remote run.
    - `pilot/apply-move.py` is the manifest-driven mechanical mover (plain moves, never `git mv`, so nothing is staged). It fits packages that use `common/` + `mod common;`. darkmatter has 0 `mod common;`, so recreate the plan and read that package's helper layout first.
    - Nextest intersects a positional name filter with `-E`, so `just <recipe> <module>::` is the narrow-test form.

    Left for Phase 7: root-level active selectors (`prompts/_prompt.md:200`, `.claude/skills/claudine/completions/shell-completions.md:779`, `.claude/skills/rust-testing/SKILL.md:565`), and the `rust-testing` note that `_test`'s `cargo test` fallback (when nextest is missing) runs a migrated suite in one process.
---
