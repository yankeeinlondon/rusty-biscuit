...
implementation_2: "2026-09-21T09:15:39-07:00"
---
...
> **started at:** 2026-09-21T09:15:39-07:00

- this implementation is attempting to implement _all_ of the review findings found in '/Volumes/coding/wt/rusty-biscuit/feat-single-os/features/2026-09-19-direct-cell-execution/review-2.md'
- this is iteration 2 of the review-to-implement cycle
- starting the work on 'Reusing a hostable L2 cell makes the resolved plan invalid' at 09:16:37-07:00
        - GitNexus: the index was stale (indexed `8657947`, HEAD `ecb5df7`), so `just gitnexus` was run (126 s, up to date at `ecb5df7`) and impact was re-run on the fresh index: `mark_reused` upstream risk LOW (6 impacted, direct callers `apply_accepted_cells` and `package_cells.add`, 0 processes); `package_cells` upstream risk LOW (7 impacted, direct caller `calculate_scope`, 0 processes); no HIGH/CRITICAL/UNKNOWN result; a text search confirms `mark_reused` is named only in `scripts/ci/affected_scope.py`
        - discovered: the review's reproduction holds through the real validator on both paths, and a THIRD path has the same hole that the review did not name — a recorded execution constraint on an environment that hosts tmux turned the L2 cell to `omit`/`prohibited` after `backends` was attached, so `calculate_scope(..., prohibitions={"macos-latest": ...})` also emitted a plan `schema.validate_resolved_plan` refuses; the accepted-gap path was already safe (`backends` was attached only when `gap is None`)
        - before-fix evidence (new tests against the unfixed planner, `python3 -m unittest test_affected_scope.ApplyAcceptedCellsTests`): 9 run, 3 failures — `test_resolving_with_l2_evidence_reuses_a_hostable_cell_as_a_valid_plan`, `test_applying_l2_evidence_to_an_executing_hostable_cell_yields_a_valid_plan`, and `test_a_prohibited_hostable_l2_cell_is_a_valid_plan`, each with `AssertionError: Lists differ: [] != ['malformed-receipt: cell alpha-core/macos-latest/L2 carries required backends but is not an executing L2 cell']`
                - with only the `mark_reused` change applied, the two reuse tests pass and the prohibited one still fails with the same message; with the `package_cells` change applied and the `mark_reused` pop removed, `test_applying_l2_evidence_to_an_executing_hostable_cell_yields_a_valid_plan` fails with the same message — each half of the fix is pinned by a test that goes red without it
        - changed `scripts/ci/affected_scope.py`:
                - `mark_reused` now also pops `backends`, beside `profile` and `requires_node` (the centralized transition both `package_cells` and `apply_accepted_cells` reach); its comment names the third execution input
                - `package_cells.add` attaches `backends` after the execution decision and only when the cell still executes, in the same place `profile`/`requires_node` are attached, which closes the prohibited path; the moved comment was updated to say so. The schema was not loosened
        - changed `scripts/ci/test_affected_scope.py`: `ApplyFixture`'s `alpha-core` now declares `["tmux", "wezterm"]` (the review's shape: a hostable backend beside one no runner hosts; all 283 pre-existing tests stayed green with that change alone) and `ApplyAcceptedCellsTests` gained the three tests above; each asserts the real `schema.validate_resolved_plan` returns no problem, no `macos-latest`/`L2` dispatch row, no build consumer for that cell, the evidence retained on the reused cell, no `backends` on it, and that the untouched `ubuntu-latest` L2 cell still executes with `backends == ["tmux"]`; the apply test also asserts byte-identity with the from-scratch resolution
        - why the suites stayed green before: the only passing evidence in the fixtures was L1, and every L2 acceptance named an environment whose tmux capability is a governed gap, so no hostable L2 cell ever crossed `execute` to `reuse`
        - docs: `.github/ci/schemas/README.md` said only "A gap cell carries none"; now "A gap, reused, or prohibited cell carries none". `.github/ci/README.md`, `docs/topics/ci-cd.md`, and `.claude/skills/rust-devops/` speak only of executing L2 cells and remain correct, so they were left alone
        - other readers checked, none changed: `cell_contract.py` and `completion.py` read `backends` only off rows/cells that execute; `scripts/ci-rollup.rs` derives its own expectation and does not read the plan cell's field; the five `.githooks/tests/fixtures/plan-*.json` documents validate and carry no `backends` on a non-executing cell
        - verification (macOS host, this checkout): all 13 `scripts/ci/test_*.py` suites OK (affected_scope 286, build_key 12, ci_local 92, completion 59, constraints 39, cross_check 13, evidence_reuse 76, local_evidence 23, publish_gaps 19, resolved_plan 96, reuse_validation 21, runner_loss 50, schema 140); `just _test repo-deps` 445 passed / 1 skipped; `just _test test-toolkit` 233 passed / 2 skipped; `just _lint repo-deps` and `just _lint test-toolkit` clean (exit 0); `env -u CDPATH .githooks/tests/test-pre-push.sh` 67 passed / 0 failed (run although no hook fixture was touched)
        - GitNexus `detect-changes --scope all`: 6 files, 16 symbols, 0 affected processes, risk low, no partial/truncated flag; the planner symbols changed are exactly `mark_reused`, `package_cells`, and its nested `add`
        - OS considerations: pure stdlib Python dict handling with no paths, processes, or platform branches; no OS-specific surface, so no `just cross-check` was run
        - nothing blocked; nothing committed
- work completed for 'Reusing a hostable L2 cell makes the resolved plan invalid' at 09:28:15-07:00
- starting the work on 'The schema-version follow-up still describes the obsolete v5 to v6 migration' at 09:29:31-07:00
        - verified: `scripts/ci/schema.py:108` has `RESOLVED_PLAN_SCHEMA_VERSION = 6` (its docstring records version 5 as this feature and version 6 as the review-1 `backends` change), so retiring `execution_path` would now produce version 7; the `rust-devops` skill was loaded for the plan-schema context; documentation-only, so no symbol was edited and no GitNexus impact run applies
        - changed `spec.md:99` (`message_to_agent` item 4): "(plan schema v5 → v6)" is now "(plan schema v6 → v7)"; the pre-existing uncommitted `review_iterations: 1` → `2` hunk in the same file is not part of this work and was left alone
        - changed `acceptance.md:263` ("Known gaps and follow-ups"): "the plan schema from v5 to v6 for no behavior change" is now "the plan schema from v6 to v7 for no behavior change"; the rest of the sentence ("so it was left for an optional follow-up (Phase 8 log, "Decisions worth reading")") stays true as written — the reason is a bump with no behavior change, and it names no specific schema bump — so nothing else was reworded
        - sweep of the feature directory, `.github/ci/`, `docs/topics/ci-cd.md`, and `.claude/skills/rust-devops/` for `execution_path` near a v5/v6/v7 mention, every hit and its decision:
                - `spec.md:99` — current handoff, **fixed**
                - `acceptance.md:263` — current handoff, **fixed**
                - `implementation-log.md:1702-1703` (Phase 7, "Notes for Phase 8": "it is required at version 5") — phase history, **kept**
                - `implementation-log.md:1801-1802` (Phase 8, "Decisions worth reading": "required at plan schema v5") — phase history, and the passage `acceptance.md` cites, **kept**
                - `implementation-log.md:2011` (Phase 9, "Pre-existing items carried forward": "plan-schema v6 follow-up") — phase history, true when written, **kept**
                - `implementation-log.md:2091` ("Implementation of Review Findings #1": "other v6 follow-ups ... were not folded into this schema bump") — the wording review-2 points at, but that section is a closed cycle's record and out of scope by instruction, **kept and flagged to the orchestrator**
                - `implementation-log.md:1640` (Phase 7 mapping row: "v4 and v6 refused by name, v5 accepted") — phase history of a test's behavior then, **kept**
                - `docs/topics/ci-cd.md:232-233` ("Plan schema version 5 stamps every area `execution_path: "rows"` ... Version 6 gives every executing L2 cell `backends`") — current-state and correct: it says which version introduced each field and states no follow-up version pair, **kept**
                - `review-2.md:7,87-96` — the finding itself, **kept**; `rulings.md:195` and `plan.md:500` matched only on `schema_version` of the never-created `direct-execution.json`, unrelated, **kept**
                - `.github/ci/README.md:947`, `.github/ci/schemas/README.md:93,131`, and `.claude/skills/rust-devops/ci-cd.md:126` name `execution_path` with no version pair nearby — no hit, nothing to change
                - `spikes/plans/*.json` (`schema_version: 5`) — historical evidence, excluded from the sweep and untouched
        - verification: `git diff` of the two files shows exactly one changed line each from this work (plus the pre-existing `review_iterations` hunk in `spec.md`); the `spec.md` frontmatter parses with PyYAML 6.0.3 `yaml.safe_load` (a dict of 15 keys, `message_to_agent` carrying the v6 → v7 line); line width and block-scalar indentation unchanged
        - test pins: grep of `scripts/ci/test_*.py`, `tools/test-toolkit/tests/`, and `scripts/*.rs` for `v5 → v6`, `v5 to v6`, `v6 → v7`, `v6 to v7`, and `acceptance.md` returned nothing; the only feature-directory path a test reads is `spikes/plans/` (`test_affected_scope.py` `CORPUS_PLANS`), untouched, and `the_ci_documentation_states_the_implemented_behavior` reads only `docs/topics/ci-cd.md` and skill files, also untouched — so no Python or Rust suite was run for this finding
        - OS considerations: prose-only edit to two Markdown files; none
        - nothing blocked; nothing committed
- work completed for 'The schema-version follow-up still describes the obsolete v5 to v6 migration' at 09:30:26-07:00
- orchestrator re-verification at 09:35:56-07:00 (independent of the subagent runs):
        - all 13 `scripts/ci/test_*.py` suites OK (affected_scope 286, build_key 12, ci_local 92, completion 59, constraints 39, cross_check 13, evidence_reuse 76, local_evidence 23, publish_gaps 19, resolved_plan 96, reuse_validation 21, runner_loss 50, schema 140)
        - `just _test repo-deps`: 445 passed, 1 skipped; `just _test test-toolkit`: 233 passed, 2 skipped
        - `just _lint repo-deps` and `just _lint test-toolkit`: clean
        - reviewed the planner diff by hand: `mark_reused` pops `backends` beside `profile` and `requires_node`, and `package_cells.add` attaches `backends` only once the cell is known to execute; the schema was not loosened
        - one side effect worth knowing: `backends` is now inserted after `profile`/`requires_node` on an executing L2 cell, so the key order of the compact plan JSON differs from the previous build. Every suite that pins plan content (hook fixtures, `test_resolved_plan`, `test_evidence_reuse`) stayed green, so nothing depends on that order
        - no `just cross-check` run was made: the change is stdlib Python dict handling plus documentation text, with no paths, processes, or platform branches

### Successful Completion

The implementation of review cycle 2 has completed successfully in 21 minutes (09:15:39 to 09:36). During this implementation all 2 review findings were evaluated to see if they could be fixed as a part of this implementation cycle: 2 were fixed, 0 were deferred.

- 'Reusing a hostable L2 cell makes the resolved plan invalid' — **fixed**: `mark_reused` now removes `backends` in the same centralized transition that removes `profile` and `requires_node`, which covers both the fresh-resolution path and `apply_accepted_cells`. A third path the review did not name had the same hole — a recorded execution constraint turned a hostable L2 cell to `omit`/`prohibited` with `backends` still attached — and is closed by attaching `backends` in `package_cells.add` only after the execution decision. Three Level 1 regressions in `ApplyAcceptedCellsTests` use a hostable tmux cell and prove the plan validates against the real schema validator, carries no L2 dispatch row or build consumer for the cell, retains the evidence, and has no `backends` on the non-executing cell; all three failed against the unfixed planner with the review's `malformed-receipt` message.
- 'The schema-version follow-up still describes the obsolete v5 to v6 migration' — **fixed**: the `spec.md` `message_to_agent` handoff (item 4) and the `acceptance.md` "Known gaps and follow-ups" entry now say v6 → v7. Phase-by-phase history was left unchanged.

The files changed by this cycle are: `scripts/ci/affected_scope.py`, `scripts/ci/test_affected_scope.py`, `.github/ci/schemas/README.md`, `features/2026-09-19-direct-cell-execution/spec.md`, `features/2026-09-19-direct-cell-execution/acceptance.md`, and this log. Nothing was committed.

Items surfaced but intentionally left alone: the "Implementation of Review Findings #1" closing paragraph above still calls the `execution_path` retirement one of the "v6 follow-ups". It is a record of what that cycle wrote at the time, so it was kept as history per the review's instruction; the current handoff text in `spec.md` and `acceptance.md` is the authoritative statement (v6 → v7).
