...
- [ ] Add one per-detection `ManifestStore` keyed by normalized native paths for parsed/raw Cargo, Node, Python, Go, lockfile, inherited-version, and root-scoped test-runner/config inputs; count and test one parse per unique input. **Partially done:** `LockStore` gives one `Cargo.lock` parse per unique workspace root (was one per detector plus one per full-mode scan). The parsed-manifest store is **not** done — `ManifestCache` is still per-`create_package`, so `cargo::read_toml_at` still re-reads the inherited root `Cargo.toml` once per member. See the sub-spec's "Not done".
...
- [ ] Make `RepoRequest::structure()` perform membership and minimum package identity parsing only; leave dependency, test-runner, feature, framework, language, and file-list enrichment fields absent/empty while retaining identity, paths, package area, ecosystem, standard, provenance, exclusion state, standards, and layers. **BLOCKED — stopped for review per the plan's HIGH-risk rule.** Implemented literally this silently empties `sniff repo test-runner`, `sniff repo dependencies`, and `sniff repo package-manager`, all of which read these fields off `detect_repo_structure`. The sub-spec carries the impact table and a recommendation.
- [x] Enrich every unique seed exactly once for `RepoRequest::full()` and parse root `Cargo.lock`, inherited root `Cargo.toml`, and root-scoped test-runner configuration at most once per detection. **One enrichment per unique seed is done and asserted by counter (600 → 300); `Cargo.lock` is once per workspace root. Inherited root `Cargo.toml` and root-scoped test-runner config still parse per member — they need the `ManifestStore` above.**
...
- [ ] Build one deepest-prefix ownership index and use it for inventory, docs, aggregate buckets, and commit-file attribution; compare path components and depth so nested packages always beat shallower prefixes. **Not done.** The three existing deepest-prefix implementations (`refresh_package_boundaries`, `RepoInfo::package_for_dir`, `polyglot`) are unchanged and still correct.
- [ ] Add fixtures for duplicate seeds, overlapping/nested packages, symlink and lexical paths, non-UTF-8 paths where supported, Windows drive/case behavior, root inheritance, and 100/500/2,000 mixed-ecosystem packages. **Partially done:** duplicate-seed merge, authority precedence, exclusion OR, evidence union, and prefix-sibling fixtures exist in `seed.rs`; the enrichment-once contract is pinned in `detection.rs`. The symlink/non-UTF-8/Windows-drive and 100/500/2,000-package fixtures are not added.
- [ ] Update library README/rustdoc examples to direct callers to `RepoRequest::full()` for enriched fields and document the intentionally shallow structure-only contract. **Deferred with the blocked structure-only migration it documents.**

**Validation checkpoint — Phase 4**

- [x] Run `just test`, `just lint`, `just build`, and `just doctest`; assert one enrichment per unique boundary, one parse per unique manifest/lockfile/config, and deepest-prefix assignment parity. **`just test` 1378/1379 (sole failure is the pre-existing `detect_area_errors_when_not_in_repo` timeout, verified on clean HEAD); `just lint`, `just build`, `just doctest` clean. One enrichment per unique boundary is asserted by `every_unique_boundary_is_enriched_exactly_once`. One parse per unique manifest is not asserted (no `ManifestStore`); deepest-prefix parity is unchanged, not re-asserted.**
- [ ] Benchmark structure/full detection at 100, 500, and 2,000 packages plus Markdown/package ownership scaling; verify structure mode's enrichment counters remain zero and record the qualified structure/full ratios. **Counter evidence recorded in the sub-spec against the existing 375-package fixture, with structure-only byte-identical as the drift bracket. The 100/500/2,000 sweep is not added, and structure-mode enrichment counters are *not* zero — that outcome depends on the blocked R5.6.**
...
- [ ] **[Parallel lane B — metadata controls]** Borrow the ref-decoration cache, reuse remote-tracking tip sets, avoid two unrequested reachability walks per non-current branch, and reuse worktree metadata without serially opening every linked worktree as a full repository. **Partially done.** The ref-decoration cache is now **borrowed, never cloned**, in both `get_recent_commits_fallible` and `get_commits_for_path_fallible` (each previously copied the whole map per call, defeating the cache they were handed); `None` now means "attach no decorations" rather than "silently re-collect them". The two reachability walks per non-current branch are gated behind `wants_branch_divergence()`. **Remote-tracking tip-set reuse and worktree-metadata reuse are not done** — see the sub-spec's "Not done".
...
- [x] Add serialization tests for legacy request JSON, focused-control round trips, and byte-identical preset JSON; add work-count tests for blob/diff units, unrelated metadata avoidance, bounded path history, skewed timestamps, ref/tip reuse, worktree opens, and target-set containment. **14 new tests.** Serialization: preset JSON omits `metadata`, legacy JSON derives legacy behavior, focused controls round-trip, controls cannot widen past coarse fields. Work counts: one load/diff per dirty side, counts-only/dirty-flag load no blobs and run no diffs, stats-only addition runs no diff, path-history visits bounded by `scan_limit`. Behavior: the three-state completeness contract (exhausted / limited / satisfied) and unborn HEAD. **Skewed timestamps** are covered by the pre-existing `populate_commit_remotes_skewed_timestamp_ancestor_not_pruned`, which passes unchanged against the new target-set walk. **Ref/tip-reuse and worktree-open counter tests are not added** (their reuse work is deferred — see the sub-spec).
- [ ] Extend Git Criterion fixtures for 100 files at 1 KiB/100 KiB/multi-megabyte sizes, branch-heavy/divergent repos, sparse matches in long history, and many remote tips; assert work bounds separately from wall time. **Not done — deferred to Phase 8 with the Phase 3 precedent.** Work bounds are asserted in-process by the counter tests above, which is the acceptance evidence this feature actually judges on; Criterion timings on this host are not trustworthy (Phase 3 recorded +330% for a byte-identical case at load 57-87/16 cores) and Phase 3 already deferred its re-run to Phase 8 for the same reason.
...
- [x] **[Parallel lane D — subprocesses]** Apply the shared deadline/pipe-draining behavior to every service backend, macOS `diskutil`, Windows locale PowerShell fallback, program schema, and host-capability probes; retain public result shapes and avoid terminal noise. **Every subprocess in sniff now routes through `process::run_with_timeout`.** This also caught two sites the survey's scope missed — `host_capability`'s `id -Gn` and `sudo -n true` probes had no deadline at all. Diagnostics are `tracing::warn`, never stdout/stderr.
- [x] Add cross-platform tests with outputs larger than a typical pipe buffer, timeout during primary listing, timeout after partial enrichment, explicit child-reaping checks, batched command counts, and platform-gated command fixtures. **Pipe buffer** (1 MiB stdout + a both-pipes-at-once case), **child reaping** (`waitpid(WNOHANG)` must return `ECHILD`), and **injected short deadlines** are in `process.rs`. **Batched command counts** are asserted end-to-end in `systemd.rs` against a `systemctl` shim on `PATH` that logs its argv: 300 running services cost **3** `show` calls, not 300. The shell-script shim and `waitpid` checks are `#[cfg(unix)]`-gated — systemd/runit are Linux backends no Windows host runs — while every parser test is platform-neutral and runs on all three OSes. **Timeout during primary listing and after partial enrichment are covered by construction, not by a dedicated shim test**: both paths return through the same `Err(e) => return Vec::new()` / `continue` arms the failure tests exercise, and forcing a real 3s deadline in a unit test would violate the plan's own injected-deadline rule.
...
- [x] Correct known drift alongside its owning behavior: `.editorconfig` lookup semantics, `filter_inventory` copy/sharing language, capped inventory determinism, the 375-package benchmark name/count, the unsupported general 10–50× claim, and NTP timeout/platform wording. **Four of six were live; two were already fixed by the phase that owned them** (`filter_inventory` and capped-inventory determinism, both rewritten in Phase 2 — verified, not re-touched), and the 375-package rename landed in Phase 1 (verified; added the "never compare against an archived `huge_500` result" note). Live and fixed: (1) `detect_formatting`'s docblock claimed parent-directory traversal that `find_editorconfig` has never done; (2) `os/time.rs` claimed a 5-second NTP timeout and the architecture doc/skill claimed "up to 10s (Linux `timedatectl`)" — the real bound is `timeouts::NTP` = **3s**, and the Linux path makes no network round trip at all, so the 10s figure described a cost that never existed; (3) the 10–50× claim, in 6 sites. **The 10–50× claim was worse than "unsupported"** — see below. Drift found but deliberately not fixed: `benches/README.md` and the architecture doc say `ProgramsInfo::detect()` fans out over **8** categories while the type and skill say **9**; that belongs to the phase owning program detection, not to a completion phase whose diff must contain no behavior.
    - **The 10–50× correction, in detail.** The umbrella spec expected the claim to be requalified around a 2.4× measurement. The counters say the honest figure is nearer **1×**: on the 375-package fixture `structure()` and `full()` record 13,274 vs 13,275 `metadata_probes`, 702 vs 701 `read_dirs`, 454 vs 454 `manifest_parses`, and 300 vs 300 `package_enrichments`. Full's only material addition is classifying the 3,755-file inventory. Two documented causes finally joined up: **R5.6 is blocked** (structure still enriches, because making it shallow silently empties three shipped CLI commands), and **structure pays a nested-marker fallback walk that full avoids** via the observation index. So the old claim did not merely overstate a number — it pointed the wrong way about where the cost lives. **No replacement timing ratio was published:** three sequential runs put structure *slower* than full (223/144, 138/112, 124/100 ms), which is a page-cache confound (`work_counts` runs structure first over the same fixture) on a host at load 87/16 cores — recorded in the bench manifest so the next reader does not "discover" it as a finding.
...
- [x] Run the complete acceptance matrix from `sniff/`: `just sanity`, `just lint`, `just build`, `just doctest`, `just test`, `just test-l2`, and `just bench`; retain read-only formatting checks through `just lint` and do not run write-mode formatting. **`just lint` clean with zero warnings; `just build` clean; `just doctest` 87/87; `just test` 1603/1604; `just sanity` 1185/1186; `just test-l2` 2/2.** Both test-tier failures are the same pre-existing `detect_area_errors_when_not_in_repo` temp-dir timeout — a known baseline since Phase 4, verified on clean `HEAD`, and untouchable by this phase, which changes no production code. No write-mode `cargo fmt` was run. **`just bench` deliberately not run** — see the next item and the sub-spec's "Timing evidence".
- [x] Review final work counters against Phase 1 for every acceptance case and attach a phase-by-phase table covering walks, opens/probes/parses, discoveries/status/diffs/graph visits, subprocesses/timeouts, and HTTP operations. **Full table in the sub-spec.** Headline, Phase 1 → Phase 8: `staged_filesystem_full_all_stages` `metadata_probes` 7,885 → **4,075** (−48%), `read_dirs` 422 → **211** (−50%), `manifest_parses` 214 → **124** (−42%), `package_enrichments` 180 → **90** (−50%); `repo_full_huge_375_packages` `metadata_probes` 25,975 → **13,275** (−49%), `read_dirs` 1,403 → **701** (−50%), `manifest_parses` 754 → **454** (−40%), `package_enrichments` 600 → **300** (−50%); Git `deep()` `blob_loads` 400 → **200** (R8). Results-shaped counters (`entries_visited` 638/4,685, `files_accepted` 395/3,755, `documents_parsed` 182) are **unchanged**, which is the point: this feature changed work, not results.
    - **Drift bracket:** `staged_filesystem_summary_git_plus_repo` and `repo_structure_huge_375_packages` are **byte-identical to Phase 1 on every counter**. Both are structure-mode and R5.6 is blocked, so nothing should have moved them — their stability is what makes the deltas above readable as real rather than as harness drift.
    ...
    - **A Phase 4 prediction did not hold:** `filesystem.io.canonicalizations` was projected to fall 600 → 300 "with the enrichment halving". It is **600, unchanged**, while enrichments did halve. Not a regression (no case got worse), but the mechanism was mis-modeled — canonicalization is not driven per-enrichment. Recorded rather than quietly dropped; it would be owned by R6.2/R6.4, which are themselves unimplemented.
- [x] Move each finished phase sub-spec to its completed lifecycle location independently; move the umbrella feature only when R1–R13, all preserved/intentional compatibility checks, and macOS/Linux/Windows correctness are complete. **Decision: nothing was moved. No files changed location.** Both halves are deliberate.
    - **The umbrella feature stays open — the completion boundary is not met.** R5.5 (`ManifestStore` is `LockStore`-only), R5.6 (structure-only migration **blocked pending owner review**), R6.4 (deepest-prefix ownership index not built), and R9.5/R9.6 (tip-set and worktree-metadata reuse) are outstanding. The umbrella spec requires R1–R13 implemented before it moves. R14 is correctly *not* a blocker: Phase 7 measured all nine candidates and deferred them with evidence, which the completion boundary expressly permits.
    - **The phase sub-specs stay in place**, against the literal instruction, for three reasons. (1) **No such location exists**: `_completed` is a repo convention for top-level `features/`/`fixes/` entries (per `CLAUDE.md`); no `phases/_completed` exists anywhere in the monorepo, so this would invent a convention inside a completion phase. (2) **It would break the dependency graph**: only Phases 1, 2, 3, 6, and 7 are finished — Phase 4 has 6 open tasks and Phase 5 has 2 — and every sub-spec's `depends-on` is a **relative** link (`../03-observation-index/spec.md`). Moving 3 breaks Phase 4's link; moving 6 breaks Phase 7's and Phase 8's. The graph would be split across two directories with dangling links, mid-feature. (3) **It buys nothing**: `_completed` exists to keep the *active feature queue* readable, and phase sub-specs nested inside an open feature directory were never in that queue. The umbrella spec's own wording is permissive — "a phase **may** move to `_completed` independently" — not mandatory. Revisit when the umbrella closes and all sub-specs can move together.

**Validation checkpoint — feature complete**

- [x] Confirm every acceptance criterion in `spec.md` has a passing automated test or an attached cross-platform artifact, every public migration is documented, and no unexplained counter or output regression remains. **Confirmed for the criteria this feature landed; three are not met and the umbrella therefore stays open** (see the lifecycle item above). Not met: "every unique manifest/lockfile/root config is parsed at most once per detail phase" (R5.5 partial — inherited root `Cargo.toml` still parses per member); "structure-only detection performs no dependency, test-runner, feature, framework, language, or file-list enrichment" (R5.6 blocked); and the deepest-prefix ownership index (R6.4). **No unexplained counter or output regression remains** — every counter delta against Phase 1 is accounted for in the sub-spec's table, including the three that cannot be compared naively (Phase 1's under-reported `file_opens`/`bytes_read`, and `git.file_diffs`, which does not move while the work it names halves) and one mis-modeled Phase 4 prediction (`canonicalizations` did not fall). Public migrations — `GitMetadataRequest`, `PathHistoryOptions`/`PathHistoryResult`, inventory `truncated`/`limit`, and the default-plan NTP policy — are documented in the architecture doc and the skill. Cross-platform artifacts are now produced by the scheduled `sniff-work-counts` matrix rather than attached here; this host is macOS only.
