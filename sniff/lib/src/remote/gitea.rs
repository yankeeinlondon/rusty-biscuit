...
impl RemoteRepoProvider for GiteaRemote {
    ...
    async fn get_repo_metadata(&self, owner: &str, repo: &str) -> Result<RepoMetadata, SniffError> {
        ...
        let html_url = info.html_url.unwrap_or_else(|| {
            format!("{}/{}/{}", self.web_url(), owner, repo)
        });
    ...
    async fn list_pull_requests(
        ...
        Ok(prs
            ...
            .filter_map(|pr| {
                ...
                let author = pr.user.and_then(|u| u.login_name().map(|s| s.to_string()))
    ...
    async fn list_issues(&self, owner: &str, repo: &str) -> Result<Vec<IssueInfo>, SniffError> {
        ...
        Ok(issues
            ...
            .filter_map(|issue| {
                ...
                let author = issue.user.and_then(|u| u.login_name().map(|s| s.to_string()))
                    .unwrap_or_else(|| "unknown".to_string());
    ...
    async fn detect_cicd(&self, owner: &str, repo: &str) -> Result<Option<CiCdInfo>, SniffError> {
        ...
        let path_equals = |entry: &GitTreeEntry, name: &str| {
            entry.path.as_deref() == Some(name)
        };
