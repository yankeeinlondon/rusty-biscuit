...
/// The recent-commits primitives are public through both re-export levels and
/// through `path_kind`, the path downstream crates already import from.
mod recent_commits_public_surface {
    use chrono::{Duration, FixedOffset};
    use sniff::filesystem::git as git_level;
    use sniff::filesystem::path_kind::{ChangeCategory, classify_path};
    use std::path::Path;

    #[test]
    fn options_and_payload_types_are_reachable_from_both_levels() {
        let from_git = git_level::RecentCommitsOptions::new()
            .selection(git_level::Selection::parse("yesterday").unwrap())
            .named_date(git_level::NamedDate::Today)
            .verbosity(git_level::RecentCommitsVerbosity::Verbose)
            .projection(git_level::RecentCommitsProjection::SourceCode)
            .has_file_type(ChangeCategory::Cicd)
            .timezone(FixedOffset::east_opt(0).unwrap());
        let from_filesystem: sniff::filesystem::RecentCommitsOptions =
            sniff::filesystem::RecentCommitsOptions::new()
                .selection(sniff::filesystem::Selection::Duration(Duration::days(1)))
                .named_date(sniff::filesystem::NamedDate::Today)
                .verbosity(sniff::filesystem::RecentCommitsVerbosity::Verbose)
                .projection(sniff::filesystem::RecentCommitsProjection::SourceCode)
                .has_file_type(ChangeCategory::Cicd)
                .timezone(FixedOffset::east_opt(0).unwrap());
        assert_eq!(from_git, from_filesystem);
        assert_eq!(sniff::filesystem::DEFAULT_RECENT_COMMIT_COUNT, 10);

        let empty: sniff::filesystem::RecentCommits = git_level::RecentCommits::default();
        assert_eq!(empty.to_json(), serde_json::json!([]));
        let _: fn(&[sniff::filesystem::RecentCommitFile]) -> git_level::RecentCommitFileTypes =
            sniff::filesystem::RecentCommitFileTypes::from_files;
        let _: Option<(
            git_level::RecentCommit,
            sniff::filesystem::RecentCommitAuthor,
            git_level::RecentCommitPackages,
            sniff::filesystem::RecentCommitFileKind,
        )> = None;
    }

    #[test]
    fn classifier_is_public_with_serialized_category_names() {
        assert_eq!(
            classify_path(Path::new(".github/workflows/ci.yml")),
            ChangeCategory::Cicd
        );
        assert_eq!(
            serde_json::to_value(ChangeCategory::WebAssets).unwrap(),
            serde_json::json!("web_assets")
        );
    }
}
