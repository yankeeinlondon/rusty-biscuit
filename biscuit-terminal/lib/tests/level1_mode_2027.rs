...
#[test]
fn supports_mode_2027_returns_true_in_kitty_pty() {
    let mut session = spawn_with_env(&[
        ("PROBE", "mode_2027_support"),
    ...
    assert!(
        output.contains("supports_mode_2027=true"),
        "expected supports_mode_2027=true for kitty, got: {output}"
    );
}

#[test]
fn supports_mode_2027_returns_false_in_apple_terminal_pty() {
    let mut session = spawn_with_env(&[
        ("PROBE", "mode_2027_support"),
    ...
    assert!(
        output.contains("supports_mode_2027=false"),
        "expected supports_mode_2027=false for Apple Terminal, got: {output}"
    );
}
