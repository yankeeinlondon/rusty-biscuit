...
fn atomic_token_to_escape(token: &str) -> Option<&'static str> {
    match token.to_lowercase().as_str() {
        ...
        "double-underline" => Some("\x1b[4:2m"),
        "curly-underline" => Some("\x1b[4:3m"),
        "dotted-underline" => Some("\x1b[4:4m"),
        "dashed-underline" => Some("\x1b[4:5m"),
        "blink" => Some("\x1b[5m"),
        "reverse" => Some("\x1b[7m"),
        "hidden" => Some("\x1b[8m"),
...
fn lookup_web_color(name: &str) -> Option<crate::utils::color::RgbColor> {
    // Convert kebab-case to the expected format for lookup
    let normalized = name.to_lowercase().replace('-', "");

    let web_color = match normalized.as_str() {
    ...
    web_color.and_then(|wc| WEB_COLOR_LOOKUP.get(&wc).copied())
}
...
fn lookup_tailwind_color(name: &str) -> Option<(u8, u8, u8)> {
    // Parse Tailwind color names like "red-500", "slate-950", etc.
    let tailwind = match name.to_lowercase().as_str() {
        // Special values (return None for these)
        "inherit" | "current" | "transparent" => return None,
