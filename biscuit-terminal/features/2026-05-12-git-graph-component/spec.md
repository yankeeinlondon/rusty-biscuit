...
/// Pre-collected git state consumed by [`GitGraph::auto`].
///
/// This is a plain input struct — no methods, no validation. The caller
/// assembles it from whatever source they have (git2, the system `git`
/// binary, mocked data in tests) and passes it in. The auto-builder applies
/// scenario selection, enforces caps, and emits the resolved Mermaid.
#[derive(Debug, Clone)]
pub struct GitGraphAuto {
    /// Explicit scenario choice. `None` infers from
    /// `current_branch == Some(default_branch)`.
    pub kind_hint: Option<GitGraphKind>,

    /// Branch HEAD is on. `None` for detached HEAD; auto returns `None`.
    pub current_branch: Option<String>,

    /// Repository default branch (e.g. `"main"`).
    pub default_branch: String,

    /// Recent commits on `default_branch`, oldest first. The auto-builder
    /// enforces caps; callers may pass more than `caps.*` allows and trust
    /// the trim.
    pub mainline: Vec<GitGraphCommit>,

    /// Branches to draw. Focused mode expects exactly one entry (the
    /// current branch); overview mode expects one per active worktree
    /// excluding the default branch.
    pub branches: Vec<GitGraphBranch>,

    /// Merge-base of `current_branch` against `default_branch`. Used only
    /// in focused mode.
    pub merge_base: Option<GitGraphCommit>,

    /// Caps. Defaults match the worktree CLI's historic numbers.
    pub caps: GitGraphCaps,

    /// When `true` AND the scenario resolves to `FocusedBranch`, the
    /// returned `GitGraph` carries populated [`VerboseDetail`]. Silently
    /// ignored in other scenarios.
    pub verbose: bool,
}
```

#### Notes on `GitGraphCommit::timestamp`

`timestamp` is `DateTime<Local>` because the primary consumer is terminal
rendering, where wall-clock-local times match user expectations and match
what `worktree` already produces today. When `darkmatter`'s HTML integration
lands, server-rendered verbose-detail timestamps will reflect the *server's*
timezone, not the viewer's. Revisit if that becomes a problem.
...
```rust
impl GitGraph {
    pub fn auto(input: GitGraphAuto) -> Option<GitGraph>;
}
...
// graph: Option<GitGraph> — None when the input is insufficient
// (e.g. detached HEAD, no branches in overview mode, no mainline).
```

`auto()` is infallible — it returns `Option<GitGraph>` rather than `Result`,
since the only "failure" mode is "the supplied data does not describe a
renderable graph", which the caller handles by skipping rendering.

**Cap enforcement.** `auto()` ALWAYS enforces `caps` itself. Callers may pass
over-cap input (e.g. 50 mainline commits when `overview_default_commits = 10`)
and trust the trim. There is no separate "raw vs. trimmed" entry point.

**Verbose suppression.** `verbose: true` is honored only when the resolved
scenario is `FocusedBranch`. In every other scenario it is silently ignored
and no `VerboseDetail` is populated on the returned `GitGraph`. Callers that
want unconditional verbose output must construct `VerboseDetail` themselves
...
`GitGraphCollectError` is defined alongside the collector module:

```rust
#[cfg(feature = "git")]
#[derive(Debug, thiserror::Error)]
pub enum GitGraphCollectError {
    ...
    #[error("failed to resolve ref `{rev}`")]
    RefResolutionFailed {
        rev: String,
        #[source]
        source: std::io::Error,
...
The type lives in `components::git_graph::collect` and is re-exported from
`components::git_graph` under `#[cfg(feature = "git")]`. `thiserror` is
already a workspace-wide dependency, so the `git` feature still incurs no
new compile-time deps.

The feature only exists so that callers who _want_ a one-liner can have
...
The `biscuit-terminal` CLI hard-wires the `git` feature on its library
dependency:

```toml
# biscuit-terminal/cli/Cargo.toml
[dependencies]
biscuit-terminal = { path = "../lib", features = ["clap", "git"] }
```

The `git-graph` subcommand is **unconditional** — there is no `#[cfg]`
gating in the CLI. The CLI builds always include the collector, and the
library's `git` feature remains opt-in (`default = []`) only for *library*
callers who want to pay zero cost.

The subcommand (e.g. `biscuit-terminal git-graph [PATH]`):
...
Downstream packagers who want a slim CLI build without git support must
fork or maintain their own `Cargo.toml` — the upstream CLI does not
provide a no-git build mode. This is a deliberate simplicity choice;
graph rendering is a headline feature of the CLI.
...
_when_ a graph should appear (focused vs. overview, narrow-terminal
...
- Verbose detail is opt-in via `GitGraphAuto::verbose = true` and is only
  populated by the auto builder when the focused-branch scenario applies.
  Callers using the typed builder may attach `VerboseDetail` directly.
...
If `verbose: true` is set but the resolved scenario is `BaseOverview` (or
`Manual`), the flag is **silently ignored** and `verbose_detail` stays
`None`. There is no warning, no error — the suppression is part of the
documented contract. Callers that want unconditional verbose output must
populate `VerboseDetail` via the typed builder.
...
- caller passes no branches in base-overview auto mode: returns `None` from
  `GitGraph::auto`, signalling the caller to skip rendering;
- caller passes branches but no mainline: returns `None`;
- caller passes `current_branch: None` (detached HEAD): returns `None`.
...
- Auto builder:
    ...
    - returns `None` when input data is insufficient (detached HEAD, no
      branches in overview mode, no mainline)
    - over-cap input is trimmed to `caps.*` rather than rejected
    - `verbose: true` with `FocusedBranch` populates `VerboseDetail`
    - `verbose: true` with `BaseOverview` leaves `verbose_detail = None`
...
- A `biscuit-terminal` CLI smoke test exercises the new `git-graph`
  subcommand against a temporary repository. The CLI hard-wires the
  library's `git` feature, so this test is unconditional.
...
10. Hard-wire `features = ["clap", "git"]` on the `biscuit-terminal` lib
    dependency in `biscuit-terminal/cli/Cargo.toml` and add the
    unconditional `git-graph` subcommand.
11. Update READMEs.
...
- `GitGraph::auto` is **infallible** with signature
  `fn auto(input: GitGraphAuto) -> Option<GitGraph>`. `GitGraphAuto` is a
  plain public input struct (no methods, no validation). `auto()` always
  enforces `caps` itself — callers may pass over-cap input and trust the
  trim. `GitGraphAuto::verbose = true` is honored only when the resolved
  scenario is `FocusedBranch`; in every other scenario it is silently
  ignored.
- `GitGraphCommit::timestamp` is `chrono::DateTime<chrono::Local>`. This is
  pragmatic for the primary terminal-rendering use case and matches what
  `worktree` produces today. The browser/server-rendered HTML path will
  show the server's timezone; revisit if that becomes a problem.
- `GitGraphCollectError` is defined under
  `components::git_graph::collect` (re-exported from `components::git_graph`
  under `#[cfg(feature = "git")]`) and uses `thiserror`, which is already a
  workspace-wide dependency.
- The `biscuit-terminal` CLI hard-wires `features = ["clap", "git"]` on its
  lib dependency. The `git-graph` subcommand is unconditional in the CLI;
  there is no no-git build mode upstream. The library's `git` feature
  remains `default = []` for library callers.
