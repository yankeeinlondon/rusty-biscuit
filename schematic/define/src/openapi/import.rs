...
impl OpenApiImport {
    ...
    pub fn build(self) -> Result<OpenApiImportResult, OpenApiError> {
        // Parse the OpenAPI document from source
        ...
        // Collect diagnostics during import
    ...
    fn parse_source(&self) -> Result<openapiv3::OpenAPI, OpenApiError> {
        match &self.source {
            OpenApiSource::Path(path) => {
                let content = std::fs::read_to_string(path).map_err(|e| OpenApiError::Io {
                    path: path.clone(),
                    message: e.to_string(),
                })?;
            ...
            OpenApiSource::Json(content) => {
                serde_json::from_str(content).map_err(|e| OpenApiError::Parse {
                    message: e.to_string(),
    ...
    fn validate_constraints(&self, doc: &openapiv3::OpenAPI) -> Result<(), OpenApiError> {
    ...
    fn map_to_rest_api(
        ...
        // Map security schemes to AuthStrategy
        let auth = self.map_auth_strategy(doc, diagnostics);

        // Map all operations to endpoints
    ...
    /// Maps security schemes to AuthStrategy.
    fn map_auth_strategy(
        &self,
        doc: &openapiv3::OpenAPI,
        diagnostics: &mut Vec<OpenApiDiagnostic>,
    ) -> AuthStrategy {
        // Look for security schemes in components
...
"#;
        ...
        // API name is sanitized to valid Rust identifier (PascalCase, no spaces)
...
"#;
    ...
    fn build_returns_error_for_invalid_yaml() {
        ...
        assert!(matches!(result.unwrap_err(), OpenApiError::Parse { .. }));
    }
...
"#;
        ...
        assert!(matches!(result.api.auth, AuthStrategy::BearerToken { .. }));
    }
...
"#;
        ...
        assert!(matches!(result.api.auth, AuthStrategy::Basic));
    }
...
"#;
        let source = OpenApiSource::yaml(yaml);
        // This should generate diagnostics (no operationId)
        let result = OpenApiImport::new(source).strict().build();

        // Strict mode should fail if there are any diagnostics
        // (missing operationId generates a warning)
...
"#;
    ...
    fn build_validates_component_count() {
        // This test verifies the constraint is checked (we won't actually
        // create 5000+ components, just verify the mechanism works)
        let yaml = r#"
openapi: "3.0.0"
info:
  title: Test
...
"#;
        ...
        // Should succeed with few components
        assert!(result.is_ok());
    }
...
"#;
        ...
        // Should have both Pet and PetList in models
        assert!(!result.models.types.is_empty());
    }
...
"#;
        ...
        // Should have a diagnostic about missing operationId
        // The endpoint should still be created with a fallback name
        assert!(!result.api.endpoints.is_empty());
    }
...
"#;
        ...
        // Should have all expected fields accessible
        let _api = &result.api;
