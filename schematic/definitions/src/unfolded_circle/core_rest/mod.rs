...
pub fn define_unfolded_circle_core_rest_api() -> RestApi {
    ...
    endpoints.push(Endpoint {
        ...
        description: "Upload a custom resource (icon/background/etc.) for the selected resource type via multipart form-data (`file`).".to_string(),
        request: Some(ApiRequest::form_data(vec![FormField::file("file")])),
    ...
    endpoints.push(Endpoint {
        ...
        description: "Download a single stored resource by type and identifier. Binary response supports image/audio/file resources.".to_string(),
        request: None,
        response: ApiResponse::Binary,
        headers: vec![],
        params: None,
    ...
    endpoints.push(Endpoint {
        ...
        description: "Upload and install a custom UI or web-configurator component package (`file`). Returns install/activation status.".to_string(),
        request: Some(ApiRequest::form_data(vec![FormField::file("file")])),
